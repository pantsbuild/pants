...
class RoundEngineTest(EngineTestBase, BaseTest):
  ...
  def test_lifecycle_ordering(self):
    with self.install_task('task1', goal='goal1', product_types=['1']) as task1, \
         self.install_task('task2', goal='goal1', product_types=['2'],
                           required_data=['1']) as task2, \
         self.install_task('task3', goal='goal3', product_types=['3'],
                           required_data=['2']) as task3, \
         self.install_task('task4', goal='goal4', required_data=['1', '2', '3']) as task4:
      with self.create_context(for_task_types=task1+task2+task3+task4):
  ...
  def test_lifecycle_ordering_install_order_invariant(self):
    ...
    with self.install_task('task1', goal='goal1', product_types=['1']) as task1, \
         self.install_task('task2', goal='goal1', product_types=['2'],
                           required_data=['1']) as task2, \
         self.install_task('task4', goal='goal4', required_data=['1', '2', '3']) as task3, \
         self.install_task('task3', goal='goal3', product_types=['3'],
                           required_data=['2']) as task4:
      with self.create_context(for_task_types=task1+task2+task3+task4):
        self._context = self.context(for_task_types=task1 + task2 + task3 + task4)
        self.engine.attempt(self._context, self.as_goals('goal4'))
        self.assert_actions('task1', 'task2', 'task3', 'task4')

  def test_inter_goal_dep(self):
    with self.install_task('task1', goal='goal1', product_types=['1']) as task1, \
         self.install_task('task2', goal='goal1', required_data=['1']) as task2:
      with self.create_context(for_task_types=task1+task2):
  ...
  def test_inter_goal_dep_self_cycle_ok(self):
    with self.install_task('task1', goal='goal1', product_types=['1'],
                           required_data=['1']) as task:
      with self.create_context(for_task_types=task):
        self.engine.attempt(self._context, self.as_goals('goal1'))
        self.assert_actions('task1')

  def test_inter_goal_dep_downstream(self):
    with self.install_task('task1', goal='goal1', required_data=['1']) as task1, \
         self.install_task('task2', goal='goal1', product_types=['1']) as task2:
      with self.create_context(for_task_types=task1+task2):
        with self.assertRaises(self.engine.TaskOrderError):
          self.engine.attempt(self._context, self.as_goals('goal1'))

  def test_missing_product(self):
    with self.install_task('task1', goal='goal1', required_data=['1']) as task:
      with self.create_context(for_task_types=task):
        with self.assertRaises(self.engine.MissingProductError):
          self.engine.attempt(self._context, self.as_goals('goal1'))

  def test_goal_cycle_direct(self):
    with self.install_task('task1', goal='goal1', required_data=['2'],
                           product_types=['1']) as task1, \
         self.install_task('task2', goal='goal2', required_data=['1'],
                           product_types=['2']) as task2:
      with self.create_context(for_task_types=task1+task2):
  ...
  def test_goal_cycle_indirect(self):
    with self.install_task('task1', goal='goal1', required_data=['2'],
                           product_types=['1']) as task1, \
         self.install_task('task2', goal='goal2', required_data=['3'],
                           product_types=['2']) as task2, \
         self.install_task('task3', goal='goal3', required_data=['1'],
                           product_types=['3']) as task3:
      with self.create_context(for_task_types=task1+task2+task3):
        for goal in ('goal1', 'goal2', 'goal3'):
          with self.assertRaises(self.engine.GoalCycleError):
            self.engine.attempt(self._context, self.as_goals(goal))

  def test_goal_ordering_unconstrained_respects_cli_order(self):
    with self.install_task('task1', goal='goal1') as task1, \
         self.install_task('task2', goal='goal2') as task2, \
         self.install_task('task3', goal='goal3') as task3:
      with self.create_context(for_task_types=task1+task2+task3):
  ...
  def test_goal_ordering_constrained_conflicts_cli_order(self):
    with self.install_task('task1', goal='goal1', required_data=['2']) as task1, \
         self.install_task('task2', goal='goal2', product_types=['2']) as task2:
      with self.create_context(for_task_types=task1+task2):
  ...
  def test_goal_ordering_mixed_constraints_and_cli_order(self):
    with self.install_task('task1', goal='goal1') as task1, \
         self.install_task('task2', goal='goal2') as task2, \
         self.install_task('task3', goal='goal3') as task3, \
         self.install_task('task4', goal='goal4', required_data=['5']) as task4, \
         self.install_task('task5', goal='goal5', product_types=['5']) as task5:
      with self.create_context(for_task_types=task1+task2+task3+task4+task5):
  ...
  def test_replace_target_roots(self):
    with self.install_task('task1', goal='goal1') as task1, \
         self.install_task('task2', goal='goal2', alternate_target_roots=[42]) as task2:
      with self.create_context(for_task_types=task1+task2):
  ...
  def test_replace_target_roots_conflict(self):
    with self.install_task('task1', goal='goal1', alternate_target_roots=[42]) as task1, \
         self.install_task('task2', goal='goal2', alternate_target_roots=[1, 2]) as task2:
      with self.create_context(for_task_types=task1+task2):
