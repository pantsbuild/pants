...
class FSTest(TestBase, SchedulerTestBase, metaclass=ABCMeta):
    ...
    def test_synchronously_merge_directories(self):
        with temporary_dir() as temp_dir:
            ...
            self.assertEqual(roland_snapshot.directory_digest, roland_merged)
    ...
    def test_strip_prefix(self):
        ...
        relevant_files = ("characters/dark_tower/roland", "characters/dark_tower/susannah")
        all_files = (
    ...
    def prime_store_with_roland_digest(self):
        """This method primes the store with a directory of a file named
        'roland' and contents 'European Burmese'."""
        with temporary_dir() as temp_dir:
            with open(os.path.join(temp_dir, "roland"), "w") as f:
                f.write("European Burmese")
            globs = PathGlobs(("*",), ())
