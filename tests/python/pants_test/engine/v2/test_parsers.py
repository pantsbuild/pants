...
class JsonParserTest(unittest.TestCase):
  ...
  def test_single(self):
    ...
    {
      "type_alias": "pants_test.engine.v2.test_parsers.Bob",
      "hobbies": [1, 2, 3]
    }
    """)
    results = self.parse(document)
    ...
    self.assertEqual('pants_test.engine.v2.test_parsers.Bob', results[0]._asdict()['type_alias'])
  ...
  def test_tricky_spacing(self):
    ...
    # Another that is aged.
    {"type_alias": "pants_test.engine.v2.test_parsers.Bob","age": 42}
    """).strip()
    results = self.parse(document)
    self.assertEqual([Bob(hobbies=[1, 2, 3]), {}, Bob(age=42)], results)
  ...
  def test_error_presentation(self):
    ...
    self.assertRegexpMatches(actual_lines[0],
                             r'Expecting (?:,|\',\'|",") delimiter: line 3 column 12 \(char 70\)')
    ...
    self.assertEqual(dedent("""
      In document at {filepath}:
          ...
          # Another that is imaginary aged.
       1: {{
       2:   "type_alias": "pants_test.engine.v2.test_parsers.Bob",
       3:   "age": 42i,
...
class PythonAssignmentsParserTest(unittest.TestCase):
  def test_no_symbol_table(self):
    document = dedent("""
    from pants_test.engine.v2.test_parsers import Bob
