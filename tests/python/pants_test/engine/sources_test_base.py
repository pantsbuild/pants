...
class SourcesTestBase(SchedulerTestBase):
  def sources_for(self, file_paths, relative_to=''):
    scheduler = self.mk_scheduler(
      create_fs_rules(),
      project_tree=FileSystemProjectTree(self.build_root),
    )

    file_paths_tuple = tuple((os.path.join(relative_to, path) for path in file_paths))
    ...
    return eager_fileset_with_spec(relative_to, {'globs': file_paths_tuple}, snapshot)
