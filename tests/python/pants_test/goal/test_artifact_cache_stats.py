...
class ArtifactCacheStatsTest(TestBase):
    ...
    TEST_REMOTE_ERROR = UnreadableArtifact(
        "bar", NonfatalArtifactCacheError(requests.exceptions.ConnectionError("Read time out"))
    )
    ...
    def test_add_hits(self):
        ...
        expected_hit_or_miss_files = {
            f"{self.TEST_CACHE_NAME_2}.misses": f"{self.TEST_SPEC_A} {str(self.TEST_LOCAL_ERROR.err)}\n",
        ...
        with self.mock_artifact_cache_stats(
            expected_stats, expected_hit_or_miss_files=expected_hit_or_miss_files
        ) as artifact_cache_stats:
            artifact_cache_stats.add_hits(self.TEST_CACHE_NAME_1, [self.target_b])
            artifact_cache_stats.add_misses(
                self.TEST_CACHE_NAME_1, [self.target_c], [self.TEST_REMOTE_ERROR]
            )
            artifact_cache_stats.add_misses(
                self.TEST_CACHE_NAME_2, [self.target_a], [self.TEST_LOCAL_ERROR]
            )

    @contextmanager
    def mock_artifact_cache_stats(self, expected_stats, expected_hit_or_miss_files=None):
        with temporary_dir() as tmp_dir:
            ...
            self.assertEqual(
                sorted(list(expected_hit_or_miss_files.keys())), sorted(os.listdir(tmp_dir))
            )
