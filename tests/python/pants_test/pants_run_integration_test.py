...
class PantsRunIntegrationTest(unittest.TestCase):
  ...
  def PANTS_SCRIPT_NAME(self) -> str:
    # NB: We depend on `pants.pex`, rather than `pants`, because we strip the prefix of source
    # roots, so `src/python/pants` becomes the directory `pants`
    ...
    if not Path('pants.pex').exists():
      warn_or_error(
        removal_version="1.22.0.dev0",
        deprecated_entity_description="using `./pants` for integration tests",
        hint="Integration tests now depend on the file `pants.pex`, rather than the script `pants`. "
             "This allows for integration tests to work with `--chroot` and V2. If you don't care "
             "about supporting these two test modes, add `PANTS_SCRIPT_NAME = 'pants'` to the "
             "subclass's definition.\nNote that pants.pex is a generated file that should not fall "
             "out of sync. See how the upstream Pants repository automatically generates this file: "
             "https://github.com/pantsbuild/pants/pull/8183",
      )
      return 'pants'
    return 'pants.pex'
