...
class ProtobufIntegrationTest(PantsRunIntegrationTest):
  def test_bundle_protobuf_normal(self):
    pants_run = self.run_pants(
        ['goal', 'bundle', 'src/java/com/pants/examples/protobuf:protobuf', '--bundle-deployjar'])
    ...
    out_path = os.path.join(get_buildroot(), 'dist', 'protobuf-bundle')
    java_run = subprocess.Popen(['java', '-cp', 'protobuf.jar',
  ...
  def test_bundle_protobuf_imports(self):
    pants_run = self.run_pants(
        ['goal', 'bundle', 'src/java/com/pants/examples/protobuf:protobuf-imports',
         '--bundle-deployjar'])
    self.assertEquals(pants_run.returncode, self.PANTS_SUCCESS_CODE,
                      "goal bundle run expected success, got {0}\n"
                      "got stderr:\n{1}\n"
                      "got stdout:\n{2}\n".format(pants_run.returncode,
                                                  pants_run.stderr_data,
                                                  pants_run.stdout_data))
    out_path = os.path.join(get_buildroot(), 'dist', 'protobuf-imports-bundle')
    java_run = subprocess.Popen(['java', '-cp', 'protobuf-imports.jar',
