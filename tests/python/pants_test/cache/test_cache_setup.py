...
class TestCacheSetup(TestBase):
    ...
    def create_task(self):
        return DummyTask(self.context(for_task_types=[DummyTask]), self.pants_workdir)
    ...
    def cache_factory(self, **options):
        ...
        return CacheFactory(
            create_options(options={"test": cache_options}).for_scope("test"),
            MockLogger(),
            self.create_task(),
    ...
    def test_sanitize_cache_spec(self):
        ...
        self.assertEqual(
            self.CACHE_SPEC_LOCAL_ONLY, cache_factory._sanitize_cache_spec([self.LOCAL_URI])
        )

        self.assertEqual(
            self.CACHE_SPEC_REMOTE_ONLY, cache_factory._sanitize_cache_spec([self.REMOTE_URI_1])
        )
    ...
    def test_resolve(self):
        ...
        self.assertEqual(
            CacheSpec(local=None, remote=f"{self.REMOTE_URI_1}|{self.REMOTE_URI_2}"),
            cache_factory._resolve(self.CACHE_SPEC_RESOLVE_ONLY),
        )

        self.assertEqual(
            CacheSpec(local=self.LOCAL_URI, remote=f"{self.REMOTE_URI_1}|{self.REMOTE_URI_2}"),
    ...
    def test_noop_resolve(self):
        ...
        self.assertEqual(
            self.CACHE_SPEC_LOCAL_ONLY, cache_factory._resolve(self.CACHE_SPEC_LOCAL_ONLY)
        )
        self.assertEqual(
            self.CACHE_SPEC_RESOLVE_ONLY, cache_factory._resolve(self.CACHE_SPEC_RESOLVE_ONLY)
        )
        self.assertEqual(
            self.CACHE_SPEC_LOCAL_RESOLVE, cache_factory._resolve(self.CACHE_SPEC_LOCAL_RESOLVE)
        )
    ...
    def test_cache_spec_parsing(self):
        def mk_cache(spec, resolver=None):
            ...
            self.set_options_for_scope(
                CacheSetup.subscope(DummyTask.options_scope), read_from=spec, compression=1
            )
            self.context(for_task_types=[DummyTask])  # Force option initialization.
            cache_factory = CacheSetup.create_cache_factory_for_task(
                self.create_task(), pinger=self.pinger, resolver=resolver
            )
        ...
        with temporary_dir() as tmpdir:
            cachedir = os.path.join(
                tmpdir, "cachedir"
            )  # Must be a real path, so we can safe_mkdir it.
    ...
    def test_read_cache_available(self):
        self.assertFalse(
            self.cache_factory(
                ignore=True, read=True, read_from=[self.EMPTY_URI]
            ).read_cache_available()
        )
        self.assertFalse(
            self.cache_factory(
                ignore=False, read=False, read_from=[self.EMPTY_URI]
            ).read_cache_available()
        )
        self.assertFalse(
            self.cache_factory(ignore=False, read=True, read_from=[]).read_cache_available()
        )
        self.assertIsNone(
            self.cache_factory(
                ignore=False, read=True, read_from=[self.EMPTY_URI]
            ).read_cache_available()
        )
    ...
    def test_write_cache_available(self):
        self.assertFalse(
            self.cache_factory(
                ignore=True, write=True, write_to=[self.EMPTY_URI]
            ).write_cache_available()
        )
        self.assertFalse(
            self.cache_factory(
                ignore=False, write=False, write_to=[self.EMPTY_URI]
            ).write_cache_available()
        )
        self.assertFalse(
            self.cache_factory(ignore=False, write=True, write_to=[]).write_cache_available()
        )
        self.assertIsNone(
            self.cache_factory(
                ignore=False, write=True, write_to=[self.EMPTY_URI]
            ).write_cache_available()
        )
