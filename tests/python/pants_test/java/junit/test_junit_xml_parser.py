...
class TestParseTestInfo(unittest.TestCase):
  @staticmethod
  def _raise_handler(e):
    raise e
  ...
  def test_parse_test_info_no_files(self):
    with temporary_dir() as junit_xml_dir:
      test_info = parse_test_info(junit_xml_dir, self._raise_handler)
  ...
  def test_parse_test_info_all_testcases(self):
    with temporary_dir() as junit_xml_dir:
      ...
      tests_info = parse_test_info(junit_xml_dir, self._raise_handler)
      self.assertEqual(
        {
          'testOK': {
          ...
          'testFailure': {
            'classname': 'org.pantsbuild.Failure',
            'result_code': 10000,
            'time': 0.27
          },
          'testError': {
          ...
          'testSkipped': {
            'classname': 'org.pantsbuild.Skipped',
            'result_code': 5000,
            'time': 0.1
          }
        }, tests_info)
  ...
  def test_parse_test_info_error_raise(self):
    with temporary_dir() as junit_xml_dir:
      ...
      with open(junit_xml_file, 'w') as fp:
        fp.write('<invalid></xml>')
      with self.assertRaises(ParseError) as exc:
        parse_test_info(junit_xml_dir, self._raise_handler)
      self.assertEqual(junit_xml_file, exc.exception.junit_xml_path)
      self.assertIsInstance(exc.exception.cause, XmlParser.XmlError)
  ...
  def test_parse_test_info_error_continue(self):
    with temporary_dir() as junit_xml_dir:
      ...
      collect_handler = self.CollectHandler()
      tests_info = parse_test_info(junit_xml_dir, collect_handler)
      self.assertEqual(2, len(collect_handler.errors))
      self.assertEqual({bad_file1, bad_file2}, {e.junit_xml_path for e in collect_handler.errors})
