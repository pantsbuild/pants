...
class MutexTaskMixinTest(TestBase):
    ...
    def mutex_group(self, targets=None):
        context = self.context(
            target_roots=targets, for_task_types=[RedLogViewer, BlueLogViewer, GreenLogViewer]
        )
    ...
    def test_none(self):
        assert_no_activations = functools.partial(
            self.assert_activation_error, MutexTaskMixin.NoActivationsError
        )
    ...
    def test_all_incompatible(self):
        ...
        with self.mutex_group(targets=[red, blue, green]) as (
            red_viewer,
            blue_viewer,
            green_viewer,
        ):
