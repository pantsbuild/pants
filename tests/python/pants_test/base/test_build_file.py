...
from pants.base.build_file import BuildFile, BuildFileOnDisk
...
class BuildFileTest(unittest.TestCase):
  ...
  def create_buildfile(self, path):
    return BuildFileOnDisk(self.root_dir, path)
  ...
  def testMustExistFalse(self):
    buildfile = BuildFileOnDisk(self.root_dir, "path-that-does-not-exist/BUILD", must_exist=False)
    self.assertEquals(OrderedSet([buildfile]), buildfile.family())

  def testMustExistTrue(self):
    with self.assertRaises(BuildFile.MissingBuildFileError):
      BuildFileOnDisk(self.root_dir, "path-that-does-not-exist/BUILD", must_exist=True)
    with self.assertRaises(BuildFile.MissingBuildFileError):
      BuildFileOnDisk(self.root_dir, "path-that-does-exist/BUILD", must_exist=True)
    with self.assertRaises(BuildFile.MissingBuildFileError):
      BuildFileOnDisk(self.root_dir, "path-that-does-exist/BUILD.invalid.suffix", must_exist=True)
  ...
  def test_buildfile_with_dir_must_exist_false(self):
    ...
    buildfile = BuildFileOnDisk(self.root_dir, 'grandparent/BUILD', must_exist=False)
  ...
  def test_buildfile_with_dir_must_exist_true(self):
    ...
    # in the default case.
    with self.assertRaises(BuildFile.MissingBuildFileError):
      BuildFileOnDisk(self.root_dir, 'grandparent/BUILD')
  ...
  def test_directory_called_build_skipped(self):
    ...
    buildfiles = BuildFileOnDisk.scan_buildfiles(os.path.join(self.root_dir, 'grandparent'))
  ...
  def test_scan_buildfiles_exclude_abspath(self):
    buildfiles = BuildFileOnDisk.scan_buildfiles(
  ...
  def test_invalid_root_dir_error(self):
    ...
    with self.assertRaises(BuildFile.InvalidRootDirError):
      BuildFileOnDisk('tmp', 'grandparent/BUILD')
