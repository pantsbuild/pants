...
class PayloadTest(TestBase):
    ...
    def test_jars_field_apidocs(self):
        ...
        self.assertEqual(JarsField([jar1]).fingerprint(), JarsField([jar2]).fingerprint())
    ...
    def test_primitive_field(self):
        ...
        self.assertEqual(PrimitiveField("foo").fingerprint(), PrimitiveField("foo").fingerprint())
        self.assertNotEqual(
    ...
    def test_excludes_field_again(self):
        ...
        self.assertEqual(ExcludesField([]).fingerprint(), ExcludesField().fingerprint())
        self.assertNotEqual(
            ExcludesField([Exclude("com", "foo")]).fingerprint(),
    ...
    def test_set_of_primitives_field(self):
        ...
        self.assertEqual(sopf({"one", "two"}), sopf({"two", "one"}))
        self.assertEqual(sopf(["one", "two"]), sopf(["two", "one"]))
        self.assertEqual(sopf(None), sopf(None))
        self.assertNotEqual(sopf(None), sopf(["one"]))
        self.assertNotEqual(sopf(None), sopf([]))
