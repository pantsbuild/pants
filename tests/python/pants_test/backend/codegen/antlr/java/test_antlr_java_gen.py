...
class AntlrJavaGenTest(NailgunTaskTestBase):
    ...
    def execute_antlr_test(self, expected_package, target_workdir_fun=None):
        ...
        for source in syn_target.sources_relative_to_source_root():
            ...
            self.assertTrue(
                os.path.isfile(source_path), f"{target_workdir} is not the source root for {source}"
            )
        ...
        for root, dirs, files in os.walk(target_workdir):
            for d in dirs:
                ...
                self.assertTrue(
                    os.listdir(full_dir), f"Empty directories should have been removed ({full_dir})"
                )
    ...
    """.format(
        ...
        self.assertEqual(
            fp1, fp2, f"Hash of generated synthetic target is not stable. {fp1} != {fp2}"
        )
