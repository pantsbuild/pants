...
class ExportIntegrationTest(ResolveJarsTestMixin, PantsRunIntegrationTest):
    _confs_args = ["--export-libraries-sources", "--export-libraries-javadocs"]
    ...
    def test_classifiers(self):
        with self.temporary_workdir() as workdir:
            ...
            self.assertIn("avro-1.7.7.jar", avro_lib_info.get("default"))
            self.assertIn("avro-1.7.7-tests.jar", avro_lib_info.get("tests"))
            self.assertIn("avro-1.7.7-javadoc.jar", avro_lib_info.get("javadoc"))
            self.assertIn("avro-1.7.7-sources.jar", avro_lib_info.get("sources"))
