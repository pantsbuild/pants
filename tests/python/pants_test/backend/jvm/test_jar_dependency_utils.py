...
class JarDependencyUtilsTest(unittest.TestCase):
    def test_m2_string_representation(self):
        ...
        org_name_ref_classifier = M2Coordinate(
            org="org.example", name="lib", rev="the-rev", classifier="classify"
        )

        self.assertEqual("org.example:lib:jar:classify:the-rev", str(org_name_ref_classifier))
        self.assertEqual(
            org_name_ref_classifier, M2Coordinate.from_string(str(org_name_ref_classifier))
        )
        ...
        org_name_type_classifier = M2Coordinate(
            org="org.example", name="lib", classifier="classify", ext="zip"
        )

        self.assertEqual("org.example:lib:zip:classify:", str(org_name_type_classifier))
        self.assertEqual(
            org_name_type_classifier, M2Coordinate.from_string(str(org_name_type_classifier))
        )

        org_name_type_jar_classifier = M2Coordinate(
            org="org.example", name="lib", classifier="classify", ext="jar"
        )
        ...
        class CoordinateLike:
            def __init__(self):
                for i, a in enumerate(attrs):
                    setattr(
                        self, a, chr(i + ord("a"))
                    )  # Set attrs to the first few letters in the alphabet.

        coord = CoordinateLike()
        m2 = M2Coordinate.create(coord)
        self.assertNotEqual(m2, coord)
        self.assertEqual(
            tuple(getattr(coord, a) for a in attrs), tuple(getattr(m2, a) for a in attrs)
        )
