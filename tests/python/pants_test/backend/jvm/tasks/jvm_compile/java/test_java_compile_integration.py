...
class JavaCompileIntegrationTest(BaseCompileIT):
    ...
    def test_java_compile_produces_different_artifact_depending_on_java_version(self):
        ...
        with temporary_dir() as cache_dir:
            ...
            java_7_artifact_dir = self.get_cache_subdir(cache_dir, other_dirs=[java_6_artifact_dir])
    ...
    def test_java_compile_reads_resource_mapping(self):
        ...
        with temporary_dir() as cache_dir:
            ...
            self.assert_success(
                self.run_pants(
                    ["compile", "testprojects/src/java/org/pantsbuild/testproject/annotation/main"],
                    config=config,
                )
            )
    ...
    def test_java_compile_with_corrupt_remote(self):
        """Tests that a corrupt artifact in the remote cache still results in a
        successful compile."""
