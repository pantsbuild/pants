...
class RscCompileIntegration(BaseCompileIT, AbstractTestGenerator):
    ...
    def generate_tests(cls):
        tests_with_generated_config = {
            ...
            if getattr(func, "_with_run_config", False)
        }
                    ...
                    with temporary_dir() as cache_dir:
                        ...
                        def populate_necessary_hermetic_options():
                            config["compile.rsc"].update(
                                {"incremental": False, "use_classpath_jars": False}
                            )
                        ...
                        for name, test in tests_with_generated_config.items():
                            cls.add_test(
                                "test_{}_resolver_{}_strategy_{}_worker_{}".format(
                                    name, resolver, execution_strategy.value, worker_count
                                ),
    ...
    def basic_binary(self, config):
        with self.do_command_yielding_workdir(
            "compile", "testprojects/src/scala/org/pantsbuild/testproject/mutual:bin", config=config
        ) as pants_run:
    ...
    def executing_multi_target_binary(self, config):
        pants_run = self.do_command(
            "run", "examples/src/scala/org/pantsbuild/example/hello/exe", config=config
        )
