...
class ResourcesTaskTestBase(TaskTestBase):
    ...
    def assert_products(self, task, target, count, expected_confs=None):
        ...
        for root, dirs, files in os.walk(chroot):
            classpath_rel_paths.extend(
                os.path.relpath(os.path.join(root, f), chroot) for f in files
            )
...
class MinimalImplResourcesTaskTest(ResourcesTaskTestBase):
    ...
    def test_no_resources_targets(self):
        task = self.create_resources_task(
            target_roots=[self.create_target("2"), self.create_target("a")]
        )
    ...
    def test_resources_targets(self):
        ...
        self.assertEqual(
            sorted([self.target("a:1"), self.target("a:3"), self.target("b:1")]),
    ...
    def test_incremental(self):
        task = self.create_resources_task(
            target_roots=[self.create_target("a:1"), self.create_target("b:1")]
        )
        processed_targets = task.execute()

        self.assertEqual(
            sorted([self.target("a:1"), self.target("b:1")]), sorted(processed_targets)
        )
        self.assert_products(task, self.target("a:1"), 1)
        self.assert_products(task, self.target("b:1"), 1)
        ...
        self.reset_build_graph()
        task = self.create_resources_task(
            target_roots=[self.create_target("a:1", contents="new"), self.create_target("b:1")]
        )
        processed_targets = task.execute()
    ...
    def test_custom_confs(self):
        task = self.create_resources_task(
            target_roots=[self.create_target("a:1")], confs=("a", "b")
        )
        processed_targets = task.execute()
...
class CustomInvalidationStrategyResourcesTaskTest(ResourcesTaskTestBase):
    ...
    def test_incremental(self):
        task = self.create_resources_task(
            target_roots=[self.create_target("a:1", tags=["plain"]), self.create_target("b:1")]
        )
        processed_targets = task.execute()
        ...
        task = self.create_resources_task(
            target_roots=[
                self.create_target("a:1", tags=["plain"]),
