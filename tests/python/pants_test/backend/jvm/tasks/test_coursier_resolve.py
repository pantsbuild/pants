...
class CoursierResolveTest(NailgunTaskTestBase):
    ...
    def test_resolve_with_remote_url(self):
        with self._temp_task_cache_dir():
            dep_with_url = JarDependency(
                "a", "b", "c", url="https://maven-central.storage-download.googleapis.com/repos/central/data/junit/junit/4.12/junit-4.12.jar"
            )
    ...
    def test_resolve_specific_with_sources_javadocs(self):
        with self._temp_task_cache_dir():
            ...
            with self._temp_workdir() as workdir:
                ...
                self.assertEqual(
                    sorted(["default", "sources", "javadoc"]),
    ...
    def test_resolve_ignores_jars_with_rev_left_off(self):
        """If a resolve jar leaves off the rev, we're supposed to get the latest version, but
        coursier doesn't currently support that.
