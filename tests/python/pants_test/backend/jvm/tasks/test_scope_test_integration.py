...
class ScopeTestIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_tests_pass(self):
        """This junit_tests() target tests for the presence of a particular
        class at runtime.
    ...
    def test_binary_compiles(self):
        """This should compile just fine, because the reference to the 'test'
        class is dynamic.

        This test is mostly just here to ensure the integrity of the
        binary besides the missing 'test' class. We want to make sure
        that the test-case below this one isn't failing just because of
    ...
    def test_binary_fails_to_run(self):
        """Ensure the binary does not have access to 'test'-scoped dependencies
        at runtime."""
