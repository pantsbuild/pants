...
class PrepareResourcesTest(TaskTestBase):
    ...
    def test_find_all_relevant_resources_targets(self):
        ...
        jvm_target = self.make_target(
            "jvm:target", target_type=JvmTarget, dependencies=[resources1]
        )
        java_library = self.make_target("java:target", target_type=JavaLibrary, sources=[])
        java_library2 = self.make_target(
            "java:target2", target_type=JavaLibrary, sources=[], dependencies=[resources4]
        )
        other_target = self.make_target(
            "other:target", target_type=self.NonJvmResourcesUsingTarget, dependencies=[resources3]
        )

        task = self.create_task(
            self.context(
                target_roots=[resources2, jvm_target, java_library, java_library2, other_target]
            )
        )
        relevant_resources_targets = task.find_all_relevant_resources_targets()
        self.assertEqual(
            sorted([self.target("resources:target1"), self.target("resources:target4")]),
    ...
    def test_find_all_relevant_resources_targets_transitive(self):
        ...
        alias_target = self.make_target(
            "alias:target", target_type=Target, dependencies=[resources_target]
        )
        jvm_target = self.make_target(
            "jvm:target", target_type=JvmTarget, dependencies=[alias_target]
        )
    ...
    def test_prepare_resources(self):
        ...
        resources = self.make_target(
            "resources:target", target_type=Resources, sources=["a/b.txt", "c.txt"]
        )
        with temporary_dir() as chroot:
            task.prepare_resources(resources, chroot)
            resource_files = []
