...
class ShaderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.shader = Shader(
            jarjar_classpath=[self.jarjar], executor=executor, binary_package_excludes=["javax"]
        )
    ...
    def test_assemble_custom_rules(self):
        ...
        rules = self.shader.assemble_binary_rules(
            "main",
            input_jar,
            custom_rules=[Shader.shade_class("bob"), Shader.exclude_class("fred")],
        )
    ...
    def test_runner_command(self):
        ...
        with self.shader.binary_shader(
            ...
            with open(rules_file, "r") as fp:
                ...
                self.assertEqual(
                    "rule com.google.common.base.* {}com.google.common.base.@1".format(
    ...
    def test_shading_exclude_package(self):
        self.assertEqual(
            ("com.foo.bar.**", "com.foo.bar.@1"), Shading.create_exclude_package("com.foo.bar")
        )
        self.assertEqual(
    ...
    def test_keep_package(self):
        self.assertEqual(
            ("keep", "com.foo.bar.**"), Shading.create_keep_package("com.foo.bar", True)
        )
        self.assertEqual(
            ("keep", "com.foo.bar.*"), Shading.create_keep_package("com.foo.bar", False)
        )
