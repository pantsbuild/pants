...
class TestLibcDirectorySearchFailure(TestBase):
  ...
  def setUp(self):
    ...
    self.native_toolchain = global_subsystem_instance(NativeToolchain)
  ...
  def test_libc_search_failure(self):
    with self.assertRaises(LibcDev.HostLibcDevResolutionError) as cm:
      self.native_toolchain._libc_dirs(self.platform)
    expected_msg = (
  ...
  def test_libc_search_noop_osx(self):
    self.assertEqual([], self.native_toolchain._libc_dirs(self.platform))
