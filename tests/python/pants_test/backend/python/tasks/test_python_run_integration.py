...
class PythonRunIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_run_27_and_then_3(self):
        with temporary_dir() as interpreters_cache:
            ...
            pants_run_3 = self.run_pants(
                command=[
                    "run",
                    "{}:echo_interpreter_version_3".format(self.testproject),
                    "--python-setup-interpreter-constraints={}".format(
                        self.py2_interpreter_constraint
                    ),
                    "--python-setup-interpreter-constraints={}".format(
                        self.py3_interpreter_constraint
                    ),
                ],
                config=pants_ini_config,
            )
    ...
    def test_run_2_by_option(self):
        with temporary_dir() as interpreters_cache:
            ...
            pants_run_2 = self.run_pants(
                command=[
                    "run",
                    "{}:echo_interpreter_version_2.7".format(self.testproject),
                    '--python-setup-interpreter-constraints=["{}"]'.format(
                        self.py2_interpreter_constraint
                    ),
                ],
    ...
    def test_die(self):
        command = [
            ...
            '--python-setup-interpreter-constraints=["{}", "{}"]'.format(
                self.py2_interpreter_constraint, self.py3_interpreter_constraint
            ),
            "--quiet",
        ]
        pants_run = self.run_pants(command=command)
