...
class PytestRunIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_pants_test_interpreter_selection_with_pexrc(self):
        """Test the pants test goal with intepreters selected from a
        PEX_PYTHON_PATH defined in a pexrc file on disk."""
    ...
    def test_pants_test_interpreter_selection_with_option_2(self):
        """Test that the pants test goal properly constrains the
        SelectInterpreter task to Python 2 using the '--python-setup-
        interpreter-constraints' option."""
        with temporary_dir() as interpreters_cache:
            pants_ini_config = {
                "python-setup": {
    ...
    def test_pants_test_interpreter_selection_with_option_3(self):
        """Test that the pants test goal properly constrains the
        SelectInterpreter task to Python 3 using the '--python-setup-
        interpreter-constraints' option."""
        with temporary_dir() as interpreters_cache:
            pants_ini_config = {
                "python-setup": {
