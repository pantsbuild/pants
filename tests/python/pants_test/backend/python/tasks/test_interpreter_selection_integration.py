...
class InterpreterSelectionIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_binary_uses_own_compatibility(self):
        """Tests that a binary target uses its own compatiblity, rather than
        including that of its transitive dependencies."""
        ...
        with temporary_dir() as distdir:
            config = {"GLOBAL": {"pants_distdir": distdir}}
            args = ['--python-setup-interpreter-constraints=["CPython>=3.6,<4"]']
