...
from pants.backend.python.rules.resolve_requirements import (ResolveRequirementsRequest,
...
class TestResolveRequirements(TestBase):
  ...
  def create_pex_and_open_pex_info(
    ...
    with zipfile.ZipFile("test.pex", "r") as pex:
  ...
  def test_resolves_dependencies(self):
    requirements = {"", "", ""}
    pex_info = self.create_pex_and_open_pex_info(requirements=requirements)
    self.assertEqual(set(pex_info[""]), requirements)

  def test_entry_point(self):
    entry_point = ""
    pex_info = self.create_pex_and_open_pex_info(entry_point=entry_point)
  ...
  def test_interpreter_constraints(self):
    ...
    pex_info = self.create_pex_and_open_pex_info(interpreter_constraints=constraints)
