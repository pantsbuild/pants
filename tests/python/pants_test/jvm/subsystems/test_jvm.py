...
class TestJvm(TestBase):
    def create_JVM(self, **kwargs):
        ...
        self.context(for_subsystems=[JVM], options={JVM.options_scope: kwargs})
    ...
    def test_implicit_via_debug_port(self):
        ...
        for option in jvm_options:
            if option.startswith("-Xrunjdwp:"):
                jdwp_options = dict(
                    kv.split("=", 1) for kv in option.replace("-Xrunjdwp:", "").split(",")
                )
                assert "address" in jdwp_options
                assert "1137" in jdwp_options["address"]
        assert (
            jdwp_options is not None
        ), "Expected jdwp options to be present specifying the debug port."
