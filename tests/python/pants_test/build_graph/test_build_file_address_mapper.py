...
class BuildFileAddressMapperTestBase(TestBase):
  def build_file_address_mapper(self):
    return BuildFileAddressMapper(self.build_file_parser, FileSystemProjectTree(self.build_root))
...
class BuildFileAddressMapperTest(BuildFileAddressMapperTestBase):

  def test_resolve(self):
    self.add_to_build_file('BUILD', 'target(name="foo")')
    address, addressable = self.build_file_address_mapper().resolve(Address.parse('//:foo'))
  ...
  def test_is_valid_single_address(self):
    ...
    self.assertFalse(self.build_file_address_mapper().is_valid_single_address(SingleAddress('', 'bad_spec')))
    self.assertTrue(self.build_file_address_mapper().is_valid_single_address(SingleAddress('', 'foozle')))
  ...
  def test_scan_addresses(self):
    ...
    self.assertEqual({BuildFileAddress(rel_path=self.build_path('BUILD'), target_name='foo'),
                      BuildFileAddress(rel_path=self.build_path('subdir/BUILD'), target_name='bar'),
                      BuildFileAddress(rel_path=self.build_path('subdir/BUILD.suffix'), target_name='baz')},
                     self.build_file_address_mapper().scan_addresses())
  ...
  def test_scan_addresses_with_root(self):
    ...
    self.assertEqual({BuildFileAddress(rel_path=self.build_path('subdir/BUILD'), target_name='bar'),
                       BuildFileAddress(rel_path=self.build_path('subdir/BUILD.suffix'), target_name='baz')},
                      self.build_file_address_mapper().scan_addresses(root=subdir))
  ...
  def test_raises_invalid_build_file_reference(self):
    ...
    with self.assertRaisesRegexp(BuildFileAddressMapper.InvalidBuildFileReference,
      ...
      self.build_file_address_mapper().spec_to_address('//non-existent-path:a')
    with self.assertRaisesRegexp(BuildFileAddressMapper.InvalidBuildFileReference,
                                 '^Spec : has no name part\s+when translating spec :'):
      self.build_file_address_mapper().spec_to_address(':')
  ...
  def test_raises_address_not_in_one_build_file(self):
    ...
    with self.assertRaisesRegexp(BuildFileAddressMapper.AddressNotInBuildFile,
                                 ...
                                 'Perhaps you meant:'
                                 '\s+:foo$'):
      self.build_file_address_mapper().resolve(address)
...
class BuildFileAddressMapperWithIgnoreTest(BuildFileAddressMapperTestBase):
  def build_file_address_mapper(self):
    return BuildFileAddressMapper(self.build_file_parser, FileSystemProjectTree(self.build_root),
  ...
  def test_scan_from_address_mapper(self):
    ...
    self.add_to_build_file('subdir/BUILD', 'target(name="bar")')
    self.assertEqual(
      {BuildFileAddress(rel_path='BUILD', target_name='foo')},
      self.build_file_address_mapper().scan_addresses())

  def test_scan_from_context(self):
    self.add_to_build_file('BUILD', 'target(name="foo")')
    self.add_to_build_file('subdir/BUILD', 'target(name="bar")')
    graph = self.context(address_mapper=self.build_file_address_mapper()).scan()
    self.assertEqual([target.address.spec for target in graph.targets()], ['//:foo'])


class BuildFileAddressMapperScanTest(BuildFileAddressMapperTestBase):
 ...
 Loading addresses from 'bad/a' failed.$"""
  ...
  def test_bad_build_files(self):
    ...
    with self.assertRaisesRegexp(AddressLookupError, self.NO_FAIL_FAST_RE):
      list(self.build_file_address_mapper().scan_specs([DescendantAddresses('')], fail_fast=False))
  ...
  def test_bad_build_files_fail_fast(self):
    ...
    with self.assertRaisesRegexp(AddressLookupError, self.FAIL_FAST_RE):
      list(self.build_file_address_mapper().scan_specs([DescendantAddresses('')], fail_fast=True))
  ...
  def assert_scanned(self, specs_strings, expected, address_mapper=None):
    ...
    address_mapper = address_mapper or self.build_file_address_mapper()
