...
class TypedCollectionTest(TypeConstraintTestBase):
    def test_str_and_repr(self):
        ...
        collection_of_multiple_subclasses = TypedCollection(SubclassesOf(self.A, self.B))
        self.assertEqual(
            "TypedCollection(SubclassesOf(A or B))", str(collection_of_multiple_subclasses)
        )
        self.assertEqual(
    ...
    def test_no_complex_sub_constraint(self):
        ...
        with self.assertRaisesWithMessage(
            TypeError,
            "constraint for collection must be a TypeOnlyConstraint! was: {}".format(
                sub_collection
            ),
        ):
...
class SomeTypedDatatype(datatype([("val", int)])):
    pass
...
class AnotherTypedDatatype(datatype([("string", str), ("elements", list)])):
    pass


class WithExplicitTypeConstraint(datatype([("a_string", str), ("an_int", Exactly(int))])):
    pass


class MixedTyping(datatype(["value", ("name", str)])):
    pass
...
class WithCollectionTypeConstraint(datatype([("dependencies", TypedCollection(Exactly(int)))])):
    pass
...
class CamelCaseWrapper(datatype([("nonneg_int", NonNegativeInt)])):
    pass
...
class DatatypeTest(TestBase):
    ...
    def test_invalid_field_name(self):
        ...
        with self.assertRaisesWithMessage(
            ValueError, "Field names cannot start with an underscore: '_no_leading_underscore'"
        ):
    ...
    def test_override_eq_disallowed(self):
        ...
        with self.assertRaisesWithMessage(
            TypeCheckError, "type check error in class OverridesEq: Should not override __eq__."
        ):
            OverridesEq(1)
...
class TypedDatatypeTest(TestBase):
    def test_class_construction_errors(self):
        ...
        with self.assertRaisesWithMessage(TypeError, expected_msg):
            ...
            class NoFields(datatype()):
                pass

        expected_msg = "Type names and field names must be valid identifiers: \"<class 'str'>\""
        with self.assertRaisesWithMessage(ValueError, expected_msg):

            class JustTypeField(datatype([str])):
                pass

        expected_msg = "Type names and field names must be valid identifiers: '3'"
        with self.assertRaisesWithMessage(ValueError, expected_msg):

            class NonStringField(datatype([3])):
                pass

        expected_msg = "Type names and field names must be valid identifiers: '32'"
        with self.assertRaisesWithMessage(ValueError, expected_msg):

            class NonStringTypeField(datatype([(32, int)])):
                pass

        expected_msg = "Encountered duplicate field name: 'field_a'"
        with self.assertRaisesWithMessage(ValueError, expected_msg):

            class MultipleSameName(datatype(["field_a", "field_b", "field_a"])):
                pass

        expected_msg = "Encountered duplicate field name: 'field_a'"
        with self.assertRaisesWithMessage(ValueError, expected_msg):

            class MultipleSameNameWithType(datatype(["field_a", ("field_a", int)])):
                pass

        expected_msg = (
            "type spec for field 'a_field' was not a type or TypeConstraint: " "was 2 (type 'int')."
        )
        with self.assertRaisesWithMessage(TypeError, expected_msg):

            class InvalidTypeSpec(datatype([("a_field", 2)])):
                pass
    ...
    def test_instance_construction_by_repr(self):
        ...
        self.assertEqual(
            str(some_object),
            "WithExplicitTypeConstraint(a_string<Exactly(str)>=asdf, an_int<Exactly(int)>=45)",
        )
        self.assertEqual(
            repr(some_object), "WithExplicitTypeConstraint(a_string='asdf', an_int=45)"
        )
        ...
        self.assertEqual(
            repr(wrapped_nonneg_int), "CamelCaseWrapper(nonneg_int=NonNegativeInt(an_int=45))"
        )
        self.assertEqual(
    ...
    def test_instance_with_collection_construction_str_repr(self):
        ...
        self.assertEqual(
            "WithCollectionTypeConstraint(dependencies<TypedCollection(Exactly(int))>=[3])",
            str(obj_with_collection),
        )
        self.assertEqual(
            "WithCollectionTypeConstraint(dependencies=[3])", repr(obj_with_collection)
        )
    ...
    def test_instance_construction_errors(self):
        ...
        expected_msg = """type check error in class CamelCaseWrapper: 1 error type checking constructor arguments:
field 'nonneg_int' was invalid: value 3 (with type 'int') must satisfy this type constraint: Exactly(NonNegativeInt)."""
        with self.assertRaisesWithMessage(TypedDatatypeInstanceConstructionError, expected_msg):
    ...
    def test_type_check_errors(self):
        ...
        expected_msg = """type check error in class SomeTypedDatatype: 1 error type checking constructor arguments:
field 'val' was invalid: value [] (with type 'list') must satisfy this type constraint: Exactly(int)."""
        ...
        expected_msg = """type check error in class AnotherTypedDatatype: 1 error type checking constructor arguments:
field 'elements' was invalid: value 'should be list' (with type 'str') must satisfy this type constraint: Exactly(list)."""
        ...
        expected_msg = """type check error in class AnotherTypedDatatype: 2 errors type checking constructor arguments:
...
field 'elements' was invalid: value 'should be list' (with type 'str') must satisfy this type constraint: Exactly(list)."""
        ...
        expected_msg = """type check error in class NonNegativeInt: 1 error type checking constructor arguments:
field 'an_int' was invalid: value 'asdf' (with type 'str') must satisfy this type constraint: Exactly(int)."""
        ...
        expected_msg = """type check error in class WithSubclassTypeConstraint: 1 error type checking constructor arguments:
...
Note that objects matching {} are not considered iterable.""".format(
            TypedCollection.exclude_iterable_constraint
        )
        with self.assertRaisesWithMessage(TypeCheckError, expected_msg):
...
field 'dependencies' was invalid: in wrapped constraint TypedCollection(Exactly(int)) matching iterable object [3, 'asdf']: value 'asdf' (with type 'str') must satisfy this type constraint: Exactly(int)."""
    ...
    def test_copy_failure(self):
        ...
        expected_msg = """type check error in class AnotherTypedDatatype: error in namedtuple() base constructor: __new__() got an unexpected keyword argument 'nonexistent_field'"""
        with self.assertRaisesWithMessage(TypeCheckError, expected_msg):
...
field 'elements' was invalid: value 3 (with type 'int') must satisfy this type constraint: Exactly(list)."""
    ...
    def test_enum_instance_creation(self):
        ...
        expected_msg = "type check error in class SomeEnum: Value 3 must be one of: [1, 2]."
    ...
    def test_enum_resolve_variant(self):
        ...
        self.assertEqual(3, one_enum_instance.resolve_for_enum_variant({1: 3, 2: 4}))
        self.assertEqual(4, two_enum_instance.resolve_for_enum_variant({1: 3, 2: 4}))
...
type check error in class SomeEnum: pattern matching must have exactly the keys [1, 2] (was: [1, 2, 3])""",
            ...
            one_enum_instance.resolve_for_enum_variant({1: 3, 2: 4, 3: 5})
...
type check error in class SomeEnum: pattern matching must have exactly the keys [1, 2] (was: [1])""",
            ...
            one_enum_instance.resolve_for_enum_variant({1: 3})
