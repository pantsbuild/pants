...
class TypedDatatypeTest(BaseTest):
  ...
  def test_instance_construction_errors(self):
    ...
    expected_msg = "Error constructing SomeTypedDatatype: __new__() got an unexpected keyword argument 'something'"
    self.assertEqual(str(cm.exception), expected_msg)

    # not providing all the fields
    with self.assertRaises(TypeError) as cm:
      SomeTypedDatatype()
    expected_msg = "Error constructing SomeTypedDatatype: __new__() takes exactly 2 arguments (1 given)"
    self.assertEqual(str(cm.exception), expected_msg)

    # unrecognized fields
    with self.assertRaises(TypeError) as cm:
      SomeTypedDatatype(3, 4)
    expected_msg = "Error constructing SomeTypedDatatype: __new__() takes exactly 2 arguments (3 given)"
    self.assertEqual(str(cm.exception), expected_msg)
...
field 'nonneg_int' was invalid: value 3 (with type 'int') must satisfy this type constraint: Exactly(NonNegativeInt).""")
    ...
    expected_msg = "Error constructing CamelCaseWrapper: __new__() got an unexpected keyword argument 'a'"
    self.assertEqual(str(cm.exception), expected_msg)
