...
class TaskTestBase(TestBase):
    ...
    def cache_check(self, expected_num_artifacts=None):
        """Sets up a temporary artifact cache and checks that the yielded-to
        code populates it.
...
class ConsoleTaskTestBase(TaskTestBase):
    ...
    def execute_task(self, targets=None, options=None):
        """Creates a new task and executes it with the given config, command
        line args and targets.
    ...
    def execute_console_task_given_context(self, context, extra_targets=None):
        """Creates a new task and executes it with the context and extra
        targets.
...
class DeclarativeTaskTestMixin:
    """Experimental mixin for task tests allows specifying tasks to be run
    before or after the task.

    Calling `self.invoke_tasks()` will create instances of and execute
    task types in `self.run_before_task_types()`, then `task_type()`,
    then `self.run_after_task_types()`.
    """
    ...
    def _create_task(self, task_type, context):
        """Helper method to instantiate tasks besides self._testing_task_type
        in the test workdir."""
