...
class TestWatchmanLauncher(BaseTest):
  ...
  def test_options_defaults(self):
    self.assertIsNone(self.watchman_launcher.options.watchman_path)
    self.assertEquals(self.watchman_launcher.options.log_level, 1)
  ...
  def test_maybe_launch(self):
    ...
    self.watchman_launcher.watchman = mock_watchman
