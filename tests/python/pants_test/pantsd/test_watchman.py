...
class TestWatchman(TestBase):
    ...
    def setUp(self):
        ...
        with unittest.mock.patch.object(
            Watchman, "_is_valid_executable", **self.PATCH_OPTS
        ) as mock_is_valid:
    ...
    def test_resolve_watchman_path_provided_exception(self):
        with self.assertRaises(Watchman.ExecutionError):
            self.watchman = Watchman(
                "/fake/path/to/watchman", metadata_base_dir=self.subprocess_dir
            )

    def test_maybe_init_metadata(self):
        # TODO(#7106): is this the right path to patch?
        with unittest.mock.patch(
            "pants.pantsd.watchman.safe_mkdir", **self.PATCH_OPTS
        ) as mock_mkdir, unittest.mock.patch(
            "pants.pantsd.watchman.safe_file_dump", **self.PATCH_OPTS
        ) as mock_file_dump:
    ...
    def test_construct_cmd(self):
        output = self.watchman._construct_cmd(
            ["cmd", "parts", "etc"], "state_file", "sock_file", "pid_file", "log_file", "log_level"
        )
    ...
    def test_launch(self):
        with unittest.mock.patch.object(
            ...
            Watchman, "write_pid"
        ) as mock_writepid, unittest.mock.patch.object(
