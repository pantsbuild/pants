...
Having a dedicated source field will let us filter the targets based on the fact that they have a `ProjectVersionSourceField` instead of checking what their alias is. This means we can refactor how we collect the relevant targets from:
...
With the current implementation, we have simply returned the contents of the file as is. We may want to add some validation, for instance, to check that a version string follows a semver convention. Let's learn how to [bring a 3rd party Python package](https://www.pantsbuild.org/docs/plugins-overview#thirdparty-dependencies) into our plugin to do that! 

To start depending on the [packaging](https://pypi.org/project/packaging/) package in our in-repo plugin, we must extend the `pants.toml` file:
...
@goal_rule
async def goal_show_project_version(
    console: Console, targets: Targets, project_version_subsystem: ProjectVersionSubsystem
) -> ProjectVersionGoal:
    ...
...
Let's run our goal with a new `--as-json` flag:
...
When the `tailor` goal is run, the build graph is analyzed to see when `PutativeTargetsRequest` is needed, i.e. to find out if there are any files (yet unknown to Pants) that look like they could potentially be made targets. For instance, if there is a `requirements.txt` file, a `python_requirement` target is created and when there is a Python `test_` module, a `python_test` target is created. To be able to extend the `tailor` goal (to allow generation of custom targets), we need to "extend" the build graph: we ask Pants to also run our rule when searching for files that maybe should have a target created.    
...
If you have multiple projects, being able to generate the targets automatically will come in very handy. You would also likely want to run the `tailor` goal in the check mode to confirm that new projects created have a `version_file` target. Remove the `version_file` target from the `myapp/BUILD` file and re-run the `tailor` goal:
...
to get the tag of the project and compare external /usr/bin/git
