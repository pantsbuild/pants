...
been generated to match. If you have a [custom tool lockfile](https://www.pantsbuild.org/2.29/docs/python/overview/lockfiles#lockfiles-for-tools) then these can and should already have been [overridden in config](https://www.pantsbuild.org/2.29/docs/python/overview/lockfiles#interpreter-constraints). If you are specifically relying on a Python 3.8 interpreter to run a tool, then
you will need to generate a custom lockfile for that tool with the appropriate interpreter constraints.
