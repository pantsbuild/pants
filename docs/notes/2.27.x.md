...
Process workunits now include `action_digest` and `command_digest` in their artifacts when a remote cache is used. These digests can be used to inspect the contents of the action or command on the remote cache.
