...
[Fixed](https://github.com/pantsbuild/pants/pull/22128) a bug where missing or exited `docker_environment` containers (i.e. containers killed or remove by some outside process) break pantsd and requires a pantsd restart. Now pants will attempt to start a new container for the `docker_environment`.
