...
Process workunits now include `action.digest` and `command.digest` in their metadata when a remote cache is used. These digests can be used to inspect the contents of the action or command on the remote cache.
