...
`[pex-cli].version` format changed from `"v2.33.4"` to `"2.33.4"`. To keep the
old format set `[pex-cli].url_template` to
`https://github.com/pex-tool/pex/releases/download/{version}/pex` and manually
set `[pex-cli].version` and `[pex-cli].known_versions`.
