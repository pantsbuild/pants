...
Added a new `[python-infer].ignored_string_imports` option to ignore selected string-import candidates during Python
dependency inference. Matching supports exact modules and dotted-child prefixes (for example, ignoring
`foo.generated` also ignores `foo.generated.loader`).
