...
Fixed a bug that caused interactive processes to hang when reading from the terminal ([#23628](https://github.com/pantsbuild/pants/issues/23628)).
...
`buf` can now be used as an alternate code generator for Python sources, as a
replacement for `protoc`. Set `protobuf_generator="buf"` on a `protobuf_sources`
or `protobuf_source` target to generate code with `buf generate` driven by a
`buf.gen.yaml` template, where plugins and output paths come from the template
rather than from Pants. The template is resolved per-target via `buf_gen_template`,
falling back to `[buf].gen_template`, then to a `buf.gen.yaml` discovered at the
repository root.
