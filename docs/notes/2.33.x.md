...
Fixed a bug which prevented resolve-specific interpreter constraints from taking effect when `[python].default_to_resolve_interpreter_constraints` was enabled. Instead, code would fall back to the global interpreter constraints which is not correct when `default_to_resolve_interpreter_constraints` is in use.
...
As part of fixing the bug mentioned above in the Python backend with resolve-specific interpreter constraints, the `FieldSet` machinery now supports `FieldSet` fields with `Field | None` types and not just `Field` subclass types. For a declaration `foo: SomeField | None` on a `FieldSet`, `foo` will be `None` if the underlying target does not have a the field compatible with `SomeField`. For a declartion `foo: SomeField`, the existing behavior remains in effect to just instantiate `SomeField` with its default value.
