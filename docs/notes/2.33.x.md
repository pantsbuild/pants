...
NodeJS-based tool subsystems (`prettier`, `pyright`, `@redocly/cli`, `@stoplight/spectral-cli`, `openapi-format`) now ship with a bundled lockfile that pins the tool's transitive dependencies when run with the default tool version. To pin dependencies for a customised tool (e.g. an overridden `version`), set `[<tool>].lockfile` to a path in your repository and run `pants generate-lockfiles --resolve=<tool>` to generate it.
