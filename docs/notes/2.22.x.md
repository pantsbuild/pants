...
The two systems are expected to be more-or-less functionally identical, and we plan to phase out the legacy system in a future release. However to ensure that this transition is not disruptive, this release runs both systems and compares their results, issuing a warning if they differ. 
