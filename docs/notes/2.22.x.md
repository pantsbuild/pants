...
A backend linter to run secret scanning via [trufflehog](https://trufflesecurity.com/trufflehog) was added. The backend can be enabled by adding `pants.backend.tools.trufflehog` to `backend_packages`. The backend supports ignoring certain paths by adding, for example:
```
[trufflehog]
exclude_options = ["**/tests/*"]
```
