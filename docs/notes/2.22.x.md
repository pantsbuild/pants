...
Scala dependency inference used to ignore self-type annotations, which lead to not being able to detect dependencies between files in the same package where self-type annotations were used. Moving forward if self-type annotations are being used and no import present for the given type, it will be assume the referenced type is defined in the same package.
