...
Deprecate implicit resolution of PEX `platform` values for Function-as-a-Service targets, now that `complete_platforms` are available for both AWS Lambda and Google Cloud Functions.
