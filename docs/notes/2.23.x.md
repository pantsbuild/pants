...
This backend adds experimental support for building these package types: `deb`. This plugin does not (yet) support building `rpm`, `apk`, and `archlinux` packages as well as `ipk` which is a recent addition to `nFPM`, though `ipk` support should be fairly straight forward to add if someone wants to work on it.
