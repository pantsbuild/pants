...
New advanced options `--downloads-intrinsic-error-delay` and `--downloads-intrinsic-max-retries` allow configuration of the retry behavior when retryable errors are encountered when the engine is downloading files.
