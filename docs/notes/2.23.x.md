...
A new option `remote_cache_log_cache_misses` has been added. When enabled, this option will log remote cache misses at the INFO level, which can be helpful for debugging remote cache issues or investigating reproducibility problems. By default, cache misses are only logged at the DEBUG level.
