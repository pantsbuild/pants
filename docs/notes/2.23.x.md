...
The `shell_command` and `adhoc_tool` target types now check whether or not outputs were actually captured. The new `outputs_match_error_behavior` and `outputs_match_mode` fields on both target types configure how the checks are performed.
