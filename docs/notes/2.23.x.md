...
New advanced options `--downloads-intrinsic-retry-delay` and `--downloads-intrinsic-max-attempts` allow configuration of the retry behavior when retryable errors are encountered when the engine is downloading files.
