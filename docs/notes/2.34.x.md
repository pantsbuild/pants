...
- A `docker_image` that depends on another `docker_image` is no longer invalidated when only the upstream image's tags change, making `{pants.hash}` usable as a tag for skipping unnecessary rebuilds ([#22575](https://github.com/pantsbuild/pants/issues/22575)).
...
Fixed a bug where a `docker_image` that depends on another `docker_image` had an unstable build context, and so an unstable `{pants.hash}`, whenever the upstream image's tags changed ([#22575](https://github.com/pantsbuild/pants/issues/22575)). The upstream image's `docker-info.json` metadata file records its tags, which commonly embed a timestamp or another per-build value, and that file was part of the downstream build context. The context now holds a `docker-info.stable.json` file carrying only the upstream image ID, so downstream images are invalidated by a change to the upstream image's content rather than to its tags.
