...
Added a default module mapping for `testcontainers-opensearch`, which provides `testcontainers.opensearch`.
...
The new option `[golang].third_party_target_generation` controls which third-party packages a `go_mod` generates targets for. The default `build-list` keeps today's behavior, generating targets for every package of every module in the module build list. `imported` instead generates targets only for third-party packages reachable through the import graph from the `go_mod`'s own first-party code, and stops target generation from downloading and analyzing modules that nothing imports.

The build list over-approximates what a repository actually uses: minimal version selection resolves at module granularity, so importing one package from a module puts that whole module, and everything it requires, on the list. Because target generation must analyze each listed module, it also downloads each one, whether or not anything imports it -- and the cost is independent of what you asked to build.

Opting in changes two behaviors: third-party packages nothing imports no longer appear in `pants list ::`, and hand-written addresses pointing at such a package no longer resolve. Imports guarded by build tags, imports for other platforms, the `tools.go` pattern, and `tool` directives in `go.mod` are all treated as imports, so they still generate targets. See [#20340](https://github.com/pantsbuild/pants/issues/20340).
