...
Pant's integration with mypy requires a single sqlite `.db` to atomically manage.  Starting in [version 2.0](https://mypy-lang.blogspot.com/2026/05/mypy-20-relased.html) mypy defaults to sharding across multiple db files.  When using versions of mypy >=2 Pants will no unconditionally set `--sqlite-num-shards=1` so cacheing continues to work.  No user action or configuration change is needed.
