...
The `python_aws_lambda_function`, `python_aws_lambda_layer` and `python_google_cloud_function` targets have a new `link_python` field. With `layout="venv"`, it points the created venv's Python at the given path instead of the interpreter that laid the venv out, which is what allows building a venv for the foreign platform the artifact will run on: for example, `link_python="/var/lang/bin/python3.13"` for an AWS Lambda runtime.

The default version of [Pex](https://github.com/pex-tool/pex) used by the Python backend has been upgraded to [`v2.103.1`](https://github.com/pex-tool/pex/releases/tag/v2.103.1). Of particular note for Pants users:

 - [Support creating a venv for a foreign platform](https://github.com/pex-tool/pex/pull/3279).
