...
The new `[GLOBAL].cache_key_excluded_env_vars` option names environment variables whose *values* are left out of process cache keys. A process still receives the variable; only the key its result is stored under ignores it. This is for a variable a process needs but that cannot change its output — a credential-helper config path that is a fresh temporary directory on every CI job, or build metadata such as `BUILDKITE_BUILD_ID` that a test process reads only to label a report — which otherwise makes every process that receives it uncacheable across runs, however deterministic the process is. Entries are exact names, or a single trailing `*` for a prefix match. Naming a variable that *does* change a process's output will serve you a stale result with nothing to indicate it, so keep the list short and obviously safe. The option is ignored when remote execution is enabled, where the cache key is also the only means of delivering environment variables to the worker.
