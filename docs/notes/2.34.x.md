...
The [`sandboxer` sidecar binary](https://www.pantsbuild.org/blog/2025/06/29/introducing-the-sandboxer) is now enabled by default. If this causes any issues you can disable it by setting `sandboxer = false` in the `[GLOBAL]` section of `pants.toml`, but please [report](https://github.com/pantsbuild/pants/issues/new/choose) the issues to us.
