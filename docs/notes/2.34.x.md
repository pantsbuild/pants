...
Building the repo-wide reverse dependency graph, used by the `dependents` goal and by `--changed-dependents`, is now logged at `INFO` instead of `DEBUG`. This work resolves dependencies for every target in the repo and dominates the runtime of those invocations in large repos, so a default-verbosity run now reports it and how long it took: `Completed: Map all targets to their dependents (42.3s)`.
