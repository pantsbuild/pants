...
The code for the main Pants Rust crate lives in `src/rust/engine`. It is part of a top-level `Cargo.toml` file at `src/rust/Cargo.toml` defining a Cargo workspace containing a number of other subcrates, which live in subdirectories of `src/rust`. Defining multiple subcrates in this way allows changes affecting one subcrate to avoid affecting other subcrates and triggering more recompilation than is necessary.
...
To run for a specific crate, such as the `fs` crate, run:
...
That will produce `src/rust/target/debug/fs_util`.
...
```bash
$ src/rust/target/debug/fs_util --local-store-path=${HOME}/.cache/pants/lmdb_store directory cat-proto --output-format=recursive-file-list <digest> <len>
```
