...
The Python Build Standalone provider needs to be updated with new upstream releases.  There are *many* artifacts here, so the hashes are stored in a json file that is updated by running (from the Pants repo root):

```bash
python3 build-support/bin/generate-pbs-urls.py
```
...
A pull request might be merged without being configured for cherry-picking, and we decide later that it should be. To cherry-pick in this case:
