...
The Python Build Standalone provider needs to be updated with new upstream releases.  There are *many* artifacts here, so the hashes are stored in a json file that is updated by running:
