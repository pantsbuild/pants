...
1. Create pull request adding the release notes file for the next version in `docs/notes/`, e.g. if you're releasing 2.8.0a0, create `docs/notes/2.9.x.md`.
   1. Copy and rename the `2.x.y-template.md` to the upcoming version (e.g. `2.33.x.md`).
   2. Update the primary heading in the Markdown file to the upcoming version.
2. NB. this can merge after the release pull request, but should be available soon so that pull requests that land in `main` can update it
