...
:::note AI/LLM generation is for *code* changes only. All community interactions, including comments, discussion, issues, PR titles, and descriptions *must be composed by a human*. Slack chat is for humans, there are now countless other services if you want a chatbot. Be respectful to one another and mindful of the person on the other end.

An exception is LLM-assisted translations. Please note when this is in use (ex: "I am translating from LANGUAGE with an LLM").
:::

This policy was inspired by the [ghostty AI Assistance Notice](https://github.com/ghostty-org/ghostty/blob/2aa773a23a87289175bd022dfb617a5e8c27e824/AI_POLICY.md) and the [Jellyfin LLM/"AI" Development Policy](https://jellyfin.org/docs/general/contributing/llm-policies/). For further discussion of LLM prose and social contracts, see [Using LLMs at Oxide](https://rfd.shared.oxide.computer/rfd/0576).
...
- the PR has release notes, by having changes in `docs/notes/`
