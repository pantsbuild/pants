...
RUN uv tool install \
  ...
  --python 3.11
```
You can avoid hardcoding wheel wheel URLs and satisfy firewall constraints by uploading these pants wheels on your private [package indexes](https://docs.astral.sh/uv/concepts/indexes/) and use these as per `uv tool`'s documentation (i.e. using `uv tool --index-url`).
