...
4. Make a request for the config files in a rule for running the tool. Use `find_config_file(ConfigFilesRequest(...))` to get the config files. The resulting `ConfigFiles` instance has a snapshot containing the config files (or will be empty if none are found). You can merge these with the other digests to pass the files to your `Process`. If a custom value was provided for the config file, you may need to pass that as an argument to the `Process`. You may also need to register rules from `pants.core.util_rules.config_files`.
