...
This guide walks through each step of adding Protobuf to generate Python sources. See [here](https://github.com/pantsbuild/pants/tree/main/src/python/pants/backend/codegen/protobuf) for the final result.
:::
...
If you used `ExternalTool` in step 1, you will use `download_external_tool()` to install the tool. Be sure to merge this with the `protocol_sources` and any other relevant input digests via `merge_digests()`.
...
async def generate_python_from_protobuf(
    ...
    download_protoc_call = download_external_tool(protoc.get_request(platform))
    ...
    all_stripped_sources_call = strip_source_roots(
        **implicitly(SourceFilesRequest(
            (tgt.get(Sources) for tgt in transitive_tgts.closure),
            for_sources_types=(ProtobufSources,)
        ))
    )
    target_stripped_sources_call = strip_source_roots(
        **implicitly(SourceFilesRequest(
            [request.protocol_target[ProtobufSources]]
        ))
    )
    ...
    (
        ...
        download_protoc_call,
        all_stripped_sources_call,
        target_stripped_sources_call,
    )
    ...
    normalized_digest, source_root = await concurrently(
        ...
        get_source_roots(source_root_request),
    )
    source_root_restored = (
        await add_prefix(AddPrefix(normalized_digest, source_root.path))
        if source_root.path != "."
