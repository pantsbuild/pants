...
If you used `ExternalTool` in step 1, you will use `download_external_tool()` to install the tool.

Typically, you will use `determine_source_files()` to get all the sources you want to run your linter on.

If you have a `--config` option, you should use `path_globs_to_digest()` to find the config file and include it in the `input_digest`.

Use `merge_digests()` to combine the different inputs together, such as merging the source files, config file, and downloaded tool.

Usually, you will use `execute_process()` to run a subprocess (see [Processes](../the-rules-api/processes.mdx)) and get a FallibleProcessResult. We use the fallible variant because Pants should not throw an exception if the linter returns a non-zero exit code. Then, you can use `LintResult.from_fallible_process_result()` to convert this into a `LintResult`.
...
async def run_shellcheck(
    ...
    download_shellcheck_call = download_external_tool(
        shellcheck.get_request(platform),
    )

    sources_call = determine_source_files(
    ...
    config_digest_call = path_globs_to_digest(
        PathGlobs(
    ...
    downloaded_shellcheck, sources, config_digest = await concurrently(
        download_shellcheck_call, sources_call, config_digest_call
    )
