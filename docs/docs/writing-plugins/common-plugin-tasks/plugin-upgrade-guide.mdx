...
@rule
async def my_rule(zip_binary: ZipBinary) -> MyOutput:
    return MyOutput()
...
addresses = await Get(Addresses, UnparsedAddressInputs(["//:addr1", "project/addr2"], owning_address=None))

# Or, use this.
targets = await Get(
    Targets,
    UnparsedAddressInputs(["//:addr1", "project/addr2"], owning_address=Address("project", target_name="original"))
)
```
