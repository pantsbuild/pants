...
Often, you will want to read from a file in your project to set kwargs like `version` or `long_description`. Use `await get_digest_contents(**implicitly(PathGlobs(...)))` to do this (see [File system](../the-rules-api/file-system.mdx)):
...
- [Read from options](../the-rules-api/options-and-subsystems.mdx). Also see [here](https://github.com/pantsbuild/pants/blob/main/pants-plugins/internal_plugins/releases/register.py) for an example.
