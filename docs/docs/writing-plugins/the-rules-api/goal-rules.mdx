...
```python
@goal_rule
async def publish(distribution: PythonDistributionTarget, console: Console) -> Publish:
   ...
```
...
@goal_rule
async def publish(targets: Targets, console: Console) -> Publish:
   relevant_targets = [
