...
Pants will attempt to infer the appropriate runtime and its platform-specific details from [the relevant interpreter constraints](../overview/interpreter-compatibility.mdx). For example, given `interpreter_constaints = ["==3.12.*"]`, Pants will build the Cloud function package to appropriate for the  `python312` runtime. The GCF environment is a relatively stable, running Linux on a x86-64 CPU, and thus Pants includes default "complete platforms" specifications that allow choosing appropriate artifacts of third-party dependencies with native code. These are chosen based on the (inferred or explicit) runtime.

The inference may not work in your scenario and/or may not be appropriate:

1. if the interpreter constraints are not obviously referring to all versions from a given minor release series, Pants won't infer the runtime. For instance, `>=3.11,<3.13` is too wide (it covers both 3.11 and 3.12), while `==3.12.1` is too specific (GCF's `python312` runtime may not use that patch version).
...
In the first case, [the `runtime` field](../../../reference/targets/python_google_cloud_function.mdx#runtime) can be used to specify which runtime is intended for each `python_google_cloud_function` target. It should be one of the values from [https://cloud.google.com/functions/docs/concepts/python-runtime](https://cloud.google.com/functions/docs/concepts/python-runtime).
...
Otherwise, the platform-dependent information should be specified with [the `complete_platforms` field](../../../reference/targets/python_google_cloud_function.mdx#complete_platforms).
