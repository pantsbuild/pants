...
The [MyPy Protobuf plugin](https://github.com/nipunn1313/mypy-protobuf) generates [`.pyi` type stubs](https://mypy.readthedocs.io/en/stable/stubs.html). If you use MyPy through Pants's [check goal](../goals/check.mdx), this will ensure MyPy understands your generated code.
...
It is recommended to share the same dependencies versions between the protobuf plugin and your code runtime dependency (see [Python Lockfile](../overview/lockfiles.mdx#sharing-lockfiles-between-tools-and-code). The requirements for the `grpclib` plugin can be defined this way:
