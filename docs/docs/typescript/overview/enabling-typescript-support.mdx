...
TypeScript compilation includes package manager config files (.npmrc, .pnpmrc, pnpm-workspace.yaml) as inputs to ensure proper cache invalidation. These files affect how dependencies are resolved and how node_modules is structured. Corporate environments with private registries or custom proxy settings will work correctly. This behavior ensures TypeScript compilation uses the correct dependency environment configured by your package manager.

To include package manager configuration files, you must explicitly declare them as file targets:
...
This explicit declaration ensures build reproducibility and prevents accidental inclusion of user-specific configuration files from outside the repository.
