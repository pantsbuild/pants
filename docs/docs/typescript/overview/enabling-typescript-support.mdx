...
file(source="tsconfig.json")

# Optional: Include package manager config files if present
file(source=".npmrc")
file(source="pnpm-workspace.yaml")
```
...
:::tip Project references enable better performance
Project references with `"composite": true` enable incremental compilation and better dependency tracking across packages.
:::
...
**Recommended**: Include TypeScript in your root `package.json`:
...
If TypeScript is not available in your project dependencies, Pants will automatically download and use a default version which can be overridden:
