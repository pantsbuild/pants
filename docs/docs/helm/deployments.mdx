...
Pants has experimental support for managing deployments via the `experimental-deploy` goal. Helm deployments provide a basic implementation of this goal.
...
Pants solves this problem by providing the ability to manage the configuration files and the different parameters of a deployment as single unit such that a simple command line as `pants experimental-deploy ::` will always have the same effect on each of the deployments previously defined.
...
- One of those value files (`common-values.yaml`) provides default values that are common to all deployments.
...
Be careful when choosing the values that are going to be calculated dynamically.
:::
...
As an example, let's show how we can use the tool [`vals`](https://github.com/helmfile/vals) as a post-renderer and replace all references to secret values stored in HashiCorp Vault by their actual values. The following example is composed of a Helm chart that creates a secret resource in Kubernetes and a Helm deployment that is configured to use `vals` as a post-renderer:
