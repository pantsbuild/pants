...
Note: Some third-party extensions of your IDE (like Pylint with VSCode) may not use the updated PYTHONPATH from the .env file. [Mutable virtualenvs with editable installs](./setting-up-an-ide.mdx#first-party-sources) work better in such cases.

## First-party sources

If you build your code into distributions, you can export PEP-660 editable installs of all python_distribution targets that own the first-party code within that resolve.
