...
A [`resource`](../../reference/targets/resource.mdx) target is for files that are members of code packages, and are loaded via language-specific mechanisms, such as Python's `importlib.resources.files()` or Java's `getResource()`.
...
if __name__ == "__main__":
    config = importlib.resources.files("project").joinpath("config.json").read_text()
