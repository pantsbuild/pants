...
### File or Directory Artifact Extraction

Much like, `adhoc_tool` or `shell_command`, the `docker_image` target is able to produce outputs that can be supplied to other targets, see the [docker documentation](https://docs.docker.com/build/building/export/#export-binaries-from-a-build) for docker-specific information.

These can be in the form of files or directories that are copied from a build stage in your Dockerfile via `COPY --from=<target_stage> <target_path> <output_path>`. Use the `output_files` field to capture individual files, or `output_directories` to capture entire directories as output.

Files are captured relative to the build root. The example below will output a binary file `src/docker/hello`, and you should see it listed when running `pants run src/docker:list-files`:
...
docker_image(
    name="hello",
    output_files=["hello"],
    target_stage="artifact"  # If your artifact stage is the last stage, this becomes optional.
)

run_shell_command(
    name="list-files",
    command="ls -latr {chroot}/src/docker",
    execution_dependencies=[":hello"],
)
```

```dockerfile tab={"label":"src/docker/Dockerfile"}
