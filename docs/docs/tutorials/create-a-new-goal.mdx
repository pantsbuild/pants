...
In this tutorial, you'll learn the basics needed to get started writing a plugin. You will create a new goal, `project-version`, which will tell you the version (retrieved from the `VERSION` text file) of a particular project in your repo. You will learn how to create a new custom target to refer to the `VERSION` file, how to author a new goal, and, most importantly, how to connect rules and targets.
...
Now, we update our `@goal_rule` to call the `get_project_version_file_view` rule:
...
async def goal_show_project_version(console: Console, targets: Targets) -> ProjectVersionGoal:
    targets = [tgt for tgt in targets if tgt.alias == ProjectVersionTarget.alias]
    for target in targets:
        project_version = await get_project_version_file_view(target)
        console.print_stdout(project_version)
    return ProjectVersionGoal(exit_code=0)
```

:::note Understanding the request and rule signatures
In the basic usage demonstrated above, we directly pass a value of the required input type to the rule. This doesn't have to be the case! When you make a request, Pants looks at all the [rules in the graph](../writing-plugins/the-rules-api/concepts.mdx#the-rule-graph) to find a way to "fill in the blanks" and go from the input to the output using all the available rules. To do so you pass the argument you have via the `**implicitly(...)` kwarg, signaling to Pants that it should use this implicit argument to compute the necessary type by traversing the rule graph.
:::

Let's consider the following scenario where you have a few `@rule`s and a rule invocation:
...
@goal_rule
async def main(...):
    result = await rule2(**implicitly(obj))
```

With the following suite of rules, Pants will "figure out" that in order to return `C`, it's necessary to call `rule1` first to get `B` and then once there's `B`, call `rule2` to get C. This means you can focus on writing individual rules and let Pants figure out how to stitch them together to satisfy a call.
...
You would normally expect a project to have only a single `version_file` target declared, so as an improvement, we could raise an exception if there are multiple targets of this type found within a single project. This is something we'll do in the following tutorial.

### Reading the `VERSION` file

Let's read the `VERSION` file and print the version number in the terminal. The `source` field of our target needs to be ["hydrated"](../writing-plugins/the-rules-api/rules-and-the-target-api.mdx#sourcesfield). [Reading a file](../writing-plugins/the-rules-api/file-system.mdx) is pretty straightforward as well. Knowing which Rules you need to invoke may be non-obvious, so make sure to review the documentation, and don't be shy about [asking for help](/community/getting-help) if you are stuck!
...
async def get_project_version_file_view(
    ...
    sources = await hydrate_sources(HydrateSourcesRequest(target[SourcesField]))
...
If the `@goal_rule` can receive multiple `version_file` targets (which may happen if the user runs the goal over multiple projects in a single invocation of Pants), the current code will call `get_project_version_file_view` iteratively.  For efficiency, it is generally encouraged to replace the iterative `await` calls in the `for` loop with a [`concurrency()` call](../writing-plugins/the-rules-api/tips-and-debugging.mdx#tip-use-concurrently-for-increased-concurrency):
...
async def goal_show_project_version(console: Console, targets: Targets) -> ProjectVersionGoal:
    ...
    results = await concurrently(
        get_project_version_file_view(target) for target in targets
    )
...
from pants.engine.rules import collect_rules, concurrently, goal_rule, rule
from pants.engine.target import HydrateSourcesRequest, SourcesField, Targets
from project_version.target_types import ProjectVersionTarget
