...
In this tutorial, we continue from where we've left in the [previous tutorial](./create-a-new-goal.mdx). Now that we have a complete goal with a custom target, we are ready to make certain improvements and learn more advanced concepts that you would likely find useful when working on your own plugins.
...
from pants.engine.rules import concurrently, goal_rule
...
Pants provides a way to automate generation of standard targets using the [`tailor`](../../reference/goals/tailor.mdx) goal. If a repo has many projects, each containing a `VERSION` file, it might be useful to generate `version_file` targets in every directory where the relevant files are found. This is what Pants does, for instance, when the Docker backend is enabled, and you have `Dockerfile` files in the codebase. To make this work for our use case, however, we need to introduce the `tailor` goal to the `VERSION` files.

There is currently no website documentation on how to extend the `tailor` goal. In a situation like this, it may be worth exploring the Pants codebase to see how this was done in other plugins that are part of Pants. Once you find a piece of code that looks like it does what you want, you can copy it and tweak it to better suit your needs. For our use case, the code used in [generation of C++ source targets](https://github.com/pantsbuild/pants/blob/672ca1d662c76f2567e432347deee8949c14d35d/src/python/pants/backend/cc/goals/tailor.py) may be handy. After making a few changes, we have a new rule we can place in a new file:
...
async def find_putative_targets(
    ...
    all_project_version_files = await path_globs_to_paths(req.path_globs("VERSION"))
    unowned_project_version_files = set(all_project_version_files.files) - set(
        all_owned_sources
    )

    putative_targets = []
    for dirname, filenames in group_by_dir(unowned_project_version_files).items():
...
When the `tailor` goal is run, the build graph is analyzed to see when `PutativeTargetsRequest` is needed, i.e. to find out if there are any files (yet unknown to Pants) that look like they could potentially be wrapped in targets. For instance, if there is a `requirements.txt` file, a `python_requirement` target is created and when there is a Python `test_*` module, a `python_test` target is created. To be able to customize the `tailor` goal (to allow generation of custom targets), we need to "extend" the build graph. That is, we ask Pants to also run our rule when searching for files that maybe should have a target created.
...
async def get_git_repo_version(buildroot: BuildRoot) -> GitTagVersion:
    ...
    git_describe = await execute_process_or_raise(
        **implicitly(Process(
...
async def goal_show_project_version(...) -> ProjectVersionGoal:
    ...
    git_repo_version = await get_git_repo_version(**implicitly())
    ...
...
This happens because Git state is external to Pants, and so modifying our repository tags doesn't invalidate the Pants cache. Therefore it is not safe to [cache the `Process` runs](../writing-plugins/the-rules-api/processes.mdx) in this case. We can change the cacheability using the `cache_scope` parameter so that our Git call is forced to run on every Pants invocation.
...
Let's add another option so that we can control whether the Git tag should be retrieved:
...
class ProjectVersionSubsystem(GoalSubsystem):
    ...
    match_git = BoolOption(
        default=False,
        help="Check that a current Git tag of the repository matches the project version.",
    )
```
...
If you know that your Git tag may be different from the project version stored in the `VERSION` file and that you would always want the output to be in JSON format, you can set these options in the `pants.toml` file for visibility (and to avoid setting them via command line flags):
...
async def goal_show_project_version(
    ...
    targets = [tgt for tgt in targets if tgt.has_field(ProjectVersionSourceField)]
    results = await concurrently(
        get_project_version_file_view(ProjectVersionTarget, target)
        for target in targets
    )
    if project_version_subsystem.match_git:
        git_repo_version = await get_git_repo_version()
...
async def find_putative_targets(
    ...
    all_project_version_files = await path_globs_to_paths(
        Paths, PathGlobs, req.path_globs("VERSION")
    )
    unowned_project_version_files = (
        set(all_project_version_files.files) - set(all_owned_sources)
    )
...
In the [next tutorial](./testing-plugins.mdx) we'll see how to write tests for our plugin!
