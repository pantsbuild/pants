...
build_output = [
    ...
    'cargo:rustc-link-search=native=pants/.pants.d/engine/out', 'cargo:rustc-cfg=rust_1_26',
    'cargo:rustc-cfg=memchr_runtime_sse42', 'cargo:rustc-env=PROTOC=/protobuf/protoc',
    'cargo:rustc-env=PROTOC_INCLUDE=/protobuf/include', 'cargo:rustc-flags=-l static=samplerate',
    ...
    'cargo:warning=                                ^', 'cargo:warning=1 warning generated.',
    'cargo:rerun-if-env-changed=PY', 'cargo:rerun-if-changed=cbindgen.toml'
]
  ...
  def test_env_rules_run_custom_invocation(self):
    ...
    result = {
        'rustc-flags': [
            ['-l', 'static=samplerate'],
            ['-l', 'dylib=stdc++'],
            ['-l', 'static=pfring', '-L', '/usr/local/lib'],
            ['-l', 'static=pcap', '-L', '/usr/local/lib'],
        ],
        'rustc-env': [['PROTOC', '/protobuf/protoc'], ['PROTOC_INCLUDE', '/protobuf/include']],
        'rustc-link-search': [['-L', 'native=pants/.pants.d/engine/out'],],
        'rustc-link-lib': [['-l', 'static=native_engine_ffi'],],
        'rustc-cfg': [['--cfg', 'rust_1_26'], ['--cfg', 'memchr_runtime_sse42']],
        'warning': [
        ...
        'rerun-if-env-changed': ['PY'],
        'rerun-if-changed': ['cbindgen.toml']
    }
