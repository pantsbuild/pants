...
class MypyTask(LintTaskMixin, ResolveRequirementsTaskBase):
  ...
  def execute(self):
    ...
    if not mypy_interpreter:
      raise TaskError(f'Unable to find a Python {self._MYPY_COMPATIBLE_INTERPRETER_CONSTRAINT} '
    ...
    if self.get_options().include_requirements:
      if interpreter_for_targets.identity.matches(self._MYPY_COMPATIBLE_INTERPRETER_CONSTRAINT):
