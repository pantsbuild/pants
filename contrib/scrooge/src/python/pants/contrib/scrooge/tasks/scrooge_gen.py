...
class ScroogeGen(SimpleCodegenTask, NailgunTask):
    ...
    def _resolve_deps(self, depmap):
        """Given a map of gen-key=>target specs, resolves the target specs into
        references."""
    ...
    def _validate_language(self, target):
        ...
        if language not in self._registered_language_aliases():
            raise TargetDefinitionException(
                target,
                "language {} not supported: expected one of {}.".format(
                    language, list(self._registered_language_aliases().keys())
                ),
            )
        return language
    ...
    def _target_type_for_language(self, language):
        ...
        if not target_types:
            raise TaskError(
                "Registered target type `{0}` for language `{1}` does not exist!".format(
                    alias_for_lang, language
                )
            )
        if len(target_types) > 1:
            raise TaskError(
                "More than one target type registered for language `{0}`".format(language)
            )
    ...
    def gen(self, partial_cmd, target, target_workdir):
        ...
        if 0 != returncode:
            raise TaskError(
                "Scrooge compiler exited non-zero for {} ({})".format(target, returncode)
            )
