...
class FindBugs(NailgunTask):
    ...
    def register_options(cls, register):
        ...
        register("--fail-on-error", type=bool, fingerprint=True, help="Fail the build on an error.")
        register(
            "--max-rank", type=int, fingerprint=True, help="Maximum bug ranking to record [1..20]."
        )
        register("--relaxed", type=bool, fingerprint=True, help="Relaxed reporting mode")
        register(
            "--nested",
            type=bool,
            default=True,
    ...
    def _is_findbugs_target(self, target):
        if not isinstance(target, (JavaLibrary, JUnitTests)):
            self.context.log.debug(
                "Skipping [{}] because it is not a java library or java test".format(
                    target.address.spec
                )
            )
            return False
        ...
        with self.invalidated(targets, invalidate_dependents=True) as invalidation_check:
            ...
            for vt in invalidation_check.invalid_vts:
                ...
                if not self.get_options().fail_on_error or sum(target_bug_counts.values()) == 0:
                    vt.update()
                bug_counts = {
                    k: bug_counts.get(k, 0) + target_bug_counts.get(k, 0) for k in bug_counts.keys()
                }
            ...
            if error_count + bug_counts["total"] > 0:
                ...
                if bug_counts["total"] > 0:
                    self.context.log.warn(
                        "Bugs: {total} (High: {high}, Normal: {normal}, Low: {low})".format(
                            **bug_counts
                        )
                    )
                if self.get_options().fail_on_error:
                    raise TaskError(
                        "failed with {bug} bugs and {err} errors".format(
    ...
    def findbugs(self, target):
        ...
        target_jars = OrderedSet(
            jar for conf, jar in runtime_classpaths.get_for_target(target) if conf == "default"
        )
        ...
        if self.get_options().exclude_filter_file:
            args.extend(
                ["-exclude", os.path.join(get_buildroot(), self.get_options().exclude_filter_file)]
            )

        if self.get_options().include_filter_file:
            args.extend(
                ["-include", os.path.join(get_buildroot(), self.get_options().include_filter_file)]
            )
