...
class NodeResolveTest(TaskTestBase):
  ...
  def test_resolve_simple(self):
    ...
    out = task.node_distribution._node_instance.run_command(
      args=[script_path]).check_output()
  ...
  def _test_resolve_optional_install_helper(self, install_optional, package_manager):
    self.create_file('src/node/util/package.json', contents=dedent("""
      {
        "name": "util",
        "version": "0.0.1"
    ...
    object_to_mock = task.node_distribution.get_package_manager(package_manager)
    with mock.patch.object(object_to_mock, 'install_packages') as exec_call:
      exec_call.return_value.run.return_value.wait.return_value = 0
      task.execute()
      exec_call.assert_called_once_with(
        install_optional=install_optional, node_paths=None)
