...
class NodeTestIntegrationTest(PantsRunIntegrationTest):
    ...
    def test_test_simple(self):
        command = ["test", "contrib/node/examples/src/node/server-project:unit"]
    ...
    def test_test_target_with_non_default_script_name(self):
        command = ["test", "contrib/node/examples/src/node/web-component-button:unit"]
        pants_run = self.run_pants(command=command)
    ...
    def test_test_preinstalled_node_module_project(self):
        ...
        with self.temporary_workdir() as workdir:
            for _ in range(2):
                command = ["test", "contrib/node/examples/src/node/preinstalled-project:unit"]
                pants_run = self.run_pants_with_workdir(command=command, workdir=workdir)
                self.assert_success(pants_run)

    def test_test_passthru_args(self):
        command = ["-q", "test", "contrib/node/examples/src/node/server-project:checkarg", "--"]
