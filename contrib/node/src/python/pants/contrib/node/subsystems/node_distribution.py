...
class NodeDistribution(object):
  ...
  class Factory(Subsystem):
    ...
    def register_options(cls, register):
      ...
      register('--version', advanced=True, default='8.6.0',
      ...
      register('--yarnpkg-version', advanced=True, default='v1.2.0', fingerprint=True,
  ...
  class Command(namedtuple('Command', ['executable', 'args', 'extra_paths'])):
    ...
    def run(self, **kwargs):
      ...
      logging.info('\nrunning node command [%s]', self.cmd)
      logging.info('environment')
      logging.info('*' * 80)
      logging.info('PATH => %s', env.get('PATH'))
      logging.info('*' * 80)
