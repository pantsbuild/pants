...
class CppCompile(CppTask):
    ...
    def register_options(cls, register):
        ...
        register(
            ...
            default=[".cc", ".cxx", ".cpp"],
            help=(
                "The list of extensions to consider when determining if a file is a "
                "C++ source file."
            ),
        )
                    ...
                    if is_cc(source):
                        if not vt.valid:
                            with self.context.new_workunit(
                                name="cpp-compile", labels=[WorkUnitLabel.MULTITOOL]
                            ):
    ...
    def _compile(self, target, results_dir, source):
        ...
        with self.context.new_workunit(
            name="cpp-compile", labels=[WorkUnitLabel.COMPILER]
        ) as workunit:
