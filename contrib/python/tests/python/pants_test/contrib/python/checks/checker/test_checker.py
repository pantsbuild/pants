...
class CheckstyleTest(unittest.TestCase):
  ...
  def test_failure_print_nit(self):
    ...
    msg = ("""T000:ERROR   a/python/fail.py:002 Classes must be UpperCamelCased\n"""
           """     |class lower_case(object):""")

    self.assert_checker(relpath='a/python/fail.py',
                        contents=contents,
                        expected_code=1,
  ...
  def test_syntax_error_nit(self):
    ...
    msg = ("""E901:ERROR   a/python/error.py:002 SyntaxError: invalid syntax\n"""
           """     |\n"""
           """     |invalid python\n"""
           """     |""")

    self.assert_checker(relpath='a/python/error.py',
                        contents=contents,
                        expected_code=1,
                        expected_message=msg)
  ...
  def test_noqa_line(self):
    def content(noqa):
      ...
      """.format(too_long='a' * self._MAX_LENGTH, noqa='  # noqa' if noqa else ''))

    self.assert_checker(relpath='a/python/pass.py', contents=content(noqa=True))

    msg = ("""E501:ERROR   PythonFile(a/python/fail.py):003 line too long (63 > 50 characters)\n"""
  ...
  def test_noqa_file(self):
    def content(noqa):
      return dedent("""
        ...
        SHORT_ENOUGH = True
        TOO_LONG = '{too_long}'
      """.format(noqa='# checkstyle: noqa' if noqa else '', too_long='a' * self._MAX_LENGTH))

    self.assert_checker(relpath='a/python/pass.py', contents=content(noqa=True))

    msg = ("""E501:ERROR   PythonFile(a/python/fail.py):004 line too long (63 > 50 characters)\n"""
           """     |TOO_LONG = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'""")
