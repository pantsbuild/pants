...
def main(args=None):
  parser = argparse.ArgumentParser(description='A Python source code linter.',
                                   fromfile_prefix_chars='@')
  parser.add_argument('--root-dir',
                      required=True,
                      metavar='PATH',
                      help='The absolute path to the root directory all sources passed are '
                           'relative to')
  parser.add_argument('sources', nargs='+')
  parser.add_argument('--severity',
                      type=str,
                      default=Nit.SEVERITY[Nit.COMMENT],
                      choices=[name for _, name in sorted(Nit.SEVERITY.items(),
                                                          key=lambda item: item[0])],
                      help='Only messages at this severity or higher are logged.')
  parser.add_argument('--strict',
                      action='store_true',
                      default=False,
                      help='If enabled, exit status will be non-zero for any nit at WARNING or '
                           'higher.')
  parser.add_argument('--suppress',
                      metavar='PATH',
                      type=str,
                      default=None,
                      help='Takes a text file where specific rules on specific files will be '
                           'skipped.')
  ...
  checker = Checker(root_dir=args.root_dir,
