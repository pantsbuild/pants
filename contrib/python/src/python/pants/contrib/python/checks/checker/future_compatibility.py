...
class FutureCompatibility(CheckstylePlugin):
    """Warns about behavior that will likely break when moving to Python
    3.x."""

    @classmethod
    def name(cls):
        return "future-compat"
    ...
    def nits(self):
        for call in self.iter_ast_types(ast.Call):
            if isinstance(call.func, ast.Attribute):
                ...
                if call.func.attr in self.BAD_ITERS and getattr(call.func.value, "id", "") != "six":
                    yield self.error(
                        "T602",
                        "{attr} disappears in Python 3.x.  Use non-iter instead.".format(
                            attr=call.func.attr
                        ),
                        call,
                    )
                ...
                if call.func.id in self.BAD_FUNCTIONS:
                    yield self.error(
                        "T603",
                        "Please avoid {func_id} as it disappears in Python 3.x.".format(
        ...
        for name in self.iter_ast_types(ast.Name):
            if name.id in self.BAD_NAMES:
                yield self.error(
                    "T604",
                    "Please avoid {id} as it disappears in Python 3.x.".format(id=name.id),
                    name,
                )
