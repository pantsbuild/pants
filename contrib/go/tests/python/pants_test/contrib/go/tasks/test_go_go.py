...
class GoEnvTest(TaskTestBase):
    ...
    def test_execute(self):
        ...
        task = self.create_task(
            self.context(target_roots=[bob_binary, jane_binary], passthru_args=["echo", "$GOPATH"])
        )
        with temporary_file() as stdout:
            ...
            with open(stdout.name, "r") as output:
                self.assertEqual(
                    output.read().strip(),
                    os.pathsep.join([task.get_gopath(bob_binary), task.get_gopath(jane_binary)]),
                )
...
class GoGoTest(TaskTestBase):
    ...
    def test_execute(self):
        ...
        task = self.create_task(
            self.context(target_roots=[bob_binary, jane_binary], passthru_args=["env", "GOPATH"])
        )
        with temporary_file() as stdout:
            task.execute(stdout=stdout)
            stdout.close()
