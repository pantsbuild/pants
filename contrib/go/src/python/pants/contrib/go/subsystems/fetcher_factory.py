...
class FetcherFactory(Subsystem):
    ...
    def subsystem_dependencies(cls):
        return super().subsystem_dependencies() + (ArchiveRetriever, GoImportMetaTagReader)

    _DEFAULT_MATCHERS = {
        # TODO: Add launchpad.net?
        r"bitbucket\.org/(?P<user>[^/]+)/(?P<repo>[^/]+)": ArchiveFetcher.UrlInfo(
            url_format=r"https://bitbucket.org/\g<user>/\g<repo>/get/{rev}.tar.gz",
            default_rev="tip",
            strip_level=1,
        ),
        r"github\.com/(?P<user>[^/]+)/(?P<repo>[^/]+)": ArchiveFetcher.UrlInfo(
