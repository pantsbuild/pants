...
class GoThriftGen(SimpleCodegenTask):
    ...
    def register_options(cls, register):
        ...
        register(
            "--thrift-import-target",
            type=target_option,
            advanced=True,
            help="Use this thrift import on symbolic defs.",
        )
        register(
            "--multiple-files-per-target-override",
            advanced=True,
            fingerprint=True,
    ...
    def _deps(self):
        ...
        if thrift_import_target is None:
            raise TaskError(
                "Option thrift_import_target in scope {} must be set.".format(self.options_scope)
            )
    ...
    def _thrift_cmd(self):
        ...
        if thrift_import is None:
            raise TaskError(
                "Option thrift_import in scope {} must be set.".format(self.options_scope)
            )
