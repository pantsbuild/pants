...
use process_execution::{
    local::KeepSandboxes, CacheContentBehavior, Context, ImmutableInputs, InputDigests,
    NamedCaches, Platform, ProcessCacheScope, ProcessMetadata,
};
use prost::Message;
    ...
    let process = process_execution::Process {
        ...
        timeout: action.timeout.map(|timeout| {
            std::time::Duration::from_nanos(
                timeout.nanos as u64 + timeout.seconds as u64 * 1000000000,
            )
        }),
        execution_slot_variable: None,
