...
impl Core {
    ...
    fn make_leaf_runner(
        ...
        let (runner, parallelism): (Box<dyn CommandRunner>, usize) =
            if remoting_opts.execution_enable {
                ...
                let runner: Box<dyn CommandRunner> = if exec_strategy_opts.local_enable_nailgun {
                    ...
                    let pool_size: usize = if exec_strategy_opts.child_max_memory > 0 {
                        max(
                            1,
                            exec_strategy_opts.child_max_memory
    ...
    fn make_command_runners(
        ...
        let remote_cache_read =
            exec_strategy_opts.remote_cache_read || remoting_opts.execution_enable;
        ...
        let make_cached_runner =
            |should_cache_read: bool| -> Result<Arc<dyn CommandRunner>, String> {
    ...
    pub fn new(
        ...
        let root_ca_certs =
            if let Some(ref path) = remoting_opts.root_ca_certs_path {
