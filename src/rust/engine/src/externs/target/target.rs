...
use crate::python::{PinnedTypeId, PyComparedBool};

use super::util::{
    NoFieldValue, PyRepr, PyReprList, combine_hashes, import_attr, import_target_attr, import_type,
    raise_invalid_field,
};
    ...
    fn eq(&self, other: &Self) -> bool {
        self.cls == other.cls
            && match (&self.union_membership, &other.union_membership) {
                (Some(a), Some(b)) => a.is(b) || a.get() == b.get(),
                (None, None) => true,
                _ => false,
            }
    }
...
impl Target {
        ...
        cached(&PLUGIN_FIELDS_CACHE, key, py, || {
            ...
            while let Some(current) = classes.pop() {
                ...
                if current.is_subclass(&target_type)? {
                    let plugin_field_type: Bound<'_, PyType> =
                        current.getattr(intern!(py, "PluginField"))?.extract()?;
                    let members =
    ...
    fn plugin_field(py: Python) -> PyResult<PluginFieldDescriptor> {
        ...
        dict.set_item("__module__", "pants.engine.target")?;
...
impl Target {
    ...
    fn validate_origin_sources_blocks(blocks: &Bound<'_, FrozenDict>, py: Python) -> PyResult<()> {
            ...
            .try_iter()?
        {
            ...
            if !value.is_instance(&source_blocks_type)? {
                return Err(PyValueError::new_err(format!(
                    "Expected `origin_sources_blocks` values to be `SourceBlocks`, got {} {}",
                    value.get_type(),
                    value.repr()?
                )));
            }
            for block in value.try_iter()? {
                ...
                if !block.is_instance(source_block_type.as_any())? {
                    return Err(PyValueError::new_err(format!(
                        "Expected `origin_sources_blocks` elements to be `SourceBlock`, got {} {}",
                        block.get_type(),
                        block.repr()?
                    )));
                }
    ...
    fn check_removal_version(
        ...
        warn_or_error(py)?.call(
            (
                &removal_version,
                format!("the {} target type", PyRepr(&alias)),
                format!(
