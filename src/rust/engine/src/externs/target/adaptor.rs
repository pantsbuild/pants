...
fn deep_freeze_kwargs_with_origin<'py>(
    kwargs: Option<&Bound<'py, PyDict>>,
    description_of_origin: &str,
    py: Python<'py>,
) -> PyResult<Py<FrozenDict>> {
    deep_freeze_kwargs(kwargs, py).map_err(|error| {
        if error.is_instance_of::<PyTypeError>(py) {
            PyTypeError::new_err(format!("In {}: {}", description_of_origin, error.value(py)))
        } else {
            error
        }
    })
...
#[pyclass(frozen, extends = FrozenOrderedSet, module = "pants.engine.internals.native_engine")]
...
impl SourceBlocks {
    ...
    fn __new__(iterable: Option<&Bound<'_, PyAny>>) -> PyResult<PyClassInitializer<Self>> {
        Ok(
            PyClassInitializer::from(FrozenOrderedSet::from_iterable(iterable)?)
                .add_subclass(SourceBlocks),
        )
    }
}
...
impl TargetAdaptor {
    ...
    fn __new__(
        ...
        let frozen_kwargs = deep_freeze_kwargs_with_origin(kwargs, &__description_of_origin__, py)?;
    ...
    fn with_new_kwargs(&self, kwargs: Option<&Bound<'_, PyDict>>, py: Python) -> PyResult<Self> {
        let frozen_kwargs =
            deep_freeze_kwargs_with_origin(kwargs, &self.description_of_origin, py)?;
