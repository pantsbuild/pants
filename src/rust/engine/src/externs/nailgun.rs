...
impl PyNailgunClient {
  ...
  fn execute(&self, command: String, args: Vec<String>, env: &PyDict, py: Python) -> PyResult<i32> {
    ...
    // NB: We assume that env var names and values are strict UTF-8 (with no lone surrogates
    //  representing invalid UTF-8 passed from the OS).  The Python-side caller must ensure this.
