...
use options::{Args, Env, Val, OptionId, OptionParser, OptionValue, Scope};
...
fn val_to_py_object(py: Python, val: Val) -> PyResult<PyObject> {
    let res = match val {
        ...
        Val::Dict(dict) => {
            ...
            for (k, v) in dict {
                pydict.set_item(k, val_to_py_object(py, v)?)?;
            }
            pydict.into_py(py)
        }
    };
...
impl PyOptionParser {
    fn get_scalar<T: ToOwned + ?Sized>(
        ...
        getter: fn(&OptionParser, &OptionId, &T) -> Result<OptionValue<T::Owned>, String>,) -> PyResult<(T::Owned, String)> {
...
impl PyOptionParser {
    ...
    fn __new__(args: Vec<String>, env: &PyDict, configs: Vec<&str>, allow_pantsrc: bool) -> PyResult<Self> {
        ...
        let option_parser = OptionParser::new(Args::new(args), Env::new(env), Some(configs), allow_pantsrc)
            .map_err(PyValueError::new_err)?;
        Ok(Self(option_parser))
    }

    fn get_bool(&self, option_id: &PyOptionId, default: bool) -> PyResult<(bool, String)> {
        self.get_scalar(option_id, &default, |op, oid, def| op.parse_bool(oid, *def))
    }

    fn get_int(&self, option_id: &PyOptionId, default: i64) -> PyResult<(i64, String)> {
        self.get_scalar(option_id, &default, |op, oid, def| op.parse_int(oid, *def))
    }

    fn get_float(&self, option_id: &PyOptionId, default: f64) -> PyResult<(f64, String)> {
    ...
    //     self.get_scalar(option_id, &default, |op, oid, def| op.parse_bool_list(oid, def))
    // }
