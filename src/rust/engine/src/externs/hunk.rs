...
fn text_block_hash(block: &Option<Py<TextBlock>>) -> isize {
    match block {
        Some(block) => block.get().hash_value(),
        None => 0,
    }
}
...
impl Hunk {
    ...
    fn __repr__(&self) -> String {
        let repr_block = |block: &Option<Py<TextBlock>>| match block {
            Some(block) => block.get().repr_string(),
            None => "None".to_string(),
        };
        format!(
            "Hunk(left={}, right={})",
            repr_block(&self.left),
            repr_block(&self.right)
        )
    }
}
