...
impl StoreFileByDigest<Failure> for Context {
    fn store_by_digest(
        ...
        let subject_path = SubjectPath::Workspace(RelativePath::new(&file.path).expect("TODO"));
        async move { context.get(DigestFile { file, subject_path }).await }.boxed()
    }
}
...
impl SubjectPath {
    pub fn to_path_in_workspace(&self, build_root: &Path) -> PathBuf {
        match self {
            SubjectPath::Workspace(path) => build_root.join(path),
            SubjectPath::LocalSystem(path) => path.clone(),
        }
    }
...
impl NodeKey {
    ...
    async fn maybe_watch(&self, context: &Context) -> NodeResult<()> {
        let (watcher, abs_path) = match self.fs_path_to_watch() {
