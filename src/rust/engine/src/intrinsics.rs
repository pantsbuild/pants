  ...
  async move {
    ...
    Ok(externs::unsafe_call(
      ...
      &[
        ...
        Snapshot::store_directory_digest(py, DirectoryDigest::new(result.output_directory))
          .map_err(throw)?,
        externs::unsafe_call(
          py,
...
fn directory_digest_to_digest_contents(
  ...
  async move {
    ...
    let snapshot = context
      .core
      .store()
      .contents_for_directory(digest.digest)
      .await
...
fn directory_digest_to_digest_entries(
  ...
  async move {
    ...
    let snapshot = context
      .core
      .store()
      .entries_for_directory(digest.digest)
      .await
...
fn remove_prefix_request_to_digest(
  ...
  async move {
    ...
    let digest = context
      ...
      .map_err(|e| throw(format!("{:?}", e)))?;
    let gil = Python::acquire_gil();
    Snapshot::store_directory_digest(gil.python(), DirectoryDigest::new(digest)).map_err(throw)
  }
  .boxed()
}
  ...
  async move {
    let (path_globs, original_digest) = Python::with_gil(|py| {
      ...
      let res: NodeResult<(PreparedPathGlobs, Digest)> = Ok((
        ...
        lift_directory_digest(py_digest).map_err(throw)?.digest,
      ));
      res
    })?;
...
fn interactive_process(
  ...
  async move {
    ...
    let (argv, run_in_workspace, restartable, input_digest, env, append_only_caches) = Python::with_gil(|py| {
      ...
      let input_digest: Digest = lift_directory_digest(py_input_digest)?.digest;
