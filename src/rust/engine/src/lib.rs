 ...
 */
struct RawExecution<'g,'t> {
  ready_ptr: *mut EntryId,
  ready_runnables_ptr: *mut Runnable,
  ready_len: u64,
  ready: Vec<EntryId>,
  ready_runnables: Vec<Runnable>,
  execution: Execution<'g, 't>,
}

impl<'g,'t> RawExecution<'g,'t> {
  fn new(execution: Execution<'g,'t>) -> RawExecution<'g,'t> {
    let mut raw =
      RawExecution {
...
#[no_mangle]
pub extern fn execution_create<'g,'t>(
  graph_ptr: *mut Graph,
  tasks_ptr: *mut Tasks
) -> *const RawExecution<'g,'t> {
  with_graph(graph_ptr, |graph| {
    with_tasks(tasks_ptr, |tasks| {
      // create on the heap, and return a raw pointer to the boxed value.
      Box::into_raw(Box::new(RawExecution::new(Execution::new(graph, tasks))))
    })
  })
}

#[no_mangle]
pub extern fn execution_next<'g,'t>(
  execution_ptr: *mut RawExecution<'g,'t>,
  ...
  with_execution(execution_ptr, |raw| {
    with_vec(completed_ptr, completed_len as usize, |completed_ids| {
      with_vec(completed_states_ptr, completed_len as usize, |completed_states| {
        ...
        let runnables = raw.execution.next(completed);
