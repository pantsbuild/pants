...
pub fn lift_file_digest(digest: &PyAny) -> Result<hashing::Digest, String> {
    let py_file_digest: externs::fs::PyFileDigest =
        digest.extract().map_err(|e| format!("{}", e))?;
    Ok(py_file_digest.0)
}
...
impl ExecuteProcess {
    async fn lift_process_input_digests(
        ...
        let input_digests_fut: Result<_, String> = Python::with_gil(|py| {
            ...
            let input_files =
                lift_directory_digest(externs::getattr(value, "input_digest").unwrap())
    ...
    fn lift_process_fields(
        ...
        let working_directory =
            match externs::getattr_as_optional_string(value, "working_directory") {
                None => None,
        ...
        let concurrency_available: usize =
            externs::getattr(value, "concurrency_available").unwrap();
...
impl Snapshot {
    ...
    pub fn lift_path_globs(item: &PyAny) -> Result<PathGlobs, String> {
        ...
        let failure_behavior: String =
            externs::getattr(glob_match_error_behavior, "value").unwrap();
    ...
    pub fn store_snapshot(py: Python, item: store::Snapshot) -> Result<Value, String> {
        let py_snapshot =
            Py::new(py, externs::fs::PySnapshot(item)).map_err(|e| format!("{}", e))?;
...
impl DownloadedFile {
    ...
    async fn run_node(self, context: Context) -> NodeResult<store::Snapshot> {
        ...
        self.load_or_download(context.core, url, expected_digest)
            .await
            .map_err(throw)
    }
}
    ...
    async fn gen_get(
        ...
        let get_futures = gets
            ...
            .map(|get| {
                ...
                async move {
                    let dependency_key = DependencyKey::new(get.output)
                        .provided_params(get.inputs.iter().map(|t| *t.type_id()));
        ...
        loop {
            ...
            match response {
                externs::GeneratorResponse::Get(get) => {
                    let values =
                        Self::gen_get(&context, workunit, &params, entry, vec![get]).await?;
