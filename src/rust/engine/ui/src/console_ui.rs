...
impl ConsoleUI {
  pub fn new(workunit_store: WorkunitStore, local_parallelism: usize, renderer: String) -> ConsoleUI {
    ConsoleUI {
      workunit_store,
      local_parallelism,
      renderer,
  ...
  pub fn initialize(&mut self, executor: Executor) -> Result<(), String> {
    ...
    self.instance = Some(Instance::new(&self.renderer, self.local_parallelism, executor)?);
...
impl Instance {
  ...
  ///
  pub fn new(renderer: &String, local_parallelism: usize, executor: Executor) -> Result<Instance, String> {
    ...
    return match renderer.as_str() {
      ...
      _ => {assert!(false); Err(String::from("Bad renderer option!"))},
    };
  }
      ...
      Instance::Prodash(prodash) => {
        ...
        for span_id in tasks_to_display.keys().cloned().collect::<Vec<_>>() {
          if heavy_hitters.contains_key(&span_id) {
            // NB `inc_by` moves the "worms" to help show continual progress
