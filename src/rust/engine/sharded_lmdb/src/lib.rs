...
impl ShardedLmdb {
    ...
    fn envs(
        ...
        for b in 0..shard_count {
            ...
            fs::safe_create_dir_all(&dir).map_err(|err| {
        ...
        self.executor
            .spawn_blocking(move || {
                ...
                for (maybe_fingerprint, bytes) in items {
                    let fingerprint =
                        maybe_fingerprint.unwrap_or_else(|| Digest::of_bytes(&bytes).hash);
                    let effective_key =
                        VersionedFingerprint::new(fingerprint, ShardedLmdb::SCHEMA_VERSION);
        ...
        self.executor
            .spawn_blocking(move || {
                ...
                loop {
                    ...
                    let effective_key =
                        VersionedFingerprint::new(digest.hash, ShardedLmdb::SCHEMA_VERSION);
