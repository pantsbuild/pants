...
impl Destination {
    ...
    > {
        ...
        match console {
            InnerDestination::Console(console) => {
                Ok(term::TermDestination::new(console, self.clone()))
            }
            _ => unreachable!(),
        }
    }
    ...
    pub fn write_stdout(&self, content: &[u8]) {
        ...
        let error_str = format!(
            "Failed to write stdout to {destination:?}, falling back to Logging: {error_res:?}"
        );
        std::mem::drop(destination);
        self.console_clear();
        log::warn!("{}", error_str);
    ...
    pub fn write_stderr(&self, content: &[u8]) {
        ...
        // Release the lock, clear the Console, log the error and retry.
        let error_str = format!(
            "Failed to write stderr to {destination:?}, falling back to Logging: {error_res:?}"
        );
        std::mem::drop(destination);
        self.console_clear();
        log::warn!("{}", error_str);
