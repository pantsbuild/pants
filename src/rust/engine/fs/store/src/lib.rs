...
use fs::{
    ...
    FileContent, FileEntry, PathStat, Permissions, RelativePath, EMPTY_DIRECTORY_DIGEST,
};
...
impl RemoteStore {
    ...
    >(
        ...
        self.maybe_download(digest, async move {
...
impl Store {
    ...
    pub async fn materialize_directory(
        ...
        self.materialize_directory_helper(destination, true, &parent_to_child, perms)
            .await
    }
        ...
        future::try_join_all(files.into_iter().map(|(path, digest, is_executable)| {
            ...
            async move {
                let content = store
                    ...
                    .await
                    .map_err(|e| {
                        e.enrich(&format!("Couldn't find file contents for {:?}", path))
                    })?;
                Ok(FileContent {
                    path,
                    content,
