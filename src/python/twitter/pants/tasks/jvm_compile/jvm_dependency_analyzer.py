...
class JvmDependencyAnalyzer(object):
  def __init__(self, context, check_missing_deps, check_missing_direct_deps, check_unnecessary_deps):
    ...
    #
    # Don't access directly. Call self._get_targets_by_file() instead.
    self._targets_by_file = None

    # Memoized map from target to all its transitive dep targets.
    # Don't access directly. Call self._get_transitive_deps_by_target() instead.
    self._transitive_deps_by_target = None

  def _get_targets_by_file(self):
  ...
  def _update_targets_by_file(self, targets):
    ...
    for tgt in targets:
      ...
      for basedir, classes in products.items():
        for cls in classes:
          #print('EEEEEE Updating for %s' % os.path.join(basedir, cls))
  ...
  def _get_transitive_deps_by_target(self):
  ...
  def _compute_missing_deps(self, srcs, actual_deps):
    ...
    targets_by_file = self._get_targets_by_file()
    transitive_deps_by_target = self._get_transitive_deps_by_target()
