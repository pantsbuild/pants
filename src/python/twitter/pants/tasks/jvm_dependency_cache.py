...
from twitter.pants.tasks.zinc_analysis_cache import ZincMergedAnalysisCache
...
class JvmDependencyCache(object):
    ...
    Parameters:
      ...
      targets: the set of targets to analyze. These should all be target types that
         inherit from jvm_target, and contain source files that will be compiled into
          jvm class files.
    """
    self.upstream_analysis_caches = upstream_analysis_caches
    self.merged_analysis_cache = None
  ...
  def _compute_jardep_contents(self):
    ...
    ivy_products = self.task.context.products.get("ivy_jar_products").get("ivy")
  ...
  def get_dependency_blame(self, from_target, to_target):
    ...
    for source in from_target.sources:
      ...
      for cl in self.merged_analysis_cache.external_deps[srcpath]:
