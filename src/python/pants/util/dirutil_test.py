...
def assert_dump_and_read(test_content, dump_kwargs, read_kwargs) -> None:
...
def create_and_check_link(source: str, link: str) -> None:
...
class File:
    ...
    @classmethod
    def empty(cls, path: str) -> File:
        return cls(path, contents="")

    @classmethod
    def read(cls, root: str, relpath: str) -> File:
...
def test_longest_dir_prefix() -> None:
...
def test_longest_dir_prefix_special() -> None:
...
def test_fast_relpath_invalid() -> None:
...
def test_mkdtemp_setup_teardown(tempfile_mkdtemp, dirutil_safe_rmtree, os_getpid, atexit_register):
    def faux_cleaner():
        pass
...
@contextmanager
def tree() -> Iterator[tuple[str, str]]:
    # root/
    #   a/
    #     b/
    #       1
...
def assert_tree(root: str, *expected: Dir | File | Symlink):
    def collect_tree() -> Iterator[Dir | File | Symlink]:
...
def test_relative_symlink_source_parent() -> None:
...
def test_relative_symlink_link_parent() -> None:
...
def test_relative_symlink_same_paths() -> None:
...
def test_relative_symlink_bad_source() -> None:
...
def test_relative_symlink_bad_link() -> None:
...
def test_relative_symlink_overwrite_existing_file() -> None:
...
def test_relative_symlink_exception_on_existing_dir() -> None:
    ...
    with temporary_dir() as tmpdir_1:
        ...
        with pytest.raises(ValueError, match=r"Path for link.*overwrite an existing directory*"):
            relative_symlink(source, link_path)


def test_rm_rf_file(file_name="./foo") -> None:
...
def test_rm_rf_dir(dir_name="./bar") -> None:
    with temporary_dir() as td, pushd(td):
        safe_mkdir(dir_name)
...
def test_rm_rf_nonexistent(file_name="./non_existent_file") -> None:
    with temporary_dir() as td, pushd(td):
        rm_rf(file_name)


def test_rm_rf_permission_error_raises(file_name="./perm_guarded_file") -> None:
    with (
        temporary_dir() as td,
        pushd(td),
...
def test_rm_rf_no_such_file_not_an_error(file_name="./vanishing_file") -> None:
    with (
        temporary_dir() as td,
        pushd(td),
        unittest.mock.patch("pants.util.dirutil.shutil.rmtree") as mock_rmtree,
...
def test_readwrite_file_binary() -> None:
    assert_dump_and_read(b"333", {"mode": "wb"}, {"binary_mode": True})
    with pytest.raises(Exception):
        # File is not opened as binary.
        assert_dump_and_read(b"333", {"mode": "w"}, {"binary_mode": True})


def test_readwrite_file_unicode() -> None:
    assert_dump_and_read("✓", {"mode": "w"}, {"binary_mode": False})
    with pytest.raises(Exception):
        # File is opened as binary.
        assert_dump_and_read("✓", {"mode": "wb"}, {"binary_mode": True})


def test_safe_concurrent_creation() -> None:
    with temporary_dir() as td:
...
def test_safe_concurrent_creation_noop() -> None:
    with temporary_dir() as td:
...
def test_safe_concurrent_creation_exception_handling() -> None:
    with temporary_dir() as td:
        expected_file = os.path.join(td, "expected_file")
...
def abs_symlink() -> Generator[tuple[str, str, str]]:
    ...
    link = os.path.join(td, "link")
    yield (td, source, link)
    safe_rmtree(td)


def test_link(abs_symlink) -> None:
    td, source, link = abs_symlink
    # Check if parent dirs will be created for the link
    link = os.path.join(td, "a", "b", "c", "link")
    create_and_check_link(source, link)


def test_overwrite_link_link(abs_symlink) -> None:
    _, source, link = abs_symlink
    # Do it twice, to make sure we can overwrite existing link
    create_and_check_link(source, link)
    create_and_check_link(source, link)


def test_overwrite_link_file(abs_symlink) -> None:
    _, source, link = abs_symlink
...
def test_overwrite_link_dir(abs_symlink) -> None:
    _, source, link = abs_symlink
    nested_dir = os.path.join(source, "a", "b", "c")
...
def test_overwrite_file(abs_symlink) -> None:
    _, source, link = abs_symlink
    touch(link)
    create_and_check_link(source, link)


def test_overwrite_dir(abs_symlink) -> None:
    _, source, link = abs_symlink
    os.makedirs(os.path.join(link, "a", "b", "c"))
    create_and_check_link(source, link)
