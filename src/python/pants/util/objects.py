    ...
    for maybe_decl in field_decls:
        ...
        if isinstance(maybe_decl, tuple):
            ...
            if isinstance(type_spec, type):
                ...
                raise TypeError(
                    "type spec for field '{}' was not a type or TypeConstraint: was {!r} (type {!r}).".format(
                        field_name, type_spec, type(type_spec).__name__
                    )
                )
    ...
    class DataType(namedtuple_cls, DatatypeMixin):
        ...
        def __new__(cls, *args, **kwargs):
            ...
            try:
                ...
                raise cls.make_type_error(f"error in namedtuple() base constructor: {e}")
            ...
            for field_name, field_constraint in fields_with_constraints.items():
                ...
                try:
                    ...
                    type_failure_msgs.append(f"field '{field_name}' was invalid: {e}")
            ...
            try:
                ...
                for field_name, value in self._asdict().items():
                    try:
                        ...
                        raise TypeError(
                            "For datatype object {} (type '{}'): in field '{}': {}".format(
      ...
      """
        ...
        def __repr__(self):
            ...
            return "{class_name}({args_joined})".format(
                class_name=type(self).__name__, args_joined=", ".join(args_formatted)
            )
        ...
        def __str__(self):
            ...
            return "{class_name}({typed_tagged_elements})".format(
                class_name=type(self).__name__, typed_tagged_elements=", ".join(elements_formatted)
            )
    ...
    class ChoiceDatatype(datatype([field_name]), ChoicesMixin):
      ...
      """
            ...
            if value not in cls._singletons:
                raise cls.make_type_error(
                    "Value {!r} must be one of: {!r}.".format(value, cls._allowed_values)
                )
        ...
        def __eq__(self, other):
            ...
            if type(self) != type(other):
                raise self.make_type_error(
                    ...
                    "enum equality is only defined for instances of the same enum class!".format(
      ...
      """
            ...
            if keys != frozenset(self._allowed_values):
                raise self.make_type_error(
                    "pattern matching must have exactly the keys {} (was: {})".format(
                        self._allowed_values, list(keys)
                    )
                )
...
class TypedCollection(TypeConstraint):
    ...
    """
        ...
        if not isinstance(constraint, TypeOnlyConstraint):
            raise TypeError(
                "constraint for collection must be a {}! was: {}".format(
                    TypeOnlyConstraint.__name__, constraint
                )
            )
    ...
    def _is_iterable(self, obj):
        return self.iterable_constraint.satisfied_by(
            obj
    ...
    def satisfied_by(self, obj):
        return self._is_iterable(obj) and all(self._constraint.satisfied_by(el) for el in obj)

    def make_collection_type_constraint_error(self, base_obj, el):
        base_error = self.make_type_constraint_error(el, self._constraint)
        return TypeConstraintError(
            "in wrapped constraint {} matching iterable object {}: {}".format(
                self, base_obj, base_error
            )
        )
    ...
    def validate_satisfied_by(self, obj):
        if not self._is_iterable(obj):
            ...
            raise TypeConstraintError(
                "in wrapped constraint {}: {}\nNote that objects matching {} are not considered iterable.".format(
                    self, base_iterable_error, self.exclude_iterable_constraint
                )
            )
        for el in obj:
    ...
    def __repr__(self):
        return "{type_constraint_type}({constraint!r})".format(
            type_constraint_type=type(self).__name__, constraint=self._constraint
        )
