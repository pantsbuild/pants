...
class BuildFile(ABC):
  ...
  def _build_files_from_paths(project_tree, rel_paths, build_ignore_patterns):
    ...
    return OrderedSet(
      sorted(
        (BuildFile._cached(project_tree, relpath) for relpath in build_files_without_ignores),
  ...
  def __init__(self, project_tree: ProjectTree, relpath: str) -> None:
    ...
    if not project_tree.exists(relpath):
      raise self.MissingBuildFileError("BUILD file does not exist at: {path}".format(path=path))
