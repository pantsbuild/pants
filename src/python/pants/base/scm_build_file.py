...
class ScmBuildFile(BuildFile):
  ...
  def set_rev(cls, rev):
    cls._rev = rev
    if cls._scm:
      cls._cls_project_tree = ScmProjectTree(cls._scm, cls._rev)
  ...
  def __init__(self, root_dir, relpath=None, must_exist=True):
    super(ScmBuildFile, self).__init__(ScmBuildFile._cls_project_tree, root_dir, relpath=relpath, must_exist=must_exist)

  @classmethod
  def from_cache(cls, root_dir, relpath, must_exist=True):
    return ScmBuildFile.cached(cls._cls_project_tree, root_dir, relpath, must_exist)
