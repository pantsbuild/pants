...
class DeprecationApplicationError(Exception):
    """The base exception type thrown for any form of @deprecation application
    error."""
...
class InvalidSemanticVersionOrderingError(DeprecationApplicationError):
    """Indicates that multiple semantic version strings were provided in an
...
class BadDecoratorNestingError(DeprecationApplicationError):
    """Indicates the @deprecated decorator was innermost in a sequence of
    layered decorators."""


def get_deprecated_tense(removal_version, future_tense="will be", past_tense="was"):
    """Provides the grammatical tense for a given deprecated version vs the
    current version."""
...
def _greater_warnings_context(context_lines_string):
    ...
    warnings.warn_explicit() doesn't use the `line` argument to
    showwarning(), but we want to make use of the warning filtering
    provided by warn_explicit(). This contextmanager overwrites the
    showwarning() method to pipe in the desired amount of context lines
    when using warn_explicit().
    """
...
def deprecated_conditional(
    predicate, removal_version, entity_description, hint_message=None, stacklevel=4
):
