...
class Test(datatype(["classname", "methodname"])):
    ...
    def enclosing(self):
        """Return a test representing all the tests in this test's enclosing
        class.
    ...
    def render_test_spec(self):
        """Renders this test in `[classname]#[methodname]` test specification
        format.
...
class RegistryOfTests:
    ...
    def match_test_spec(self, possible_test_specs):
        """This matches the user specified test spec with what tests Pants
        knows.
    ...
    def index(self, *indexers):
        """Indexes the tests in this registry by sets of common properties
        their owning targets share.
...
def parse_failed_targets(test_registry, junit_xml_path, error_handler):
    """Parses junit xml reports and maps targets to the set of individual tests
    that failed.
