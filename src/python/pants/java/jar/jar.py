...
from pants.targets.jvm_binary import Duplicate, JarRules, Skip
...
class Jar(object):
  ...
  @staticmethod
  def tool_targets(config):
    """"""
    return config.getlist('jar-tool', 'bootstrap-tools', default=[':jar-tool'])

  MANIFEST_PATH = 'META-INF/MANIFEST.MF'
  """The path of a jar's manifest."""
  ...
  class Entry(AbstractClass):
    ...
    def dest(self):
      """
      :returns: The destination path of the entry in the jar.
      """
      return self._dest
    ...
    def materialize(self, scratch_dir):
      ...
      :param string scratch_dir:  A temporary directory that may be used to do any work in required
        to materialize a source file.
  ...
  def __init__(self, jar_tool_classpath, path, overwrite=False, compressed=True, jar_rules=None,
               executor=None, jvm_options=None):
    """Opens a jar that can be written to.

    By default the jar will be updated if it already exists and added entries will be compressed.

    If the jar is newly created or otherwise overwritten and no manifest file is explicitly added,
    a default manifest will be created; otherwise the explicitly added manifest will be used.

    :param list jar_tool_classpath: the classpath for the the underlying jar tool
    ...
    :param executor: an executor to run the java jar tool with
    :param list jvm_options: jvm args to run the jar tool with
    """
    if not path or not isinstance(path, Compatibility.string):
      raise ValueError('The jar path must be a non-empty string, got %s of type %s.'
                       % (path, type(path)))

    if not isinstance(compressed, bool):
      raise ValueError('The compressed setting must be a bool, got %s of type %s.'
                       % (compressed, type(compressed)))

    if jar_rules and not isinstance(jar_rules, JarRules):
      raise ValueError('The jar_rules must be a `JarRules` instance, got %s of type %s.'
                       % (jar_rules, type(jar_rules)))

    if executor and not isinstance(executor, Executor):
      raise ValueError('The executor must be an `Executor` instance, got %s of type %s.'
                       % (executor, type(executor)))

    self._jar_tool_classpath = maybe_list(jar_tool_classpath)
    self._path = path
    self._overwrite = overwrite
    self._compressed = compressed
    self._rules = jar_rules or JarRules(default_dup_action=Duplicate.FAIL)
    self._executor = executor or SubprocessExecutor()
    self._jvm_options = ['-Xmx64M'] + maybe_list(jvm_options or [])
  ...
  def writestr(self, path, contents):
    ...
    :param string contents: the contents of the file to write to ``path``
    """
    if not path or not isinstance(path, Compatibility.string):
      raise ValueError('The path must be a non-empty string')
    if contents is None or not isinstance(contents, Compatibility.string):
      raise ValueError('The contents must be a string')

    self._add_entry(self.MemoryEntry(path, contents))

  def _add_entry(self, entry):
    if self.MANIFEST_PATH == entry.dest:
  ...
  def close(self):
    """Finalizes all scheduled writes to this jar.

    :returns: the path of the updated or newly written jar as a convenience
    :raises: Jar.Error if there was a problem finalizing the writes
    """
    args = [
        '-update=%s' % self._flag(not self._overwrite),
        '-compress=%s' % self._flag(self._compressed),
    ]
    ...
    args.append('-default_action=%s' % self._action_name(self._rules.default_dup_action))
    ...
    if skip_patterns:
      args.append('-skip=%s' % ','.join(map(lambda p: p.pattern, skip_patterns)))
    ...
    with temporary_dir() as stage_dir:
      ...
      try:
        result = self._executor.execute(self._jar_tool_classpath,
                                        'com.twitter.common.jar.tool.Main',
                                        args=args,
                                        jvm_options=self._jvm_options)
        if result != 0:
          raise self.Error('Failed to write jar to %s with exit code %d' % (self._path, result))
        return self._path
      except self._executor.Error as e:
        raise self.Error('Unexpected error writing jar to %s: %s' % (self._path, e))
