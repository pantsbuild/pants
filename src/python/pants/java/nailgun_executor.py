...
class NailgunExecutor(Executor, FingerprintedProcessManager):
  ...
  def __init__(
    ...
    FingerprintedProcessManager.__init__(
      self, name=identity, process_name=self._PROCESS_NAME, metadata_base_dir=metadata_base_dir
    )
        ...
        try:
          ...
          raise self.Error(
            "Problem launching via {ng_desc} command {main} {args}: {msg}".format(
              ng_desc=nailgun or "<no nailgun connection>", main=main, args=" ".join(args), msg=e
            )
          )
