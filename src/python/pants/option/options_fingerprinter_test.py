...
class OptionsFingerprinterTest(TestBase):
  ...
  def test_fingerprint_dict(self) -> None:
    ...
    d3 = {"a": 1, "b": 2}
    fp1, fp2, fp3 = (
      self.options_fingerprinter.fingerprint(DictValueComponent.create, d) for d in (d1, d2, d3)
    )
  ...
  def test_fingerprint_dict_with_files_order(self) -> None:
    f1, f2 = (
      self.create_file(f, contents=c)
      for (f, c) in (("foo/bar.config", "blah blah blah"), ("foo/bar.config", "meow meow meow"))
    )
