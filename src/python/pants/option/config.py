...
class Config(ABC):
    ...
    Supports recursive variable substitution using standard python
    format strings. E.g., %(var_name)s will be replaced with the value
    of var_name.
    """
    ...
    def _create_parser(cls, *, seed_values=None):
        """Creates a config parser that supports %([key-name])s value
        substitution.
    ...
    def get(self, section, option, type_=str, default=None):
        """Retrieves option from the specified section (or 'DEFAULT') and
        attempts to parse it as type.

        If the specified section does not exist or is missing a
        definition for the option, the value is looked up in the DEFAULT
        section.  If there is still no definition found, the default
    ...
    def configs(self):
        """Returns the underlying single-file configs represented by this
        object."""
    ...
    def get_value(self, section, option):
        """Returns the value of the option in this config as a string, or None
