...
class GlobalOptions(BootstrapOptions, Subsystem):
    ...
    def validate_instance(cls, opts):
        ...
        deprecated_conditional(
            ...
            hint=softwrap(
                """
                Disable `--remote-cache-eager-fetch`.

                Eagerly fetching from the remote cache only makes sense with a local executor.
                """
            ),
        )

        deprecated_conditional(
            lambda: opts.remote_execution
            and (not opts.remote_cache_read or not opts.remote_cache_write),
            removal_version="2.15.0.dev0",
            entity="Using `--remote-execution` without `--remote-cache-read` and `--remote-cache-write`.",
            hint=softwrap(
                """
                Use of `--remote-execution` implies use of a remote cache. Both
                `--remote-cache-read` and `--remote-cache-write` should be set as well.
                """
            ),
        )
