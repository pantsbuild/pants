...
class BootstrapOptions:
    ...
    process_execution_docker_strategy = EnumOption(
        ...
        help=softwrap(
            """
            The strategy used to launch Docker containers when the `docker_environment`
            target is in use.

            The `cached` strategy `exec`s processes in cached Docker containers, while the
            `uncached` strategy uses a fresh container per process.

            The default value of `auto` will choose the fastest known-safe strategy for the
            platform that Pants is running on, which currently means using the `uncached` strategy
            when Docker is implemented via virtualization (on macOS and Windows in particular). This
            is to avoid filesystem consistency issues which can be observed when files inside a bind mount
            change on the host machine. See https://github.com/docker/roadmap/issues/7 for more information
            on macOS filesystem virtualization status.
            """
        ),
    )
