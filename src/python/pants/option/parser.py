...
class Parser:
    ...
    class BooleanConversionError(ParseError):
        """Indicates a value other than 'True' or 'False' when attempting to
        parse a bool."""
    ...
    class MutuallyExclusiveOptionError(ParseError):
        """Raised when more than one option belonging to the same mutually
        exclusive group is specified."""
    ...
    def walk(self, callback):
        """Invoke callback on this parser and its descendants, in depth-first
        order."""
        callback(self)
    ...
    class ParseArgsRequest(
        ...
        def _create_flag_value_map(flags):
            """Returns a map of flag -> list of values, based on the given flag
            strings.

            None signals no value given (e.g., -x, --foo). The value is
            a list because the user may specify the same flag multiple
            times, and that's sometimes OK (e.g., when appending to
            list-valued options).
            """
        ...
        def __new__(
            cls, flags_in_scope, namespace, get_all_scoped_flag_names, levenshtein_max_distance
        ):
            """
      ...
      """
            ...
            return super(Parser.ParseArgsRequest, cls).__new__(
                cls, flag_value_map, namespace, get_all_scoped_flag_names, levenshtein_max_distance
            )
    ...
    def _raise_error_for_invalid_flag_names(
        ...
        """Identify similar option names to unconsumed flags and raise a
        ParseError with those names."""
        matching_flags = {}
    ...
    def option_registrations_iter(self):
        """Returns an iterator over the normalized registration arguments of
        each option in this parser.
    ...
    def _unnormalized_option_registrations_iter(self):
        """Returns an iterator over the raw registration arguments of each
        option in this parser.
    ...
    def _recursive_option_registration_args(self):
        ...
        Includes all the options we inherit recursively from our
        ancestors.
        """
        if self._parent_parser:
            for args, kwargs in self._parent_parser._recursive_option_registration_args():
    ...
    def _check_deprecated(self, dest, kwargs):
        """Checks option for deprecation and issues a warning/error if
        necessary."""
    ...
    def parse_dest(*args, **kwargs):
        ...
        If an explicit `dest` is specified, returns that and otherwise
        derives a default from the option flags where '--foo-bar' ->
        'foo_bar' and '-x' -> 'x'.
        """
    ...
    def _compute_value(self, dest, kwargs, flag_val_strs):
        ...
        The source of the default value is chosen according to the
        ranking in RankedValue.
        """
        ...
        def to_value_type(val_str):
            if val_str is None:
                ...
                try:
                    ...
                    raise ParseError(
                        "Error applying type '{}' to option value '{}', for option '--{}' in {}: {}".format(
                            type_arg.__name__, val_str, dest, self._scope_str(), e
                        )
                    )
