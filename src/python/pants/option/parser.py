...
class Parser:
  ...
  class ParseArgsRequest:
    ...
    def __init__(
      ...
      self.get_all_scoped_flag_names = (
        get_all_scoped_flag_names
      )  # type: ignore[assignment]  # cannot assign a method
  ...
  def _compute_value(self, dest, kwargs, flag_val_strs):
    ...
    def expand(val_or_str):
      if (
        kwargs.get("fromfile", True) and isinstance(val_or_str, str) and val_or_str.startswith("@")
      ):
    ...
    if is_list_option(kwargs):
      ...
      raise ParseError(
        "Multiple cmd line flags specified for option {} in {}".format(dest, self._scope_str())
      )
    elif len(flag_vals) == 1:
