...
class OptionsTest(TestBase):
  ...
  def test_arg_scoping(self) -> None:
    ...
    global_options = self._parse(env={"PANTS_CONFIG_OVERRIDE": "['123','456']"}).for_global_scope()
    ...
    global_options = self._parse(flags='--target-listy=\'["//:foo", "//:bar"]\'').for_global_scope()
  ...
  def test_list_option(self) -> None:
    ...
    check(flags="--listy=-[1,5,6]", env_val="+[6,7]", config_val="+[4,5]", expected=[2, 3, 4, 7])
    check(flags="--listy=+[8,9]", env_val="-[4,5]", config_val="+[4,5],-[3]", expected=[1, 2, 8, 9])

    # Overwriting from env, then appending and filtering.
    check(flags="--listy=+[8,9],-[6]", env_val="[6,7]", config_val="+[4,5]", expected=[7, 8, 9])

    # Overwriting from config, then appending.
    check(flags="--listy=+[8,9]", env_val="+[6,7]", config_val="[4,5]", expected=[4, 5, 6, 7, 8, 9])

    # Overwriting from flags.
    check(flags="--listy=[8,9]", env_val="+[6,7]", config_val="+[4,5],-[8]", expected=[8, 9])

    # Filtering all instances of repeated values.
    check(flags="--listy=-[5]", config_val="[1, 2, 5, 3, 4, 5, 6, 5, 5]", expected=[1, 2, 3, 4, 6])

    # Filtering a value even though it was appended again at a higher rank.
    check(flags="--listy=+[4]", env_val="-[4]", config_val="+[4,5]", expected=[*default, 5])

    # Filtering a value even though it was appended again at the same rank.
    check(env_val="-[4],+[4]", config_val="+[4,5]", expected=[*default, 5])
  ...
  def test_dict_list_option(self) -> None:
    ...
    check(flags='--dict-listy=\'+[{"d": 4, "e": 5}, {"f": 6}]\'', expected=two_elements_appended)
  ...
  def test_no_recursive_subsystem_options(self) -> None:
    options = Options.create(
      env={}, config=self._create_config(), known_scope_infos=[subsystem("foo")], args=["./pants"]
    )
  ...
  def test_designdoc_example(self) -> None:
    ...
    # Get defaults from config and environment.
    config = {"DEFAULT": {"b": "99"}, "compile": {"a": "88", "c": "77"}}
  ...
  def test_file_spec_args(self) -> None:
    with temporary_file(binary_mode=False) as tmp:
      ...
      flags = (
        f'--target-spec-file={tmp.name} --pants-config-files="[]" ' "compile morx:tgt fleem:tgt"
      )
  ...
  def test_deprecated_options(self) -> None:
    ...
    assert_deprecation_triggered(
      config={"GLOBAL": {"global_crufty": "crufty1"}}, option="global_crufty", expected="crufty1"
    )
    assert_deprecation_triggered(
  ...
  def test_mutually_exclusive_options(self) -> None:
    ...
    assert_mutually_exclusive_raised(flags="--new-name=foo", config={"GLOBAL": {"old_name": "bar"}})
    assert_mutually_exclusive_raised(
  ...
  def test_middle_scoped_options(self) -> None:
    ...
    options = self._parse(flags="--a=100 ", config={"compile": {"a": 99}}, env={"PANTS_A": "100"})
    self.assertEqual(100, options.for_global_scope().a)
    self.assertEqual(100, options.for_scope("compile").a)
    self.assertEqual(100, options.for_scope("compile.java").a)
  ...
  def test_fromfile_env(self) -> None:
    def parse_func(dest, fromfile):
      return self._parse(flags="fromfile", env={f"PANTS_FROMFILE_{dest.upper()}": f"@{fromfile}"})
  ...
  def test_pants_global_designdoc_example(self) -> None:
    # The example from the design doc.
    # Get defaults from config and environment.
    config = {"GLOBAL": {"b": "99"}, "compile": {"a": "88", "c": "77"}}
  ...
  def test_scope_deprecation(self) -> None:
    ...
    options = Options.create(
      ...
      known_scope_infos=[DummyOptionable1.get_scope_info(), DummyOptionable2.get_scope_info()],
