...
class SubsystemDependency(OptionableFactory):
    """Indicates intent to use an instance of `subsystem_cls` scoped to
    `scope`."""
    ...
    def options_scope(self):
        ...
        This is the scope that option values are read from when
        initializing the instance indicated by this dependency.
        """
        if self.is_global():
...
class SubsystemClientMixin:
    ...
    def subsystem_dependencies_iter(cls):
        """Iterate over the direct subsystem dependencies of this
        Optionable."""
    ...
    def subsystem_closure_iter(cls):
        """Iterate over the transitive closure of subsystem dependencies of
        this Optionable.
    ...
    def known_scope_infos(cls):
        """Yield ScopeInfo for all known scopes for this optionable, in no
        particular order.
