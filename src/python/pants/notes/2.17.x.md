...
The `scie-pants` launcher has numerous benefits, such as:
...
Related to this, pants' distribution model is changing, and 2.17 will be the last version that includes the universal (cross-platform) `pants` PEX on the GitHub Release. For future releases, smaller per-platform PEXes will be attached instead.
