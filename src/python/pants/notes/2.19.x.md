...
The deprecation has expired for the `[GLOBAL].remote_store_chunk_upload_timeout_seconds` and `[GLOBAL].remote_cache_read_timeout_millis` options. Use [`[GLOBAL].remote_store_rpc_timeout_millis`](https://www.pantsbuild.org/v2.19/docs/reference-global#remote_store_rpc_timeout_millis) and [`[GLOBAL].remote_cache_rpc_timeout_millis`](https://www.pantsbuild.org/v2.19/docs/reference-global#remote_cache_rpc_timeout_millis) instead.
...
`TestResult.from_fallible_process_result` now accepts a tuple of multiple `FallibleProcessResult`s, instead of just one. ([#19760](https://github.com/pantsbuild/pants/pull/19760)).
