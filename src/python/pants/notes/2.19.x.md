...
The [docker backend](https://www.pantsbuild.org/2.19/docs/docker) now has explicit support for using [`buildx` (and thus BuildKit)](https://github.com/docker/buildx#buildx), via [the new `use_buildx` option](https://www.pantsbuild.org/v2.19/docs/reference-docker#use_buildx). This also allows adding few additional fields for [`docker_image` targets](https://www.pantsbuild.org/v2.19/docs/reference-docker_image), for `buildx`-specific functionality:
