...
The ``1.4.0`` stable release, with no addition changes since the rc5 release. Thanks for your patience!
...
* Both pytest and junit now have support for ``--chroot`` and a ``files(..)`` target was added,
  to support loose-file isolation in unit tests, and improving the safety of test caching.
...
* PEX files support bringing interpretter constraints with them to their runtime
