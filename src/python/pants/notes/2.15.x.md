...
configured as desired. Pants previously supported local execution (which is now called a `local_environment`)
and remote execution via the Remote Execution API (which is now called a `remote_environment`). Pants 2.15
also supports executing build actions within a Docker container via the `docker_environment` target type.
(Note: `docker_environment` is a distinct concept from the Docker backend which actually builds images.)
...
`pants.backend.experimental.openapi.lint.spectral` for linting support, and
`pants.backend.experimental.openapi.codegen.java` for Java code generation support.

#### Go 

The Go backend received several minor changes including:
...
Several changes and additions were made to the Plugin API:

#### Formatters

The schema used for formatters and linters has been changed in a backwards incompatible way in order to allow for
formatters that do not require a target to operate.

#### Synthetic Targets

Plugins may now add "synthetic targets" to the build graph to create freestanding targets programmatically. 
These synthetic targets can also be generator targets in order to further create generated targets.
