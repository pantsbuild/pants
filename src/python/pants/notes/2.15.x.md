...
`./pants test` now supports batching compatible tests together so that they execute in the same build action.
This will help to improve the performance of test execution by avoiding the overhead of setting up the
execution sandbox unnecessarily when running multiple tests. The Python backend is the first user of this
