...
class AnonymousTelemetryCallback(WorkunitsCallback):
    def __init__(self, anonymous_telemetry: AnonymousTelemetry) -> None:
        super().__init__()
        self._enabled = anonymous_telemetry.enabled
        self._unhashed_repo_id = anonymous_telemetry.repo_id

        # We log here instead of in __call__ as the call may be async and not log to the console.
        ...
        if self._enabled:
            if self._unhashed_repo_id is None:
                logger.error(
                ...
                self._enabled = False
            elif not self.validate_repo_id(self._unhashed_repo_id):
                logger.error(
    ...
    def __call__(
        ...
        *,
        ...
        if not finished or not self._enabled or self._unhashed_repo_id is None:
            return
