...
def _create_stable_task_type(superclass, options_scope):
    """Creates a singleton (via `memoized`) subclass instance for the given
    superclass and scope.
...
class Goal:
    ...
    def by_name(cls, name):
        """Returns the unique object representing the goal of the specified
        name.
    ...
    @classmethod
    def subsystems(cls):
        """Returns all subsystem types used by all tasks, in no particular
        order.
...
class _Goal(object):
    ...
    def subsystems(self):
        """Returns all subsystem types used by tasks in this goal, in no
        particular order."""
        ret = set()
    ...
    def has_task_of_type(self, typ):
        """Returns True if this goal has a task of the given type (or a subtype
        of it)."""
        for task_type in self.task_types():
    ...
    def active(self):
        ...
        Some goals are installed in pants core without associated tasks
        in anticipation of plugins providing tasks that implement the
        goal being installed. If no such plugins are installed, the goal
        may be inactive in the repo.
        """
