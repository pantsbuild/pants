...
class UnionProducts:
    ...
    def add_for_target(self, target, products):
        """Updates the products for a particular target, adding to existing
        entries.
    ...
    def add_for_targets(self, targets, products):
        """Updates the products for the given targets, adding to existing
        entries.
    ...
    def remove_for_target(self, target, products):
        """Updates the products for a particular target, removing the given
        existing entries.
    ...
    def get_for_targets(self, targets):
        """Gets the union of the products for the given targets, preserving the
        input order.
    ...
    def get_product_target_mappings_for_targets(self, targets):
        """Gets the product-target associations for the given targets,
...
class Products:
    """An out-of-band 'dropbox' where tasks can place build product information
    for later tasks to use.
    ...
    class ProductMapping:
        """Maps products of a given type by target. Each product is a map from
        basedir to a list of files in that dir.
      ...
      """
        ...
        def get(self, target):
            """Returns the product mapping for the given target as a tuple of
            (basedir, products list). Can return None if there is no mapping
        ...
        def __getitem__(self, target):
            ...
            Returns the product mapping for the given target as a map of
            <basedir> -> <products list>. If no mapping exists, returns
            an empty map whose values default to empty lists. So you can
            use the result without checking for None.
            """
            return self.by_target[target]

        def itermappings(self):
            """Returns an iterable over all pairs (target, product) in this
            mapping. Each product is itself a map of <basedir> -> <products
            list>.
        ...
        def keys_for(self, basedir, product):
            """Returns the set of keys the given mapped product is registered
            under.
    ...
    def require(self, typename):
        """Registers a requirement that file products of the given type by
        mapped.
    ...
    def register_data(self, typename, value):
        """Registers a data product, raising if a product was already
        registered.
    ...
    def get_only(self, product_type, target):
        """If there is exactly one product for the given product type and
        target, returns the full filepath of said product.
