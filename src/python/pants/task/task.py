...
class TaskBase(SubsystemClientMixin, Optionable, metaclass=ABCMeta):
    ...
    def stable_name(cls):
        ...
        We synthesize subclasses of the task types at runtime, and these synthesized subclasses may
        have random names (e.g., in tests), so this gives us a stable name to use across runs, e.g.,
        in artifact cache references.
        """
    ...
    def cache_target_dirs(self):
        """Whether to cache files in VersionedTarget's results_dir after exiting an invalidated
        block.
    ...
    def cache_incremental(self):
        """For incremental tasks, indicates whether the results of incremental builds should be
        cached.
    ...
    def check_artifact_cache_for(self, invalidation_check):
        ...
        By default we check for each invalid target. Can be overridden, e.g., to instead check only
        for a single artifact for the entire target set.
        """
