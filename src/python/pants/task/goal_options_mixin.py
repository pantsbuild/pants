...
class GoalOptionsRegistrar(Optionable):
    ...
    This is useful when you want the possibility of setting the value
    for all tasks at once and/or setting them per-task. E.g., turning
    all linters on or off, or turning individual linters on or off
    selectively.
    """
    ...
    def registrar_for_scope(cls, goal):
        """Returns a subclass of this registrar suitable for registering on the
        specified goal.

        Allows reuse of the same registrar for multiple goals, and also
        allows us to decouple task code from knowing which goal(s) the
        task is to be registered in.
        """
