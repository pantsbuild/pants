    ...
    with self.best_url_selector.select_best_url() as best_url:
      ...
      try:
        if "PUT" == method:
          ...
          response = session.head(url, timeout=self._read_timeout_secs, allow_redirects=True)
        elif "DELETE" == method:
          response = session.delete(url, timeout=self._write_timeout_secs, allow_redirects=True)
        else:
          raise ValueError("Unknown request method {0}".format(method))
      except RequestException as e:
        raise NonfatalArtifactCacheError("Failed to {0} {1}. Error: {2}".format(method, url, e))
