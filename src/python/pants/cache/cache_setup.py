...
class CacheFactory(object):
  ...
  def make_task_cache_dirname(task):
    """Generate a cache directory name specific to a Task instance.

    :param task: Task to cache results for.
    :return: string to use as a directory name.
    """
    return '{}-{}'.format(task.stable_name(), task.fingerprint)

  @staticmethod
  def make_task_cache_glob_str(task_cls):
    """Generate a glob which will locate cache directories generated by a
    CacheFactory, for instances of a Task subclass.

    :param task_cls: Task subclass to glob for.
    :return: String to use in a glob.
    """
