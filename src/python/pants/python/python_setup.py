...
class PythonSetup(Subsystem):
    ...
    def register_options(cls, register):
        ...
        register(
            ...
            advanced=True,
            help=(
                 "The maximum number of concurrent jobs to resolve wheels with. Because Pants "
                 "can run multiple subprocesses in parallel, the total parallelism will be "
                 "`--process-execution-local-parallelism x --python-setup-resolver-jobs`. Setting "
                 "this option higher may result in better parallelism, but, if set too high, may "
                 "result in starvation and Out of Memory errors."
            ),
        )
