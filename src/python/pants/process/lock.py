...
class OwnerPrintingInterProcessFileLock(InterProcessLock):
    ...
    def missing_message_output(self):
        return "Pid {} waiting for a file lock ({}), but there was no message at {} indicating who is holding it.".format(
            os.getpid(), self.path, self.message_path
        )
        ...
        if not self.acquired:
            try:
                with open(self.message_path, "rb") as f:
                    ...
                    output = "PID {} waiting for a file lock ({}) held by: {}".format(
                        os.getpid(), self.path, message
                    )
            except IOError as e:
