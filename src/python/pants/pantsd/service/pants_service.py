...
class PantsService(ABC):
    ...
    def setup(self, services):
        """Called before `run` to allow for service->service or other side-
        effecting setup.
    ...
    @abstractmethod
    def run(self):
        """The main entry-point for the service called by the service
        runner."""

    def mark_pausing(self):
        """Triggers pausing of the service, without waiting for it to have
        paused.
...
class _ServiceState(object):
    """A threadsafe state machine for controlling a service running in another
    thread.
    ...
    def mark_pausing(self):
        """Requests that the service move to the Paused state, without waiting
        for it to do so.
    ...
    def mark_terminating(self):
        """Requests that the service move to the Terminating state, without
