...
* One of your ``BUILD`` files needs a target that represents
  ``netrc`` auth::
...
* Your ``'auth'`` section for that repository should
...
To get Pants publish working in the first place,
someone needs to :ref:`configure your codebase <setup_publish>` to register one
or more artifact repositories and, optionally, enable some features.
