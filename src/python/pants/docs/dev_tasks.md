...
- `action`: A string action that specifies that the registered option should something other than
  set a literal value. The two most common actions are:
...
- `advanced`: Indicates that an option is intended either for use by power users, or for use in
  pants.ini. By default, advanced options are hidden in `./pants help`.
