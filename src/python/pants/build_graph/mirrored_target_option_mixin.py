...
class MirroredTargetOptionDeclaration:
    """An interface for operations to perform on an option which may also be
    set on a target."""
...
class MirroredTargetOptionMixin(ABC):
    """Get option values which may be set in this subsystem or in a Target's
    keyword argument.
    ...
    def mirrored_target_option_actions(self):
        """Subclasses should override and return a dict of (subsystem option
        name) -> selector function.

        A selector is a 1-argument function accepting a target and
        returning a value, or None. This property should return a dict
        mapping this subsystem's options attribute name (with
    ...
    def get_scalar_mirrored_target_option(self, option_name, target):
        """Get the attribute `field_name` from `target` if set, else from this
        subsystem's options."""
