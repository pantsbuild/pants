...
class Scm(ABC):
    """Abstracts high-level scm operations needed by pants core and pants
    tasks.
    ...
    def changed_files(self, from_commit=None, include_untracked=False, relative_to=None):
        """Returns a list of files with uncommitted changes or else files
        changed since from_commit.
    ...
    def changes_in(self, diffspec, relative_to=None):
        """Returns a list of files changed by some diffspec (eg sha, range,
        ref, etc)
    ...
    def changelog(self, from_commit=None, files=None):
        """Produces a changelog from the given commit or the 1st commit if none
        is specified until the present workspace commit for the changes
        affecting the given files.
    ...
    def tag(self, name, message=None):
        """Tags the state in the local workspace and ensures this tag is on the
        server.
    ...
    @abstractmethod
    def push(self):
        """Push the current branch of the local repository to the corresponding
        local branch on the server.
