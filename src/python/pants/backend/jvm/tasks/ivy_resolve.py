...
class IvyResolve(IvyTaskMixin, NailgunTask):
  ...
  def register_options(cls, register):
    ...
    register(
      "--open",
      type=bool,
      help="Attempt to open the generated ivy resolve report " "in a browser (implies --report)",
    )
  ...
  def _generate_ivy_report(self, result):
    def make_empty_report(report, organisation, module, conf):
      no_deps_xml_template = dedent(
        ...
        organisation=organisation, module=module, conf=conf, timestamp=time.strftime("%Y%m%d%H%M%S")
      )
