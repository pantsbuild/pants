...
class PomWriter:
    ...
    def _as_versioned_jar(self, internal_target):
        """Fetches the jar representation of the given target, and applies the latest pushdb
        version."""
...
def target_internal_dependencies(target):
    ...
    Directly declared deps are those that are explicitly listed in the definition of a target,
    rather than being depended on transitively. But in order to walk through aggregator targets such
    as `target`, `dependencies`, or `jar_library`, this recursively descends the dep graph and stops
    at Jarable instances.
    """
...
class JarPublish(HasTransitiveOptionMixin, ScmPublishMixin, JarTask):
    ...
    * ``--restrict-push-branches`` - Optional list of branches to restrict publishing to.
    ...
    def confirm_push(self, coord, version):
        """Ask the user if a push should be done for a particular version of a particular
        coordinate.
