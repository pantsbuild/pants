...
class ClasspathProducts(object):
  ...
  def get_for_targets(self, targets, context=None):
    ...
    :rtype: list of (string, string)
    """
    cp_entries = self.get_classpath_entries_for_targets(targets, context=context)
    return [(conf, cp_entry.path) for conf, cp_entry in cp_entries]

  def get_classpath_entries_for_targets(self, targets, respect_excludes=True, context=None):
    """Gets the classpath products for the given targets.
    ...
    :param pants.goal.context.Context context: If not `None`, will attempt to materialize any
      classpath entries to disk which were generated by hermetic execution and which have not
      already been materialized to disk.
    ...
    classpath_entries = list(OrderedSet([
      cp for cp, target in self.get_product_target_mappings_for_targets(targets, respect_excludes)
    ]))

    if context is not None:
      missing_entries = [
        entry for (scope, entry) in classpath_entries
  ...
  def get_artifact_classpath_entries_for_targets(self, targets, respect_excludes=True, context=None):
  ...
  def get_internal_classpath_entries_for_targets(self, targets, respect_excludes=True, context=None):
  ...
  def __str__(self):
    return "ClasspathProduct(\n  classpaths={}, \n  excludes={}\n)".format(self._classpaths, self._excludes)
