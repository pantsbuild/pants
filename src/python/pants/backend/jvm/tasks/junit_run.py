...
from pants.java.junit.junit_xml_parser import (
    RegistryOfTests,
    Test,
...
class JUnitRun(PartitionedTestRunnerTaskMixin, JvmToolTaskMixin, JvmTask):
    ...
    def register_options(cls, register):
        ...
        register(
            ...
            choices=[
                "allow-all",
                "disallow",
                "disallow-leaks-from-test-cases",
                "disallow-leaks-from-test-suites",
            ],
            default="allow-all",
            advanced=True,
            fingerprint=True,
    ...
    def _args(self, fail_fast, output_dir):
        ...
        if options.integrated_security_manager:
            ...
            args.append(
                "-security-thread-handling={}".format(
                    thread_lookup[options.security_manager_threads]
                )
            )
            args.append(
                "-security-exit-handling={}".format(
                    exit_lookup[options.security_manager_allow_exits]
                )
            )
            args.append(
                "-security-network-handling={}".format(
