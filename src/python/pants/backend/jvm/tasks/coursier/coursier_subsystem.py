...
class CoursierSubsystem(Subsystem):
  ...
  """
    ...
    def bootstrap_coursier(self, workunit_factory):
        ...
        coursier_bootstrap_dir = os.path.join(
            opts.pants_bootstrapdir, "tools", "jvm", "coursier", opts.version
        )

        bootstrap_jar_path = os.path.join(coursier_bootstrap_dir, "coursier.jar")

        if not os.path.exists(bootstrap_jar_path):
            with workunit_factory(
                name="bootstrap-coursier", labels=[WorkUnitLabel.TOOL]
            ) as workunit:
                with safe_concurrent_creation(bootstrap_jar_path) as temp_path:
                    ...
                    try:
                        ...
                        raise self.Error(
                            "Problem fetching the coursier bootstrap jar! {}".format(e)
                        )
                    else:
