...
class Scoverage(CoverageEngine):
    ...
    def _iter_datafiles(self, output_dir):
        ...
        and all measurement files are called "scoverage.measurements.<Thread
        ID>".

        This function is used in [instrument(output_dir)] function below
        to clean up all pre-existing scoverage files before generating
        new ones.
        """
    ...
    def _iter_datadirs(self, output_dir):
        ...
        Returns the parent directories under which all the scoverage
        data (for all targets) is stored. Currently, since all scoverage
        data for a test target is stored under `scoverage/measurements`,
