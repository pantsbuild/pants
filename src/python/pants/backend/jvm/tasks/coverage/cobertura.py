...
class Cobertura(CoverageEngine):
  ...
  def instrument(self, output_dir):
    ...
    if len(files_to_instrument) > 0:
      ...
      args = ["--basedir", self._settings.workdir, "--datafile", self._canonical_datafile]

      if self._include_user_classpath:
        closure = BuildGraph.closure(
          self._targets, bfs=True, include_scopes=Scopes.JVM_TEST_SCOPES, respect_intransitive=True
        )
      ...
      with temporary_file(binary_mode=False) as tmp_file:
        ...
        if result != 0:
          raise TaskError(
            "java {0} ... exited non-zero ({1})" " 'failed to instrument'".format(main, result)
          )
  ...
  def _execute_cobertura(self, workunit_name, tool_classpath, main, args):
    ...
    if result != 0:
      raise TaskError(
        "java {} ... exited non-zero ({}) - failed to {}".format(main, result, workunit_name)
      )
