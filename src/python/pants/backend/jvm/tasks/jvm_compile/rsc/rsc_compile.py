...
class RscCompile(ZincCompile):
  ...
  def _create_compile_jobs(self, compile_contexts, invalid_targets, invalid_vts, classpath_product):
    jobs = super(RscCompile, self)._create_compile_jobs(compile_contexts, invalid_targets,
      invalid_vts, classpath_product)

    # Create a target for the jdk outlining so that it'll only be done once per run.
    ctx = RscCompileContext(
      target=None,
      analysis_file=None,
      classes_dir=None,
      jar_file=None,
      zinc_args_file=None,
      rsc_mjar_file=os.path.join(self.workdir, '--jdk--', 'm.jar'),
      log_dir=os.path.join(self.workdir, '--jdk--', 'logs'),
      sources=[],
      rsc_index_dir=os.path.join(self.workdir, '--jdk--', 'index'),
      rsc_outline_dir=os.path.join(self.workdir, '--jdk--', 'outline'),
    )
    target = self.context.add_new_target(
      Address(fast_relpath(ctx.rsc_index_dir, get_buildroot()), 'jdk'), Target, )

    # Save the target off so that it can be referenced later
    self._jvm_lib_target = target
    def work_for_vts_rsc_jdk(ctx):
      cp_entries = []
      ...
      #TODO maybe keep if can pass to something----------------------------
      ...
      with Timer() as timer:
        ...
        metacp_wu = self._runtool(
          ...
          tgt='jdk',
          input_files=tuple(
            #  dependency_classpath + scalac_classpath_path_entries + classpath_rel
          ),
        ...
        metai_classpath = self._collect_metai_classpath(
          metacp_result, tuple(), tuple(jvm_lib_jars_abs))
        ...
        abs_output = [(conf, os.path.join(get_buildroot(), x))
          for conf in self._confs for x in metai_classpath if x] # TODO where is the empty x coming from ?????
        print(metai_classpath)
    ...
    synthetic_jobs = [
      ...
      #       zinc_args_file=None,
      #       sources=None,
      #       rsc_index_dir=os.path.join(self.workdir, '--jdk--'),
      #       rsc_outline_dir=None
      #     )),
      #   [self._rsc_key_for_target(target) for target in invalid_dependencies],
      #   self._size_estimator([]),
      # )
    ]
    return synthetic_jobs + jobs
  ...
  def create_compile_jobs(self,
    ...
    def work_for_vts_rsc_jar_library(vts, ctx):
      ...
      dependency_classpath = self._zinc.compile_classpath(
        ...
        ctx.target,
        extra_cp_entries=self._extra_compile_time_classpath)
      dependency_classpath.extend(self._jvm_lib_jars_abs)
  ...
  def _collect_metai_classpath(self, metacp_result, classpath_rel, jvm_lib_jars):
    ...
    def desandboxify_pantsd_loc(path):
      if prefix and path.startswith(prefix):
        ...
        return os.path.join('.pants.d', path.rsplit('.pants.d/')[-1])
      else:
        return path
