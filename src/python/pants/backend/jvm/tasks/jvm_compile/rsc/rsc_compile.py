...
class RscCompile(ZincCompile, MirroredTargetOptionMixin):
  ...
  def register_options(cls, register):
    ...
    cls.register_jvm_tool(
      register,
      "rsc",
      classpath=[JarDependency(org="com.twitter", name="rsc_2.12", rev="0.0.0-768-7357aa0a")],
    ...
    cls.register_jvm_tool(
      ...
      "scalac-outliner",
      classpath=[
        JarDependency(org="org.scala-lang", name="scala-compiler", rev=scalac_outliner_version)
      ],
      custom_rules=[
      ...
      if not vts.valid:
        ...
        with Timer() as timer:
          ...
          if use_youtline and not hermetic and self.get_options().zinc_outline:
            ...
            args = (
              ["-cp", os.pathsep.join(classpath_entry_paths), "-d", rsc_jar_file_relative_path]
        ...
        self._record_target_stats(
          tgt, len(classpath_entry_paths), len(target_sources), timer.elapsed, False, outliner
        )
  ...
  def _runtool_hermetic(self, main, tool_name, distribution, input_digest, ctx):
    ...
    if not use_youtline and self._rsc.use_native_image:
      ...
      initial_args = (
        [distribution.java] + rsc_jvm_options + ["-cp", os.pathsep.join(tool_classpath), main]
      )
    (argfile_snapshot,) = self.context._scheduler.capture_snapshots(
      [PathGlobsAndRoot(PathGlobs([fast_relpath(ctx.args_file, get_buildroot())]), get_buildroot())]
    )
  ...
  def _zinc_outline(self, ctx, relative_classpath, target_sources, youtline_args):
    ...
    scala_path = [
      relative_to_exec_root(classpath_entry.path) for classpath_entry in scalac_classpath_entries
    ]
