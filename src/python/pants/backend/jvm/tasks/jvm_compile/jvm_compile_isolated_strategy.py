...
class JvmCompileIsolatedStrategy(JvmCompileStrategy):
  ...
  def _compute_classpath_entries(self, compile_classpaths,
    ...
    compile_classpath_exclude_patterns = compile_classpath_excludes.get_for_target(compile_context.target)

    def is_excluded(entry):
      return any(excluded in entry for excluded in compile_classpath_exclude_patterns)

    compile_classpath = [(conf, entry) for conf, entry in raw_compile_classpath
                         if not is_excluded(entry)] + extra_compile_time_classpath
  ...
  def compile_chunk(self,
    ...
    compile_classpaths = self.context.products.get_data('compile_classpath')
    compile_classpath_exclude_patterns = self.context.products.get_data('compile_classpath_exclude_patterns')
    ...
    jobs = self._create_compile_jobs(compile_classpaths,
                                     compile_classpath_exclude_patterns,
                                     compile_contexts,
                                     extra_compile_time_classpath,
