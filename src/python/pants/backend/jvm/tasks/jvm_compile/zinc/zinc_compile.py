  ...
  def _compile_hermetic(self, jvm_options, ctx, classes_dir, jar_file,
    ...
    if self._zinc.use_native_image:
      ...
      image_specific_argv =  ['.jdk/bin/java'] + jvm_options + [
        # Put all the scala deps on the classpath, so that zinc can find them in the nailgun JVMs CWD.
        # Fingerprinting for the jvm is done with a digest of the -cp and not the content of the files.
        # TODO fingerprint scala jars deeply https://github.com/pantsbuild/pants/issues/8481
