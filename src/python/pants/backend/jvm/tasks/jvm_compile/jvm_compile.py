...
class JvmCompile(CompilerOptionSetsMixin, NailgunTaskBase):
    ...
    To subclass for a specific JVM language, implement the static values
    and methods mentioned below under "Subclasses must implement".
    """
    ...
    def post_compile_extra_resources(self, compile_context):
        """Produces a dictionary of any extra, out-of-band resources for a
        target.
    ...
    def post_compile_extra_resources_digest(
        ...
        """Compute a Digest for the post_compile_extra_resources for the given
        context."""
    ...
    def create_empty_extra_products(self):
        """Create any products the subclass task supports in addition to the
        runtime_classpath.
    ...
    def register_extra_products_from_contexts(self, targets, compile_contexts):
        ...
        It is called for valid targets at start, then for each completed
        invalid target, separately, during compilation.
        """

    def select_runtime_context(self, ccs):
        """Select the context that contains the paths for runtime classpath
        artifacts.
    ...
    def execute(self):
        ...
        with self.invalidated(
            ...
            self.do_compile(invalidation_check, compile_contexts, classpath_product)
    ...
    def do_compile(self, invalidation_check, compile_contexts, classpath_product):
        """Executes compilations for the invalid targets contained in a single
        chunk."""
    ...
    def _upstream_analysis(self, compile_contexts, classpath_entries):
        """Returns tuples of classes_dir->analysis_file for the closure of the
        target."""
    ...
    def create_compile_jobs(
        ...
        """Return a list of jobs, and a count of those jobs that represent
        meaningful ("countable") work."""
        ...
        double_check_cache_job = Job(
            self.exec_graph_double_check_cache_key_for_target(compile_target),
    ...
    def check_cache(self, vts):
        """Manually checks the artifact cache (usually immediately before
        compilation.)

        Returns true if the cache was hit successfully, indicating that
    ...
    def should_compile_incrementally(self, vts, ctx):
        """Check to see if the compile should try to re-use the existing
        analysis.

        Returns true if we should try to compile the target
        incrementally.
        """
    ...
    def _compute_sources_for_target(self, target):
        """Computes and returns the sources (relative to buildroot) for the
        given target."""
    ...
    def _plugin_targets(self, compiler):
        """Returns a map from plugin name to the targets that build that
        plugin."""
        if compiler == "javac":
    ...
    class _HermeticDistribution(object):
        ...
        def find_libs_path_globs(self, names):
            ...
            path_globs = PathGlobsAndRoot(PathGlobs(tuple(libs_unrooted)), self._underlying.home)
