...
class JvmCompile(NailgunTaskBase, GroupMember):
  ...
  def _register_vts(self, compile_contexts):
    ...
    def add_abs_resources(target, root_dir, resources):
      if resources_by_target is not None:
        resources_by_target[target].add_abs_paths(root_dir, resources)
      ...
      def add_products_by_target(files):
        for f in files:
          ...
          if clsname:
            ...
            add_abs_resources(target, classes_dir, resources)
          else:
            # Is a resource.
            add_abs_resources(target, classes_dir, [f])
    ...
    for compile_context in compile_contexts:
      ...
      # Add resources to products.
      for root, abs_paths in extra_resources:
        add_abs_resources(target, root, abs_paths)
