...
class JvmdocGen(JvmTask):
  ...
  def generate_doc(self, language_predicate, create_jvmdoc_command):
    ...
    with self.invalidated(targets) as invalidation_check:
      ...
      if self.combined:
        self._generate_combined(language_predicate, jvmdoc_targets, create_jvmdoc_command)
      else:
        self._generate_individual(language_predicate, jvmdoc_targets, create_jvmdoc_command)
  ...
  def _generate_combined(self, language_predicate, targets, create_jvmdoc_command):
    gendir = os.path.join(self.workdir, 'combined')
    if targets:
      classpath = self._doc_classpath(language_predicate, targets)
      safe_mkdir(gendir, clean=True)
      command = create_jvmdoc_command(classpath, classpath, gendir, *targets)
      if command:
  ...
  def _generate_individual(self, language_predicate, targets, create_jvmdoc_command):
    jobs = {}
    for target in targets:
      gendir = self._gendir(target)
      classpath = self._doc_classpath(language_predicate, [target])
  ...
  def _doc_classpath(self, lang_predicate, targets):
    ...
    # NB: Javadoc gets confused by class files generated by scalac if those classfiles share
    #     packages with the classes that are having docs generated for them.
    include_scopes = Scopes.JVM_RUNTIME_SCOPES
    classpath_product = self.context.products.get_data('runtime_classpath')
    closure = BuildGraph.closure(targets, bfs=True, include_scopes=include_scopes,
      respect_intransitive=True)
    closure = [t for t in closure if lang_predicate(t) or isinstance(t, JarLibrary)]
    return ClasspathUtil.classpath(closure, classpath_product, self.confs)
