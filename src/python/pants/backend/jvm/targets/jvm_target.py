...
class JvmTarget(Target, Jarable):
  ...
  def traversable_dependency_specs(self):
    ...
    for resource_spec in self._resource_specs:
      yield resource_spec
    if self.payload.get_field_value('sources'):
      # TODO(Eric Ayers): is it a hack to add this here?
      source_set = self.payload.sources.source_set
      if not source_set.is_synthetic:
        yield source_set.address.spec
