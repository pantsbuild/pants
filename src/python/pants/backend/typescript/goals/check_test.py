...
def parse_typescript_build_status(stdout: str) -> dict[str, bool]:
    ...
    return {
        ...
        'has_typescript_output': 'AM -' in stdout or 'PM -' in stdout,  # Basic check for TypeScript timestamp output
    }
...
def _create_rule_runner(package_manager: str) -> RuleRunner:
    """Create RuleRunner with given package manager. Cache isolation via unique build_roots."""
...
def basic_rule_runner(
    basic_project_test: tuple[str, str]
) -> tuple[RuleRunner, str, str]:
...
@pytest.fixture
def workspace_rule_runner(
    workspace_project_test: tuple[str, str]
) -> tuple[RuleRunner, str, str]:
...
def pnpm_rule_runner(
    pnpm_project_test: tuple[str, str]
) -> tuple[RuleRunner, str, str]:
...
def test_typescript_compilation_output_generation(
    ...
    for expected_file in expected_files:
        assert expected_file in snapshot.files, f"❌ Missing expected file: {expected_file}. Found: {sorted(snapshot.files)}"
...
def test_typescript_incremental_compilation_cache_rule_runner(
    ...
    assert os.path.exists(typescript_cache_dir), f"Cache directory not created: {typescript_cache_dir}"
    ...
    assert len(tsbuildinfo_files) > 0, f"No .tsbuildinfo files found in cache directory"
    print(f"✅ Cache created with {len(tsbuildinfo_files)} .tsbuildinfo file(s)")
    
    # Verify .tsbuildinfo content
    with open(tsbuildinfo_files[0], 'r') as f:
    ...
    if ts_status1['has_typescript_output']:
        assert ts_status1['full_build'], f"Should perform full build on first run. Output: {combined_output1}"
        print("✅ First run performed full build as expected")
    else:
        print("ℹ️  First run: No TypeScript verbose output detected (may not have --verbose enabled)")
    ...
    if ts_status2['has_typescript_output']:
        assert ts_status2['incremental_build'], f"Should use incremental build on second run. Output: {combined_output2}"
        assert not ts_status2['full_build'], f"Should NOT perform full build on second run. Output: {combined_output2}"
    ...
    if ts_status3['has_typescript_output']:
        assert ts_status3['full_build'], f"Should perform full build after file modification. Output: {combined_output3}"
        assert not ts_status3['incremental_build'], f"Should NOT report incremental when files changed. Output: {combined_output3}"
    ...
    if ts_status4['has_typescript_output']:
        assert ts_status4['incremental_build'], f"Should use incremental build after rebuild. Output: {combined_output4}"
        assert not ts_status4['full_build'], f"Should NOT perform full build when cache is valid. Output: {combined_output4}"
    ...
    print(f"   All 4 scenarios passed: cache creation → incremental → file change → incremental")
    print("="*60)
