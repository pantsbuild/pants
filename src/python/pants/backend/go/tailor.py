    ...
    for dirname, filenames in group_by_dir(unowned_go_files).items():
        putative_targets.append(
            PutativeTarget.for_target_type(
                GoPackage, path=dirname, name="pkg", triggering_sources=sorted(filenames)
            )
        )
...
async def find_putative_go_module_targets(
    ...
    for dirname, filenames in group_by_dir(unowned_go_mod_files).items():
        putative_targets.append(
            PutativeTarget.for_target_type(
                GoModule, path=dirname, name="mod", triggering_sources=sorted(filenames)
            )
        )
    ...
    for resolved_go_module in resolved_go_modules:
        for module_descriptor in resolved_go_module.modules:
            putative_targets.append(
                PutativeTarget.for_target_type(
                    GoExternalModule,
                    path=resolved_go_module.target.address.spec_path,
                    name=compute_go_external_module_target_name(
                        module_descriptor.path, module_descriptor.version
                    ),
                    triggering_sources=[],
                    kwargs={
                        "path": module_descriptor.path,
