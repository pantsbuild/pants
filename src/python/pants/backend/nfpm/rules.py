...
from pants.backend.nfpm.util_rules.sandbox import populate_nfpm_content_sandbox
from pants.backend.nfpm.util_rules.generate_config import generate_nfpm_yaml
...
async def package_nfpm_package(
    ...
    nfpm_content_sandbox = await populate_nfpm_content_sandbox(NfpmContentSandboxRequest(field_set), **implicitly())

    nfpm_config, downloaded_tool, output_dir_digest = await concurrently(
        generate_nfpm_yaml(NfpmPackageConfigRequest(field_set, nfpm_content_sandbox.digest), **implicitly()),
    ...
    stripped_digest = await remove_prefix_request_to_digest(RemovePrefix(result.output_digest, output_dir))
