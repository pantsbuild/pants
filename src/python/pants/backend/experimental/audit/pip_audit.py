...
class VulnerabilityData():
    ...
    fixed_in: List[str] # A list of versions that can be upgraded to that resolve the vulnerability.
...
def audit_constraints_strings(constraints_strings, session, excludes_ids) -> Dict[str, str]:
    ...
    for constraint_string in constraints_strings:
        ...
        if len(specifiers) != 1:
            raise ValueError("Unexpected specifier from a lockfile (not exactly one): {}", specifiers)
        ...
        vulnerabilities[str(requirement)] = [
            ...
            if result.vuln_id not in excludes_ids
        ]
    return vulnerabilities


def audit_constraints_string(package_name: str, version: str, session: requests.Session) -> Optional[str]:
