...
class CountLinesOfCode(ConsoleTask):
  ...
  def console_output(self, targets):
    ...
    directory_digest = self.context._scheduler.merge_directories(
      tuple(s.directory_digest for s in input_snapshots + (cloc_snapshot, list_file_snapshot))
    )
    ...
    req = ExecuteProcessRequest(
      argv=cmd, input_files=directory_digest, output_files=("ignored", "report"), description="cloc"
    )
