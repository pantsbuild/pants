...
_SCALAFIX_CONF_FILENAME = ".scalafix.conf"
...
@rule
async def gather_scalafix_config_files(request: GatherScalafixConfigFilesRequest) -> ScalafixConfigFiles:
    ...
    config_file_globs = [os.path.join(dir, _SCALAFIX_CONF_FILENAME) for dir in source_dirs_with_ancestors]
    ...
    for source_dir in source_dirs:
        config_file = find_nearest_ancestor_file(
            config_files_set, source_dir, _SCALAFIX_CONF_FILENAME
        )
        if not config_file:
            raise ValueError(
                f"No scalafmt config file (`{_SCALAFIX_CONF_FILENAME}`) found for "
...
@rule
async def partition_scalafix(request: ScalafixRequest.PartitionRequest, tool: ScalafixSubsystem) -> Partitions[PartitionInfo]:
    if tool.skip:
        return Partitions()
...
@rule
async def scalafix_fix(request: ScalafixRequest.Batch, jdk: InternalJdk, tool: ScalafixSubsystem, scala: ScalaSubsystem, scalac: Scalac) -> FixResult:
    partition_info = cast(PartitionInfo, request.partition_metadata)
    merged_digest = await Get(Digest, MergeDigests([partition_info.config_snapshot.digest, request.snapshot.digest]))

    result = await Get(ProcessResult, JvmProcess(
        jdk=jdk,
        argv=[
            "scalafix.cli.Cli",
            "--verbose",
