...
class SetuptoolsSCMDummySourceField(OptionalSingleSourceField):
    ...
    help = "A version generated by setuptools_scm"


class SetuptoolsSCMTagRegexField(StringField):
    default = None
    alias = "tag_regex"
    help = softwrap(
        """
        A Python regex string to extract the version part from an SCM tag.
        ...
        Note that this is unrelated to the tags field and to Pants's own tags concept.
...
class SetuptoolsSCMWriteToField(StringField):
    required = True
    alias = "write_to"
    help = "Generate the version data to this relative path."


class SetuptoolsSCMWriteToTemplateField(StringField):
    """A format string that is given the current version as the `version` format kwarg.

    E.g., 'version = "{version}"'

    See See https://github.com/pypa/setuptools_scm
    """

    required = True
    alias = "write_to_template"
    help = softwrap(
        """
...
class SetuptoolsSCMVersion(Target):
    alias = "setuptools_scm_version"
    core_fields = (
        *COMMON_TARGET_FIELDS,
    ...
    help = softwrap(
        f"""
        A version information generator, using setuptools_scm.

        Provides a constrained but useful subset of the full functionality of setuptools_scm,
        avoiding pitfalls in the interaction of setuptools_scm with Pants's hermetic environments.

        In particular, we ignore setuptools_scm config in pyproject.toml. Instead you must provide
