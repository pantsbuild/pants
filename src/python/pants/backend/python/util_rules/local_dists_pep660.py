...
if prepare_metadata is not None:
    metadata_path = prepare_metadata(metadata_dir, wheel_config_settings)
else:
    # Optional PEP 517 method not defined. Use build_wheel instead.
    wheel_path = backend.build_wheel(metadata_dir, wheel_config_settings)
    full_wheel_path = os.path.join(metadata_dir, wheel_path)

    import zipfile
    with zipfile.ZipFile(full_wheel_path, 'r') as zip_ref:
        file_names = zip_ref.namelist()
        dist_info_files = [n for n in file_names if ".dist-info" in n]
        zip_ref.extractall(metadata_dir, dist_info_files)
    os.unlink(full_wheel_path)

print("metadata_path: {{metadata_path}}".format(metadata_path=metadata_path))
"""
        ...
        CreateDigest(
            [
                FileContent(
                    ...
                    interpolate_backend_wrapper(dist_output_dir, request),
                ),
            ]
        ),
    )

    merged_digest = await Get(Digest, MergeDigests((request.input, backend_wrapper_digest)))
