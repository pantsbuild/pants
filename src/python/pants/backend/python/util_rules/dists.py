...
class DistBuildRequest:
    ...
    config_settings: FrozenDict[str, tuple[str, ...]] | None = None
...
config_settings = {config_settings_str}

wheel_path = backend.build_wheel(dist_dir, config_settings) if build_wheel else None
sdist_path = backend.build_sdist(dist_dir, config_settings) if build_sdist else None
...
def interpolate_backend_shim(dist_dir: str, request: DistBuildRequest) -> bytes:
    ...
    config_settings = (
        None
        if request.config_settings is None
        else {k: list(v) for k, v in request.config_settings.items()}
    )
    return _BACKEND_SHIM_BOILERPLATE.format(
        ...
        config_settings_str=distutils_repr(config_settings),
    ).encode()
