...
FORCED_NATIVE_BDIST_WHEEL = '''
from wheel.bdist_wheel import bdist_wheel

class wrapped_bdist_wheel(bdist_wheel):
...
class SetupPy(Task):
  ...
  def write_setup(self, root_target, reduced_dependencies, chroot):
    ...
    force_native = setup_keywords.pop('force_native', False)
    ...
    def generate_str(setup_keywords, force_native):
      converted = pprint.pformat(convert(setup_keywords))
      # HACK: If force_native is True, splice the `cmdclass` kwarg into the raw pformat'd
      # string to preserve an eval-time reference to the injected `wrapped_bdist_wheel` var.
      if force_native:
        assert converted[-1] == '}', 'unexpected dict conversion!'
        converted = ''.join((
          converted[:-1],
          ",\n 'cmdclass': {'bdist_wheel': wrapped_bdist_wheel}",
          converted[-1]
        ))
      return converted
    ...
    chroot.write(self._setup_boilerplate().format(
      setup_code=FORCED_NATIVE_BDIST_WHEEL if force_native else '',
      setup_dict=generate_str(setup_keywords, force_native),
