...
def test_resolve_stevedore_entry_points() -> None:
    ...
    rule_runner = RuleRunner(
        rules=[
            resolve_stevedore_entry_points,
            QueryRule(ResolvedStevedoreEntryPoints, (ResolveStevedoreEntryPointsRequest,)),
        ]
    )

    def assert_resolved(*, entry_point: str | None, expected: EntryPoint | None) -> None:
        plugin_name = "plugin"
    ...
    assert set(resolved.val) == {
        ...
        StevedoreEntryPoint("b", EntryPoint(module="custom.entry_point", function="func")),
