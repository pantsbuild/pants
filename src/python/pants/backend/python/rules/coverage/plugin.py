...
class SimpleFileReporter(PythonFileReporter):
  ...
  def __init__(self, relative_source_root, source_root_stripped_filename, coverage=None):
...
class ChrootRemappingPlugin(CoveragePlugin):
  ...
  def __init__(self, src_chroot_path, src_to_target_base):
  ...
  def file_reporter(self, filename):
    src = os.path.relpath(filename, self._src_chroot_path)
    target_base = self._src_to_target_base.get(src)
    return SimpleFileReporter(relative_source_root=target_base, source_root_stripped_filename=src)
