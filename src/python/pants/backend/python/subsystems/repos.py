...
class PythonRepos(Subsystem):
    ...
    path_mappings = StrListOption(
        help=softwrap(
            ...
            in `pants.toml`, you could set this option to `%(buildroot)s/python_wheels`
            ...
            recommend setting that each user set this option via a `.pants.rc` file
            ({doc_url('options#pantsrc-file')} or the environment variable
