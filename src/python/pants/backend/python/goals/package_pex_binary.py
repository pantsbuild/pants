...
async def package_pex_binary(
    ...
    if not entry_point.val:
        logger.warning(
            f"No entry point set for the `pex_binary` target {field_set.address}, so the binary "
            "built via the `package` goal will create a PEX that acts similarly to a virtual "
            f"environment's interpreter; for example, running "
            f"`{dist_dir.relpath / output_filename}` will open a REPL (with access to all of the "
            "binary's first and third-party dependencies).\n\nDid you mean to instead set the "
            "`sources` field or `entry_point` field? See "
