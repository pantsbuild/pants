...
class MultiMatcherTest(unittest.TestCase):
  ...
  def test_pattern_name_checks(self):
    bad_config1 = {"required_matches": {"unknown_path_pattern1": (), "unknown_path_pattern2": ()}}
    with self.assertRaisesRegex(
      ValueError,
    ...
    with self.assertRaisesRegex(
      ValueError, "required_matches uses unknown content " "pattern names: unknown_content_pattern1"
    ):
