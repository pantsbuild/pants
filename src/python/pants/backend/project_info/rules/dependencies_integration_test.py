...
class DependenciesIntegrationTest(GoalRuleTestBase):
  ...
  def test_no_target(self):
    self.assert_dependencies(target="", expected=[], transitive=False)

  def test_no_dependencies(self):
    self.create_python_library(path="some/target")
    self.assert_dependencies(target="some/target", expected=[], transitive=False)
    self.assert_dependencies(target="some/target", expected=[], transitive=True)
