...
class AntlrJavaGen(SimpleCodegenTask, NailgunTask):
    ...
    def _rearrange_output_for_package(self, target_workdir, java_package):
        ...
        Antlr emits a directory structure based on the relative path provided for the grammar file.
        If the source root of the file is different from the Pants build root, then the Java files
        end up with undesired parent directories.
        """
