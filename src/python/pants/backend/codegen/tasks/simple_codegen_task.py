...
class SimpleCodegenTask(Task):
  ...
  def _find_sources_in_workdir(self, target_workdir):
    ...
    for root, _, files in safe_walk(target_workdir):
      ...
      for name in files:
        if not self.exclude_source(rel_root, name):
          yield os.path.join(rel_root, name)

  def exclude_source(self, rel_root, name):
    """Sometimes a source file will be generated that we don't want to
    compile (e.g. it's an example or otherwise not relevant to our needs).
    Individual code generators can override this to exclude those.
    """
