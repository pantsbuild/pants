...
class SimpleCodegenTask(Task):
  ...
  def _find_sources_generated_by_dependencies(self, target, target_workdir):
    sources = OrderedSet()

    def add_sources(dep):
      if dep is not target:
        dep_sources = self._find_sources_generated_by_target(dep, target_workdir)
        dep_sources = [fast_relpath(source, target_workdir) for source in dep_sources]
  ...
  def _find_sources_strictly_generated_by_target(self, target, target_workdirs):
    ...
    by_dependencies = self._find_sources_generated_by_dependencies(target, target_workdir)
