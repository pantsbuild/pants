...
class SimpleCodegenTask(Task):
  ...
  def find_sources(self, target, target_workdir):
    ...
    :rtype: OrderedSet
    """
    return self._find_sources_strictly_generated_by_target(target, target_workdir)
  ...
  def _find_sources_strictly_generated_by_target(self, target, target_workdir):
    """Compute sources generated by only the given gen target, and not also by its deps.

    This method should only be called after all dependencies have already been built and injected
    into the graph.
    ...
    # If there were duplicates remove them from this target, and warn or error.
    if duplicates_by_target:
