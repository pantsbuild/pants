...
from pants.backend.codegen.protobuf.kotlin import dependency_inference
...
class GenerateKotlinFromProtobufRequest(GenerateSourcesRequest):
    input = ProtobufSourceField
    output = KotlinSourceField


@rule(desc="Generate Kotlin from Protobuf", level=LogLevel.DEBUG)
async def generate_kotlin_from_protobuf(
    request: GenerateKotlinFromProtobufRequest,
    protoc: Protoc,
) -> GeneratedSources:
    ...
    result = await Get(
        ...
        Process(
            ...
            description=f"Generating Kotlin sources from {request.protocol_target.address}.",
...
def rules():
    return [
        ...
        *dependency_inference.rules(),
        UnionRule(GenerateSourcesRequest, GenerateKotlinFromProtobufRequest),
