...
class DockerOptions(Subsystem):
    ...
    suggest_renames = BoolOption(
        ...
        removal_hint=softwrap(
            """
            Rename suggestions are now generated by the native engine and are fast even for very
            large build contexts, so this toggle is no longer necessary. Rename suggestions will
            always be shown when a `docker_image` build fails.
            """
        ),
    )
