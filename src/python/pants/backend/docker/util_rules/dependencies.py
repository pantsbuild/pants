...
async def inject_docker_dependencies(request: InjectDockerDependencies) -> InjectedDependencies:
    ...
    dockerfile_address = await Get(DockerfileAddress, Address, request.dependencies_field.address)
    dockerfile_info = await Get(DockerfileInfo, DockerfileInfoRequest(dockerfile_address.address))
    targets = await Get(
        Targets,
