...
class ChangedTargetTask(ChangedFileTaskMixin, NoopExecTask):
  """A base class for tasks which find changed targets to act on.

  Frequently other tasks already exist that actually do the desired work eg "compile" or "test".

  A subclass of ChangedTargetTask can be used to create a new goal that runs an existing tasks
  on what the SCM indicates are changed target rather than what the user specifies.

  This is done by schduling that goal or task, unaltered, and running it as usual, with the
  ...
  installed in a goal to and provide some known product type.
  """
  @classmethod
