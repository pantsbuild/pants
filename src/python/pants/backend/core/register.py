...
def register_goals():
  ...
  task(name='compile', action=NoopCompile).install('compile') # Stub for scheduling 'compile'.
  task(name='compile-changed', action=CompileChanged,
  ).install().with_description('Compile changed targets.')

  task(name='test', action=NoopTest).install('test') # Stub for scheduling 'test'.
  task(name='test-changed', action=TestChanged,
  ).install().with_description('Test changed targets.')
