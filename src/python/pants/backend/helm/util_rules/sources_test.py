...
def test_collects_file_sources_dependencies(rule_runner: RuleRunner) -> None:
    rule_runner.write_files(
        {
            "BUILD": dedent(
                """\
                helm_chart(name='mychart', dependencies=[':image'])

                file(name="image", source='image.png')
                """
            ),
            "Chart.yaml": HELM_CHART_FILE,
            "values.yaml": HELM_VALUES_FILE,
    ...
    tgt = rule_runner.get_target(address)
    source_files = rule_runner.request(
        HelmChartSourceFiles, [HelmChartSourceFilesRequest.create(tgt, include_files=True)]
    )

    assert "image.png" in source_files.snapshot.files
