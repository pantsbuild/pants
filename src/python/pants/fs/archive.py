...
class Archiver(ABC):
    def extract(self, path, outdir, concurrency_safe=False, **kwargs):
        """Extracts an archive's contents to the specified outdir with an
        optional filter.
    ...
    def create(self, basedir, outdir, name, prefix=None):
        """Creates an archive of all files found under basedir to a file at
        outdir of the given name.

        If prefix is specified, it should be prepended to all archive
        paths.
        """
...
class XZCompressedTarArchiver(TarArchiver):
    ...
    Invokes an xz executable to decompress a .tar.xz into a tar stream,
    which is piped into the extract() method.
    """
    ...
    def _invoke_xz(self, xz_input_file):
        ...
        This allows streaming the decompressed tar archive directly into
        a tar decompression stream, which is significantly faster in
        practice than making a temporary file.
        """
