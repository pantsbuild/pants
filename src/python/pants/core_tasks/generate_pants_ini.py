...
class GeneratePantsIni(ConsoleTask):
  ...
  def console_output(self, _targets):
    pants_ini = Path(get_default_pants_config_file())
    ...
    if pants_ini.exists():
      raise TaskError(
        f"{pants_ini} already exists. To update config values, please directly modify "
        f"pants.ini. For example, you may want to add these entries:\n\n{pants_ini_content}"
      )

    yield dedent(f"""\
      Adding sensible defaults to {pants_ini}:
      * Pinning `pants_version` to `{pants_version}`.
      """)

    pants_ini.write_text(pants_ini_content)
