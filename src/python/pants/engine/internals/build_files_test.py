...
@pytest.mark.parametrize(
    "include,exclude,file_name,expected",
    (
        ("BUILD", None, "BUILD", True),
        ("BUILD.foo", None, "BUILD", False),
        ("BUILD.*", None, "BUILD.foo", True),
        ("BUILD.*", "BUILD.foo", "BUILD.foo", False),
    ),
)
def test_build_file_options_matches(
    include: str, exclude: str | None, file_name: str, expected: bool
) -> None:
    options = BuildFileOptions((include,), (exclude,) if exclude else ())
    assert options.matches(file_name) is expected
