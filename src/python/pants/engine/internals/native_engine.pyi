...
# NB: Each stub should indicate whether it expects positional-only args (via `/`) or
# keyword-only args (via `*`). Free functions must use positional args, whereas class methods can
# use either style, but cannot mix and match within the method.
...
def stdio_thread_console_set(
    stdin_fileno: int, stdout_fileno: int, stderr_fileno: int, /
) -> None: ...
