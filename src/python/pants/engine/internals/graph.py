...
async def resolve_target(
    ...
    requested = next((tgt for tgt in generated if tgt.address == address), None)
    if requested is None:
        bulleted_list_sep = "\n  * "
        generated_str = bulleted_list_sep.join(tgt.address.spec for tgt in generated)
        raise ValueError(
            ...
            f"{bulleted_list_sep}{generated_str}\n\n"
            "Did you mean to use one of those addresses?"
        )
    return WrappedTarget(requested)
...
async def resolve_targets(
    ...
    expanded_targets.update(
        ...
        for tgt in (generated_targets if generated_targets else {generator})
    )
...
async def resolve_dependencies(
    ...
    if target_types_to_generate_requests.is_generator(tgt) and not tgt.address.is_file_target:
        ...
        generated_addresses = tuple(t.address for t in generated_targets)
    ...
    if (
        ...
        generated_addresses = tuple(
            t.address for t in generated_targets if t.address != request.field.address
        )
