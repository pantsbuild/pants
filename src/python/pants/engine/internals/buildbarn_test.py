...
from pants.engine.internals import buildbarn
...
def test_launcher_fails_when_manifest_lacks_storage_image(tmp_path: Path) -> None:
    ...
    launcher = buildbarn.BuildbarnLauncher(manifest_path=manifest_path, temp_dir=tmp_path)
...
def test_write_compose_logs_uses_compose_command(tmp_path: Path) -> None:
    ...
    launcher = buildbarn.BuildbarnLauncher(temp_dir=tmp_path, run_command=run_command)
    launcher._project_name = "pants-buildbarn-test"
    launcher._runtime_root = tmp_path
    launcher._env_file = tmp_path / "compose.env"
    launcher._compose_file = tmp_path / "docker-compose.yaml"
    (tmp_path / "logs").mkdir(parents=True, exist_ok=True)

    buildbarn._write_compose_logs(launcher)
