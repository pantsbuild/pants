...
@contextmanager
def buildbarn_cache_address() -> BuildbarnAddress:
...
def _run_process(process: Process, *, address: str, instance_name: str) -> tuple[dict[str, bytes], Digest, dict[str, int]]:
    ...
    time.sleep(1)
    return {file_content.path: file_content.content for file_content in contents}, result.output_digest, rule_runner.scheduler.get_metrics()
...
def test_buildbarn_remote_cache_roundtrips_file_and_directory_outputs() -> None:
    ...
    with buildbarn_cache_address() as buildbarn:
