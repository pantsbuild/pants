...
class StructTest(unittest.TestCase):
    def test_attribute_error_raised_in_property(self) -> None:
        """This tests that Struct#__getattr__ doesn't prevent correct attribution of
        AttributeErrors."""
        ...
        self.assertEqual(
            "'StructWithProperty' object has no attribute 'missing_attribute'", str(cm.exception)
        )
    ...
    def test_extend(self) -> None:
        extends = Struct(
            age=32, label="green", items=[], extends=Struct(age=42, other=True, items=[1, 2])
        )
    ...
    def test_merge(self) -> None:
        merges = Struct(
            ...
            items=[3],
            knobs={"b": False},
            merges=[Struct(age=42, other=True, items=[1, 2], knobs={"a": True, "b": True})],
        )

        # Merging is lazy, so we don't pick up the other field yet.
        self.assertNotEqual(
            Struct(age=32, items=[3, 1, 2], knobs={"a": True, "b": True}, other=True), merges
        )

        # But we do pick it up now.
        self.assertEqual(
            Struct(age=32, items=[3, 1, 2], knobs={"a": True, "b": True}, other=True),
            merges.create(),
        )
    ...
    def test_extend_and_merge(self) -> None:
        extends_and_merges = Struct(
            ...
            label="green",
            items=[5],
            extends=Struct(age=42, other=True, knobs={"a": True}, items=[1, 2]),
            merges=[
                Struct(age=52, other=False, items=[1, 3, 4], knobs={"a": False, "b": True}),
                Struct(items=[2]),
            ],
        )
        self.assertEqual(
