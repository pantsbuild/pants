...
# NB: We don't make this an `AsyncField` like you may expect because Struct and Addressable will
# have already done some hydration for us. To hydrate even further from Sequence[Address] ->
# Targets, use `await Get[Targets](Addresses(dependencies.value)`.
class Dependencies(PrimitiveField):
    ...
    # documentation, which is autogenerated from these type hints, to leak that.
    @classmethod
    def compute_value(
        cls, raw_value: Optional[Iterable[str]], *, address: Address
