...
class ProductDescription(datatype([("value", str)])):
    pass
...
class MultiPlatformExecuteProcessRequest(
    ...
    def __new__(cls, request_dict):
        ...
        if len({req.description for req in request_dict.values()}) != 1:
            raise ValueError(
                f"The `description` of all execute_process_requests in a {cls.__name__} must be identical."
            )

        return super().__new__(cls, validated_constraints, tuple(request_dict.values()))
...
class ExecuteProcessResult(
    datatype([("stdout", bytes), ("stderr", bytes), ("output_directory_digest", Digest)])
):
