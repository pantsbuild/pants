...
@pytest.mark.parametrize(
  ["name_to_deps", "expected_order"],
  [
    [{}, ()],
    [{"A": ()}, ("A",)],
    [{"A": (), "B": ("A",)}, ("A", "B")],
    [{"A": (), "B": ("A",), "C": ("B",), "D": ("A",), "E": ("C", "D")}, ("A", "B", "C", "D", "E")],
    [
...
def test_topo_sort(
  name_to_deps: Dict[str, Tuple[str, ...]], expected_order: Tuple[str, ...]
) -> None:
  ...
  assert expected_order == tuple(
    ht.address for ht in topo_sort(hts)
...
def test_topo_sort_filtered() -> None:
  ...
  name_to_deps: Dict[str, Tuple[str, ...]] = {
    "A": (),
    "B": ("A",),
    "C": ("B",),
    "D": ("A",),
  ...
  assert ("A", "B", "E") == tuple(
    ht.address for ht in topo_sort(filtered_hts)
  )  # type: ignore[comparison-overlap]
