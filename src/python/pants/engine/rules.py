...
def rule(*args, cacheable=True) -> Callable:
    ...
    if len(args) == 2:
        ...
        if not isinstance(return_type, type):
            raise ValueError(
                f"The return_type decorator parameter must be a type, "
        ...
        for index, parameter_type in enumerate(parameter_types):
            if not isinstance(parameter_type, type):
                raise ValueError(
                    f"The parameter_types decorator parameter must contain only types. "
    ...
    def __new__(cls, union_base, union_member):
        if not getattr(union_base, "_is_union", False):
            raise cls.make_type_error(
                "union_base must be a type annotated with @union: was {} (type {})".format(
                    union_base, type(union_base).__name__
                )
            )
...
functions decorated with @rule.""".format(
    ...
    class NormalizedRules(datatype(["rules", "union_rules"])):
        pass
