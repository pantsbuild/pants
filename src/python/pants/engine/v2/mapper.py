...
from pants.engine.v2.objects import Serializable
...
class AddressMap(datatype('AddressMap', ['path', 'objects_by_name'])):
  ...
  def parse(cls, filepath, filecontent, symbol_table_cls, parser_cls):
    ...
    :type parser_cls: A :class:`pants.engine.v2.parser.Parser`
    """
    try:
...
class AddressMapper(object):
  ...
  def __init__(self, symbol_table_cls, parser_cls, build_pattern=None):
    ...
    files can be customized.  See the `pants.engine.v2.parsers` module for example parsers.
