...
def addressable(type_constraint):
  ...
  :class:`pants.engine.v2.objects.Resolvable` person or else a string address pointing to one.
...
def addressable_list(type_constraint):
  ...
  Some (or all) elements of the list may be :class:`pants.engine.v2.objects.Resolvable` elements
  to resolve later.
...
def addressable_dict(type_constraint):
  ...
  Some (or all) values in the dict may be :class:`pants.engine.v2.objects.Resolvable` values to
  resolve later.
