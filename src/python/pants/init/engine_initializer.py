...
def _compute_default_sources_globs(base_class, target_type):
    """Look up the default source globs for the type, and return as a tuple of
    (globs, excludes)."""
...
def _apply_default_sources_globs(base_class, target_type):
    """Mutates the given TargetAdaptor subclass to apply default sources from
    the given legacy target type."""
...
def _make_target_adaptor(base_class, target_type):
    """Create an adaptor subclass for the given TargetAdaptor base class and
    legacy target type."""
    globs, excludes = _compute_default_sources_globs(base_class, target_type)
    if globs is None:
        return base_class
...
class LegacyGraphScheduler:
    """A thin wrapper around a Scheduler configured with @rules for a symbol
    table."""
...
class LegacyGraphSession:
    """A thin wrapper around a SchedulerSession configured with @rules for a
    symbol table."""
    ...
    def run_console_rules(self, options_bootstrapper, goals, target_roots):
        """Runs @console_rules sequentially and interactively by requesting
        their implicit Goal products.
...
class EngineInitializer:
    """Constructs the components necessary to run the v2 engine with v1
    BuildGraph compatibility."""
    ...
    def setup_legacy_graph(native, options_bootstrapper, build_configuration):
        """Construct and return the components necessary for LegacyBuildGraph
        construction."""
