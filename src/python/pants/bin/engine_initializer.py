...
class LegacySymbolTable(SymbolTable):
  ...
  @memoized_method
  def table(self):
    aliases = {alias: TargetAdaptor for alias in self.aliases().target_types}
...
class EngineInitializer(object):
  ...
  def setup_legacy_graph(pants_ignore_patterns,
    ...
    :param SymbolTable symbol_table: A SymbolTable instance to use for build file parsing, or
    ...
    scm = get_scm()
    symbol_table = symbol_table or LegacySymbolTable()

    project_tree = FileSystemProjectTree(build_root, pants_ignore_patterns)

    # Register "literal" subjects required for these tasks.
    # TODO: Replace with `Subsystems`.
    address_mapper = AddressMapper(parser=LegacyPythonCallbacksParser(symbol_table),
