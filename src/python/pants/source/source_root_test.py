...
def _find_root(
    ...
    def _mock_fs_check(pathglobs: PathGlobs) -> FileListing:
        for glob in pathglobs.globs:
            if glob in (existing_marker_files or []):
                d, f = os.path.split(pathglobs.globs[0])
                return FileListing(files=(f,), dirs=(d,))
        return FileListing(files=tuple(), dirs=tuple())
        ...
        return cast(
            ...
            run_rule_with_mocks(
                ...
                mock_gets=[
                    ...
                    MockGet(product_type=FileListing, subject_type=PathGlobs, mock=_mock_fs_check),
                ],
            ),
        )
...
def test_all_roots() -> None:
    ...
    def provider_rule(_: PathGlobs) -> FileListing:
        return FileListing((), dirs)
    ...
    output = run_rule_with_mocks(
        ...
        mock_gets=[
            MockGet(product_type=FileListing, subject_type=PathGlobs, mock=provider_rule),
            MockGet(
                product_type=OptionalSourceRoot,
                subject_type=SourceRootRequest,
