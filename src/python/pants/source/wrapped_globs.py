...
class FilesetWithSpec(AbstractClass):
  ...
  def empty(rel_root):
    ...
    return SnapshotBackedEagerFilesetWithSpec(rel_root, {'globs': []}, EMPTY_SNAPSHOT)
...
class SnapshotBackedEagerFilesetWithSpec(FilesetWithSpec):
  def __init__(self, rel_root, filespec, snapshot):
    """
    :param rel_root: The root for the given filespec, relative to the buildroot.
    :param filespec: A filespec as generated by `FilesetRelPathWrapper`, which represents
    ...
    """
    super(SnapshotBackedEagerFilesetWithSpec, self).__init__(rel_root, filespec)
  ...
  @memoized_property
  def files(self):
    return tuple(fast_relpath(fd.path, self.rel_root) for fd in self._snapshot.files)
  ...
  def __repr__(self):
    return 'SnapshotBackedEagerFilesetWithSpec(rel_root={!r}, snapshot={!r})'.format(self.rel_root, self._snapshot)
