...
object Settings {
  ...
  val options = Seq(
    ...
    boolean(   "-debug",                       "Set log level to debug",                     (s: Settings) => s.copy(consoleLog = s.consoleLog.copy(logLevel = Level.Debug))),
    string(    "-log-level", "level",          "Set log level (debug|info|warn|error)",      (s: Settings, l: String) => s.copy(consoleLog = s.consoleLog.copy(logLevel = Level.withName(l)))),
    boolean(   "-no-color",                    "No color in logging",                        (s: Settings) => s.copy(consoleLog = s.consoleLog.copy(color = false))),
    boolean(   "-log-phases",                  "Log phases of compilation for each file",    (s: Settings) => s.copy(consoleLog = s.consoleLog.copy(logPhases = true))),
