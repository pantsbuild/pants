...
final class ZincDiagnosticsReporter(reporter: Reporter)
  ...
  private def fixedDiagnosticMessage(d: Diagnostic[_ <: JavaFileObject]): String = {
    ...
    def fixWarnOrErrorMessage = {
      ...
      val lines: Seq[String] = tmp.split(END_OF_LINE_MATCHER) match {
  ...
  override def report(d: Diagnostic[_ <: JavaFileObject]) {
    ...
    val pos: xsbti.Position = new xsbti.Position {
