...
When we say `BUILD` files were named `BUILD`, we really meant `BUILD` or
...
Configure JVM Artifact Fetching
-------------------------------

Pants uses `ivy` to fetch artifacts. To specify where it should fetch those artifacts,
bring over a working `build-support/ivy/ivysettings.xml` file from a working Pants workspace
and tweak to fit your situation.
