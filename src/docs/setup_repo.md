...
BUILD files describe the dependency graph of your codebase.  You'll need to create some BUILD
files in order to use Pants.

+ How to write [[BUILD files|pants('src/docs:build_files')]].
...
on large binary blobs. E.g., you can implement this using Varnish in front of nginx (with the `dav_methods PUT DELETE;` enabled).
