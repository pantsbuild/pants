...
public class CollectionOptionHandler<T> extends DelimitedOptionHandler<T> {
  ...
  public interface ItemParser<T> {
     ...
     */
    ItemParser<String> IDENTITY = new ItemParser<String>() {
      @Override public String parse(String item) {
        return item;
      }
    };
   ...
   * @param defaultMetaVariable The default meta variable to use to describe the individual items
   *                            in the collection.
  ...
  public CollectionOptionHandler(
    ...
    super(parser, option, setter, ",", new OneArgumentOptionHandler<T>(parser, option, setter) {
      @Override protected T parse(String argument)  {
