...
* `default`: The default scope when it is not specified on a target. `default` targets are
  included on classpaths at both compiletime and runtime.
...
One effect of this behaviour is that you can introduce intermediate aliases to "re-scope" a target
when they are used in multiple ways:
