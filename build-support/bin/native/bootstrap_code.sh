...
function _build_binary_impl() {
  local native_engine_version="$1"
...
function bootstrap_native_code() {
  ...
  with_shared_cache_dir "$native_engine_version" "$target_dir" "$cur_fingerprint_file" \
                        "$maybe_cached_target_dir" \
                        _build_binary_impl "$native_engine_version"
