...
class CargoBin(datatype([('exe', Snapshot)])): pass
...
def obtain_cargo_bin_location(rustup, rustup_exe):
  ...
  yield CargoBin(cargo_exe)


class CargoInstallation(datatype(['cargo_installed_version'])): pass
...
def ensure_cargo_installed(rustup, cargo_bin):
  ...
  yield CargoInstallation(cargo_installed_version=rustup.cargo_version)
