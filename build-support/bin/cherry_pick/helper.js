class CherryPickHelper {
  ...
  async cherry_pick_failed(merge_commit_sha, milestone) {
    ...
    await this.octokit.rest.issues.createComment({
      ...
      body: `
        ...
        4. Run \`pants run build-support/bin/cherry_pick/make_pr.js -- --pull-number="${this.issue_context.pull_number}" --milestone="${milestone}"\`
        `.replace("        ", ""),
    });
  }
  ...
  async cherry_pick_to_milestone(milestone) {
    ...
    const title =
      pull.title + `(Cherry-pick of #${this.issue_context.pull_number})`;
    const body = pull.body;
    const author = pull.user.login;
    const category_label = pull.labels
      .map((label) => label.name)
      .filter((label_name) => label_name.startsWith("category:"))[0];
    const approvers = await this.octokit.rest.pulls
      .listReviews({
        ...this.issue_context,
      })
      .filter((review) => review.state === "APPROVED")
      .map((review) => review.user.login);
    const pull_reviewers = [author, ...approvers];
